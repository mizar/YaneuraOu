...
Bitboard ForwardRanksBB[COLOR_NB][RANK_NB] = {
    {ZERO_BB, RANK1_BB, RANK1_BB | RANK2_BB, RANK1_BB | RANK2_BB | RANK3_BB, RANK1_BB | RANK2_BB | RANK3_BB | RANK4_BB,
     ~(RANK9_BB | RANK8_BB | RANK7_BB | RANK6_BB), ~(RANK9_BB | RANK8_BB | RANK7_BB), ~(RANK9_BB | RANK8_BB),
     ~RANK9_BB},
...
Bitboard attackCalc(const Square square, const Bitboard& occupied, const bool isBishop) {
	// 飛車と角の利きの方角
	const SquareWithWall deltaArray[2][4] = {{SQWW_U, SQWW_D, SQWW_L, SQWW_R}, {SQWW_LU, SQWW_LD, SQWW_RD, SQWW_RU}};
...
#if defined(USE_BMI2)
			attacks[index + occupiedToIndex(occupied & blockMask[sq], blockMask[sq])] =
...
std::ostream& operator<<(std::ostream& os, const Bitboard& board) {
	for (Rank rank = RANK_1; rank <= RANK_9; ++rank) {
		for (File file = FILE_9; file >= FILE_1; --file) os << ((board & (file | rank)) ? " *" : " .");
		os << endl;
	}
...
Bitboard effects_from(Piece pc, Square sq, const Bitboard& occ) {
	switch (pc) {
		...
		case B_LANCE:
			return lanceEffect(BLACK, sq, occ);
		case B_KNIGHT:
		...
		case W_LANCE:
			return lanceEffect(WHITE, sq, occ);
		case W_KNIGHT:
...
void Bitboards::init() {
	...
	auto calcBishopEffectMask = [](Square sq, int n) {
		...
		for (Rank r = RANK_2; r <= RANK_8; ++r)
			for (File f = FILE_2; f <= FILE_8; ++f) {
				...
				if (abs(dr) == abs(df) && (!!((int)dr == (int)df) ^ n)) result ^= (f | r);
			}
	...
	for (auto c : COLOR)
		for (auto sq : SQ) {
			...
			if (pawn) {
				...
				if (pawn2) tmp = bishopEffect(sq2, ALL_BB) & RANK_BB[rank_of(pawn2.pop())];
			}
	...
	for (auto sq : SQ) PAWN_DROP_MASKS[sq] = ~FILE_BB[SquareToFile[sq]].p[Bitboard::part(sq)];
	...
	for (auto Us : COLOR)
		for (auto ksq : SQ) {
			...
			if (enemy_field(Us) & ksq) {
				if (file_of(ksq) != FILE_1) target |= lanceStepEffect(them, ksq + SQ_R);
				if (file_of(ksq) != FILE_9) target |= lanceStepEffect(them, ksq + SQ_L);
			}
			CheckCandidateBB[ksq][LANCE - 1][Us] = target;

			// 桂で王手になる可能性のあるものは、ksqに敵の桂をおいたところに移動できる桂(不成) +
			// ksqに金をおいた範囲(enemyGold)に成りで移動できる桂
			target = ZERO_BB;
