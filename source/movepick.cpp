...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh, const LowPlyHistory* lp,
	...
	stage = (pos.in_check() ? EVASION_TT : MAIN_TT) + !(ttm && pseudo_legal(pos, ttm));
...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh,
	...
	stage = (pos.in_check() ? EVASION_TT : QSEARCH_TT) +
	        !(ttm && (pos.in_check() || depth > DEPTH_QS_RECAPTURES || to_sq(ttm) == recaptureSquare) &&
...
MovePicker::MovePicker(const Position& p, Move ttm, Value th, const CapturePieceToHistory* cph)
	...
	// 置換表の指し手がないなら、次のstageから開始する。
	stage = PROBCUT_TT + !(ttm && pos.capture(ttm) && pseudo_legal(pos, ttm) && pos.see_ge(ttm, threshold));
}
...
void MovePicker::score() {
	...
	for (auto& m : *this) {
		if (Type == CAPTURES) {
			...
			m.value =
			    (*mainHistory)[from_to(m)][pos.side_to_move()] + 2 * (*continuationHistory[0])[movedSq][movedPiece] +
			    2 * (*continuationHistory[1])[movedSq][movedPiece] +
			    2 * (*continuationHistory[3])[movedSq][movedPiece] + (*continuationHistory[5])[movedSq][movedPiece] +
		...
		} else  // Type == EVASIONS
		{
			...
			if (pos.capture(m))
				...
				m.value = (*mainHistory)[from_to(m)][pos.side_to_move()] +
				          (*continuationHistory[0])[to_sq(m)][pos.moved_piece_after(m)] - (1 << 28);
		}
	}
}
...
Move MovePicker::select(Pred filter) {
	while (cur < endMoves) {
		...
		if (T == Best) std::swap(*cur, *std::max_element(cur, endMoves));

		// filter()のなかで*curにアクセスして判定するのでfilter()は引数を取らない。
		if (*cur != ttMove && filter()) return *cur++;
		cur++;
	}
	return MOVE_NONE;
}
...
top:
	switch (stage) {
...
#else
			endMoves = Search::Limits.generate_all_legal_moves ? generateMoves<CAPTURES_PRO_PLUS_ALL>(pos, cur)
			                                                   : generateMoves<CAPTURES_PRO_PLUS>(pos, cur);
#endif
		...
		case REFUTATION:
			...
			if (select<Next>([&]() {
				    return *cur != MOVE_NONE && !pos.capture_or_pawn_promotion(*cur) && pseudo_legal(pos, *cur);
			    }))
				return *(cur - 1);
...
#else
				endMoves = Search::Limits.generate_all_legal_moves ? generateMoves<NON_CAPTURES_PRO_MINUS_ALL>(pos, cur)
...
#else
			endMoves = Search::Limits.generate_all_legal_moves ? generateMoves<EVASIONS_ALL>(pos, cur)
		...
		case QCAPTURE_:
			...
			if (depth != DEPTH_QS_CHECKS) return MOVE_NONE;
		...
		// 王手となる指し手の生成
		case QCHECK_INIT:
			// この前のフェーズでCAPTURES_PRO_PLUSで生成していたので、駒を取らない王手の指し手生成(QUIET_CHECKS) -
			// 歩の成る指し手の除外 が必要。 (歩の成る指し手は王手であろうとすでに生成して試したあとである)
...
#else
			endMoves = Search::Limits.generate_all_legal_moves ? generateMoves<QUIET_CHECKS_ALL>(pos, cur)
			                                                   : generateMoves<QUIET_CHECKS>(pos, cur);
#endif
