...
#if defined(USE_SHARED_MEMORY_IN_EVAL) && defined(_WIN32) && (defined(EVAL_KPPT) || defined(EVAL_KPP_KKPT))
	// 評価関数パラメーターを共有するか。
	// デフォルトで有効に変更。(V4.90～)
	...
	for (size_t idx = 0; idx < om.size(); ++idx)
		for (const auto& it : om)
			if (it.second.idx == idx) {
				...
				if (o.type == "string" || o.type == "check" || o.type == "combo") os << " default " << o.defaultValue;

				if (o.type == "spin")
					// この範囲はStockfishではfloatになっているが、
					// やねうら王では、s64に変更する。
					os << " default " << s64(stoll(o.defaultValue)) << " min " << o.min << " max " << o.max;

				// コンボボックス(やねうら王、独自追加)
				// USIで規定されている。
				if (o.list.size())
					for (auto v : o.list) os << " var " << v;
...
EnteringKingRule to_entering_king_rule(const std::string& rule) {
	for (size_t i = 0; i < ekr_rules.size(); ++i)
		if (ekr_rules[i] == rule) return (EnteringKingRule)i;
...
void build_option(string line) {
	...
	while (!scanner.eol()) {
		...
		if (token == "name")
			name = scanner.get_text();
		else if (token == "type")
			option_type = scanner.get_text();
		else if (token == "default")
			value = scanner.get_text();
		else if (token == "min")
			min_value = stoll(scanner.get_text());
		else if (token == "max")
			max_value = stoll(scanner.get_text());
		else if (token == "var") {
	...
	if (Options.count(name) != 0) {
		// typeに応じたOptionの型を生成して代入する。このときに "<<"を用いるとidxが変わってしまうので
		// overwriteで代入する。
		if (option_type == "check")
			Options[name].overwrite(Option(value == "true"));
		else if (option_type == "spin")
			Options[name].overwrite(Option(stoll(value), min_value, max_value));
		else if (option_type == "string")
			Options[name].overwrite(Option(value.c_str()));
		else if (option_type == "combo")
			Options[name].overwrite(Option(combo_list, value));
	} else
...
void read_engine_options() {
	...
	if (!ifs.fail()) {
		std::string str;
		while (Tools::getline(ifs, str)) build_option(str);
	}
}
