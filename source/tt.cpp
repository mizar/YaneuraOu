...
void TTEntry::save(Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev) {
	...
	u16 pos_key = (u16)(k >> 1);
	if (m || pos_key != key16) move16 = (uint16_t)m;
...
TTEntry* TranspositionTable::read_probe(const Key key, bool& found) const {
...
#if defined(USE_GLOBAL_OPTIONS)
	if (!GlobalOptions.use_hash_probe) return found = false, first_entry(0);
#endif
	...
	for (int i = 0; i < ClusterSize; ++i) {
		if (tte[i].key16 == key16 || !tte[i].depth8) return found = (bool)tte[i].depth8, &tte[i];
	}
