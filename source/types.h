...
constexpr File file_of(Square sq) { /* return (File)(sq / 9); */ /*ASSERT_LV2(is_ok(sq));*/
...
// →　行数は長くなるが速度面においてテーブルを用いる。
constexpr Rank rank_of(Square sq) { /* return (Rank)(sq % 9); */ /*ASSERT_LV2(is_ok(sq));*/
	return SquareToRank[sq];
}

// 筋(File)と段(Rank)から、それに対応する升(Square)を返す。
constexpr Square operator|(File f, Rank r) {
	Square sq = (Square)(f * 9 + r); /* ASSERT_LV2(is_ok(sq));*/
	return sq;
}

// ２つの升のfileの差、rankの差のうち大きいほうの距離を返す。sq1,sq2のどちらかが盤外ならINT_MAXが返る。
constexpr int dist(Square sq1, Square sq2) {
	return (!is_ok(sq1) || !is_ok(sq2)) ? INT_MAX
	                                    : std::max(abs(file_of(sq1) - file_of(sq2)), abs(rank_of(sq1) - rank_of(sq2)));
}
...
enum SquareWithWall : int32_t {
	// 相対移動するときの差分値
	SQWW_R  = SQ_R - (1 << 9) + (1 << 24),
	SQWW_U  = SQ_U - (1 << 14) + (1 << 19),
	SQWW_D  = -int(SQWW_U),
	SQWW_L  = -int(SQWW_R),
	...
	SQWW_11 = SQ_11 | (1 << 8) /* bit8 is 1 */ | (0 << 9) /*右に0升*/ | (0 << 14) /*上に0升*/ | (8 << 19) /*下に8升*/ |
	          (8 << 24) /*左に8升*/,
...
enum Bound {
	...
	BOUND_UPPER,  // 上界(真の評価値はこれより小さい) =
	              // 詰みのスコアや、nonPVで評価値があまり信用ならない状態であることを表現する。
	BOUND_LOWER,  // 下界(真の評価値はこれより大きい)
	BOUND_EXACT =
	    BOUND_UPPER | BOUND_LOWER  // 真の評価値と一致している。PV nodeでかつ詰みのスコアでないことを表現する。
};
...
enum PieceType : uint32_t {
	...
	// こうすることで、成りを求めるときに pc |=
	// 8;で求まり、かつ、先手の全種類の駒を列挙するときに空きが発生しない。(DRAGONが終端になる)
	NO_PIECE_TYPE,
	PAWN /*歩*/,
	LANCE /*香*/,
	KNIGHT /*桂*/,
...
static std::string usi_piece(Piece pc) {
	return std::string((pc & 32) ? "D" : "") + std::string(USI_PIECE).substr((pc & 31) * 2, 2);
}
...
constexpr Piece make_piece(Color c, PieceType pt) { /*ASSERT_LV3(color_of(pt) == BLACK && pt!=NO_PIECE); */
	return (Piece)((c << 4) + pt);
}
...
enum PieceNumber : u8 {
	PIECE_NUMBER_PAWN   = 0,
	PIECE_NUMBER_LANCE  = 18,
	PIECE_NUMBER_KNIGHT = 22,
	PIECE_NUMBER_SILVER = 26,
	PIECE_NUMBER_GOLD   = 30,
	PIECE_NUMBER_BISHOP = 34,
	PIECE_NUMBER_ROOK   = 36,
	PIECE_NUMBER_KING   = 38,
	PIECE_NUMBER_BKING  = 38,
	PIECE_NUMBER_WKING  = 39,  // 先手、後手の玉の番号が必要な場合はこっちを用いる
	PIECE_NUMBER_ZERO   = 0,
	PIECE_NUMBER_NB     = 40,
};
...
enum Move : uint32_t {
	...
	MOVE_NULL =
	    (1 << 7) +
	    1,  // NULL MOVEを意味する指し手。Square(1)からSquare(1)への移動は存在しないのでここを特殊な記号として使う。
...
constexpr int from_to(Move m) {
	return (int)(from_sq(m) + (is_drop(m) ? (SQ_NB - 1) : 0)) * (int)SQ_NB + (int)to_sq(m);
}
...
// USI形式で指し手を表示する
static std::ostream& operator<<(std::ostream& os, Move16 m) {
	os << m.to_usi_string();
	return os;
}
...
constexpr bool operator<(const ExtMove& first, const ExtMove& second) { return first.value < second.value; }
...
// 手駒のbit位置
constexpr int PIECE_BITS[PIECE_HAND_NB] = {0,         0 /*歩*/,  8 /*香*/,  12 /*桂*/,
                                           16 /*銀*/, 20 /*角*/, 24 /*飛*/, 28 /*金*/};
...
constexpr int PIECE_BIT_MASK[PIECE_HAND_NB] = {0,        31 /*歩は5bit*/, 7 /*香は3bit*/, 7 /*桂*/,
                                               7 /*銀*/, 3 /*角*/,        3 /*飛*/,       7 /*金*/};

constexpr u32 PIECE_BIT_MASK2[PIECE_HAND_NB] = {0,
...
constexpr u32 HAND_BIT_MASK = PIECE_BIT_MASK2[PAWN] | PIECE_BIT_MASK2[LANCE] | PIECE_BIT_MASK2[KNIGHT] |
                              PIECE_BIT_MASK2[SILVER] | PIECE_BIT_MASK2[BISHOP] | PIECE_BIT_MASK2[ROOK] |
...
// 手駒pcを持っているかどうかを返す。
constexpr u32 hand_exists(Hand hand, PieceType pr) { /* ASSERT_LV2(PIECE_HAND_ZERO <= pr && pr < PIECE_HAND_NB); */
...
// 特定種類の駒を持っているかを判定する
constexpr bool hand_exists(HandKind hk, Piece pt) { /* ASSERT_LV2(PIECE_HAND_ZERO <= pt && pt < PIECE_HAND_NB); */
	return static_cast<bool>(hk & (1 << (pt - 1)));
}
...
enum MOVE_GEN_TYPE {
	...
	// 王手となる指し手(歩の不成などは含まない)で、CAPTURES_PRO_PLUSの指し手は含まない指し手 QUIET_CHECKS_PRO_MINUS_ALL,
...
template <MOVE_GEN_TYPE gen_type>
ExtMove* generateMoves(const Position& pos, ExtMove* mlist, Square recapSq);  // RECAPTURES,RECAPTURES_ALL専用
...
constexpr Key make_key(uint64_t seed) { return (seed * 6364136223846793005ULL + 1442695040888963407ULL) & ~1ULL; }
...
static Value draw_value(RepetitionState rs, Color c) { /* ASSERT_LV3(is_ok(rs)); */
