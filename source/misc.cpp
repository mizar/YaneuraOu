...
std::string GetFileName(const std::string& path) {
	...
	if (path_index1 == std::string::npos && path_index2 == std::string::npos) return path;
...
void init(int argc, char* argv[]) {
	...
	char* pgmptr = nullptr;
	if (!_get_pgmptr(&pgmptr) && pgmptr != nullptr && *pgmptr) argv0 = pgmptr;
#endif
#else
	pathSeparator = "/";
#endif
	...
	if (binaryDirectory.find("." + pathSeparator) == 0) binaryDirectory.replace(0, 1, workingDirectory);
}
