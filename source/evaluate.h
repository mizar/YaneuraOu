...
// BonaPiece、f側だけを表示する。
inline std::ostream& operator<<(std::ostream& os, ExtBonaPiece bp) {
	os << bp.fb;
	return os;
}
...
struct EvalList {
	...
	void put_piece(PieceNumber piece_no, Square sq, Piece pc) {
		set_piece_on_board(piece_no, BonaPiece(kpp_board_index[pc].fb + sq),
		                   BonaPiece(kpp_board_index[pc].fw + Inv(sq)), sq);
	}
	...
	void clear() {
		...
		for (auto& p : pieceListFb) p = BONA_PIECE_ZERO;

		for (auto& p : pieceListFw) p = BONA_PIECE_ZERO;

		for (auto& v : piece_no_list_hand) v = PIECE_NUMBER_NB;

		for (auto& v : piece_no_list_board) v = PIECE_NUMBER_NB;
#endif
	}
