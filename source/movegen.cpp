...
struct GenerateDropMoves {
	ExtMove* operator()(const Position& pos, ExtMove* mlist, const Bitboard& target) {
		...
		if (hand_except_pawn_exists(hand)) {
			...
			int num = 0;
			if (hand_exists(hand, KNIGHT))
				drops[num++] = make_move_drop(KNIGHT, SQ_ZERO) + OurDropPt(Us, KNIGHT);

			int nextToKnight = num;  // 桂を除いたdropsのindex
			if (hand_exists(hand, LANCE))
				drops[num++] = make_move_drop(LANCE, SQ_ZERO) + OurDropPt(Us, LANCE);
