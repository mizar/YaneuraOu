...
#define MAKE_MOVE_TARGET(target_)                                          \
	{                                                                      \
		FOREACH_BB(target_, to, { mlist++->move = make_move(from, to); }); \
	}
...
#define MAKE_MOVE_TARGET_PRO_ONLY(target_)                                         \
	{                                                                              \
		FOREACH_BB(target_, to, { mlist++->move = make_move_promote(from, to); }); \
	}
...
#define MAKE_MOVE_TARGET_PRO_BR(target_)                  \
	{                                                     \
		FOREACH_BB(target_, to, {                         \
			mlist++->move = make_move_promote(from, to);  \
			if (All) mlist++->move = make_move(from, to); \
		});                                               \
	}
...
#define MAKE_MOVE_TARGET(target_)                                                          \
	{                                                                                      \
		FOREACH_BB(target_, to, { mlist++->move = make_move(from, to) + OurPt(Us, Pt); }); \
	}

// 移動させる駒が不明な場合
#define MAKE_MOVE_TARGET_UNKNOWN(target_)                                                                   \
	{                                                                                                       \
		FOREACH_BB(target_, to, { mlist++->move = make_move(from, to) + Move(pos.piece_on(from) << 16); }); \
	}
...
#define MAKE_MOVE_TARGET_PRO_ONLY(target_)                                                            \
	{                                                                                                 \
		FOREACH_BB(target_, to, { mlist++->move = make_move_promote(from, to) + OurProPt(Us, Pt); }); \
	}
...
#define MAKE_MOVE_TARGET_DROP(target_, Pt)                                                        \
	{                                                                                             \
		FOREACH_BB(target_, sq, { mlist++->move = make_move_drop(Pt, sq) + OurDropPt(Us, Pt); }); \
	}
...
template <PieceType Pt, Color Us, bool All>
	...
	FORCE_INLINE ExtMove* operator()(const Position& pos, Square from, const Bitboard& target_, ExtMove* mlist) {
		...
		switch (Pt) {
			...
			case KNIGHT: {
				while (target) {
					to = target.pop();
					if (canPromote(Us, to)) mlist++->move = make_move_promote(from, to) + OurProPt(Us, Pt);
...
template <MOVE_GEN_TYPE GenType, PieceType Pt, Color Us, bool All>
struct GeneratePieceMoves {
	FORCE_INLINE ExtMove* operator()(const Position& pos, ExtMove* mlist, const Bitboard& target) {
		...
		while (pieces) {
			...
			// 移動できる場所 = 利きのある場所
			auto target2 = Pt == LANCE    ? lanceEffect(Us, from, occ)
	...
	FORCE_INLINE ExtMove* operator()(const Position& pos, ExtMove* mlist, const Bitboard& target) {
		...
		while (target2) {
			...
			if (canPromote(Us, to)) {
				...
				if (All && rank_of(to) != T_RANK1) mlist++->move = make_move(from, to) + OurPt(Us, PAWN);
			} else
...
struct GenerateDropMoves {
	ExtMove* operator()(const Position& pos, ExtMove* mlist, const Bitboard& target) {
		...
		if (hand_exists(hand, PAWN)) {
			...
			Square   pawnSq;
			foreachBB(pawnBB, pawnSq, [&](const int part) { target2.p[part] &= PAWN_DROP_MASKS[pawnSq]; });
		...
		if (hand_except_pawn_exists(hand)) {
			...
			if (nextToLance == 0) {
				...
				switch (num) {
					case 1:
						FOREACH_BB(target2, sq, { UNROLLER1({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 2:
						FOREACH_BB(target2, sq, { UNROLLER2({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 3:
						FOREACH_BB(target2, sq, { UNROLLER3({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 4:
						FOREACH_BB(target2, sq, { UNROLLER4({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					default:
						UNREACHABLE;
				}
				...
				{
					case 0:
						break;  // 香・桂以外の持ち駒がないケース
					case 1:
						FOREACH_BB(target1, sq,
						           { UNROLLER1({ mlist++->move = (Move)(drops[i + nextToLance] + sq); }); });
						break;
					case 2:
						FOREACH_BB(target1, sq,
						           { UNROLLER2({ mlist++->move = (Move)(drops[i + nextToLance] + sq); }); });
						break;
					case 3:
						FOREACH_BB(target1, sq,
						           { UNROLLER3({ mlist++->move = (Move)(drops[i + nextToLance] + sq); }); });
						break;
					case 4:
						FOREACH_BB(target1, sq,
						           { UNROLLER4({ mlist++->move = (Move)(drops[i + nextToLance] + sq); }); });
						break;
					default:
						UNREACHABLE;
				}
				...
				switch (num - nextToKnight)  // 2段目に対する桂以外の駒打ちの指し手の生成(最大で5種の駒)
				{
					...
					case 1:
						FOREACH_BB(target2, sq,
						           { UNROLLER1({ mlist++->move = (Move)(drops[i + nextToKnight] + sq); }); });
						break;
					case 2:
						FOREACH_BB(target2, sq,
						           { UNROLLER2({ mlist++->move = (Move)(drops[i + nextToKnight] + sq); }); });
						break;
					case 3:
						FOREACH_BB(target2, sq,
						           { UNROLLER3({ mlist++->move = (Move)(drops[i + nextToKnight] + sq); }); });
						break;
					case 4:
						FOREACH_BB(target2, sq,
						           { UNROLLER4({ mlist++->move = (Move)(drops[i + nextToKnight] + sq); }); });
						break;
					case 5:
						FOREACH_BB(target2, sq,
						           { UNROLLER5({ mlist++->move = (Move)(drops[i + nextToKnight] + sq); }); });
						break;
					default:
						UNREACHABLE;
				}
				...
				{
					case 1:
						FOREACH_BB(target3, sq, { UNROLLER1({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 2:
						FOREACH_BB(target3, sq, { UNROLLER2({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 3:
						FOREACH_BB(target3, sq, { UNROLLER3({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 4:
						FOREACH_BB(target3, sq, { UNROLLER4({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 5:
						FOREACH_BB(target3, sq, { UNROLLER5({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					case 6:
						FOREACH_BB(target3, sq, { UNROLLER6({ mlist++->move = (Move)(drops[i] + sq); }); });
						break;
					default:
						UNREACHABLE;
				}
...
ExtMove* generate_general(const Position& pos, ExtMove* mlist, Square recapSq = SQ_NB) {
	...
	static_assert(GenType != EVASIONS_ALL && GenType != NON_EVASIONS_ALL && GenType != RECAPTURES_ALL,
	              "*_ALL is not allowed.");
	...
	const Bitboard target =
	    (GenType == NON_CAPTURES) ? pos.empties() :  // 捕獲しない指し手 = 移動先の升は駒のない升
	        ...
	        ? pos.empties()
	        :  // 捕獲しない指し手 - 歩の成る指し手 = 移動先の升は駒のない升 - 敵陣(歩のときのみ)
	        (GenType == CAPTURES_PRO_PLUS) ? pos.pieces(~Us)
	                                       :  // 捕獲 + 歩の成る指し手 = 移動先の升は敵駒のある升 + 敵陣(歩のときのみ)
	        (GenType == NON_EVASIONS) ? ~pos.pieces(Us)
	                                  :  // すべて = 移動先の升は自駒のない升
	        (GenType == RECAPTURES) ? Bitboard(recapSq)
	                                :  // リキャプチャー用の升(直前で相手の駒が移動したわけだからここには移動できるはず)
	        ALL_BB;  // error

	// 歩の移動先(↑のtargetと違う部分のみをオーバーライド)
	const Bitboard targetPawn =
	    (GenType == NON_CAPTURES_PRO_MINUS) ? (pos.empties() & ~enemy_field(Us))
	                                        :  // 駒を取らない指し手 かつ、歩の成る指し手を引いたもの
	        (GenType == CAPTURES_PRO_PLUS) ? (pos.pieces(~Us) | (~pos.pieces(Us) & enemy_field(Us)))
	                                       :  // 歩の場合は敵陣での成りもこれに含める
	        target;
...
struct make_move_target_general {
	ExtMove* operator()(const Position& pos, Piece pc, Square from, const Bitboard& target, ExtMove* mlist) {
		...
		switch (type_of(pc)) {
			case PAWN:
				mlist = make_move_target<PAWN, Us, All>()(pos, from, effect & target, mlist);
				break;
			case LANCE:
				mlist = make_move_target<LANCE, Us, All>()(pos, from, effect & target, mlist);
				break;
			case KNIGHT:
				mlist = make_move_target<KNIGHT, Us, All>()(pos, from, effect & target, mlist);
				break;
			case SILVER:
				mlist = make_move_target<SILVER, Us, All>()(pos, from, effect & target, mlist);
				break;
			case GOLD:
				mlist = make_move_target<GOLD, Us, All>()(pos, from, effect & target, mlist);
				break;
			case BISHOP:
				mlist = make_move_target<BISHOP, Us, All>()(pos, from, effect & target, mlist);
				break;
			case ROOK:
				mlist = make_move_target<ROOK, Us, All>()(pos, from, effect & target, mlist);
				break;
			case KING:
				mlist = make_move_target<KING, Us, All>()(pos, from, effect & target, mlist);
				break;
			case PRO_PAWN:
				mlist = make_move_target<PRO_PAWN, Us, All>()(pos, from, effect & target, mlist);
				break;
			case PRO_LANCE:
				mlist = make_move_target<PRO_LANCE, Us, All>()(pos, from, effect & target, mlist);
				break;
			case PRO_KNIGHT:
				mlist = make_move_target<PRO_KNIGHT, Us, All>()(pos, from, effect & target, mlist);
				break;
			case PRO_SILVER:
				mlist = make_move_target<PRO_SILVER, Us, All>()(pos, from, effect & target, mlist);
				break;
			case HORSE:
				mlist = make_move_target<HORSE, Us, All>()(pos, from, effect & target, mlist);
				break;
			case DRAGON:
				mlist = make_move_target<DRAGON, Us, All>()(pos, from, effect & target, mlist);
				break;
			default:
				UNREACHABLE;
		}
...
ExtMove* make_move_target_pro(Square from, const Bitboard& target, ExtMove* mlist) {
	...
	while (bb) {
		...
		if (Promote)
			...
			if (((Pt == PAWN) &&
			     ((!All && !canPromote(Us, to)) || (All && rank_of(to) != (Us == BLACK ? RANK_1 : RANK_9)))) ||
			    ((Pt == LANCE) &&
...
template <Color Us, bool All>
ExtMove* make_move_check(const Position& pos, Piece pc, Square from, Square ksq, const Bitboard& target,
                         ExtMove* mlist) {
...
#define GEN_MOVE_HD_CHECK(X, X_Effect)                                 \
	...
	switch (type_of(pc)) {
		...
		case PAWN:
			...
			break;
		case LANCE:
			GEN_MOVE_LANCE_CHECK(LANCE, lanceEffect, goldEffect);
			break;
		case KNIGHT:
			GEN_MOVE_NONPRO_CHECK(KNIGHT, knightEffect, goldEffect);
			break;
		case SILVER:
		...
		case PRO_PAWN:
			...
			break;
		case PRO_LANCE:
			GEN_MOVE_GOLD_CHECK(PRO_LANCE, goldEffect);
			break;
		case PRO_KNIGHT:
			GEN_MOVE_GOLD_CHECK(PRO_KNIGHT, goldEffect);
			break;
		case PRO_SILVER:
...
struct GenerateCheckDropMoves<Us, PAWN> {
	ExtMove* operator()(const Position& pos, const Bitboard& target, ExtMove* mlist) {
		...
		if (bb)  // 歩を打って王手になる箇所は1箇所しかないのでwhileである必要はない。
		{
			...
			if (pos.legal_pawn_drop(Us, to)) mlist++->move = make_move_drop(PAWN, to) + OurDropPt(Us, PAWN);
		}
		return mlist;
	}
};
...
ExtMove* generate_checks(const Position& pos, ExtMove* mlist) {
	...
	const Bitboard x = ((pos.pieces(PAWN) & check_candidate_bb(Us, PAWN, themKing)) |
	                    ...
	                    (pos.pieces(HORSE) &
	                     check_candidate_bb(Us, ROOK, themKing))  // check_candidate_bbにはROOKと書いてるけど、HORSE
	                    ) &
	                   pos.pieces(Us);
	...
	const Bitboard target = (GenType == CHECKS || GenType == CHECKS_ALL) ? ~pos.pieces(Us)
	                                                                     :  // 自駒がない場所が移動対象升
	...
	if (hand_exists(h, PAWN)) mlist = GenerateCheckDropMoves<Us, PAWN>()(pos, pos.check_squares(PAWN) & empties, mlist);
	...
	if (hand_exists(h, GOLD)) mlist = GenerateCheckDropMoves<Us, GOLD>()(pos, pos.check_squares(GOLD) & empties, mlist);
	if (hand_exists(h, BISHOP))
		mlist = GenerateCheckDropMoves<Us, BISHOP>()(pos, pos.check_squares(BISHOP) & empties, mlist);
	if (hand_exists(h, ROOK)) mlist = GenerateCheckDropMoves<Us, ROOK>()(pos, pos.check_squares(ROOK) & empties, mlist);
...
ExtMove* generateMoves(const Position& pos, ExtMove* mlist, Square sq = SQ_NB) {
	return pos.side_to_move() == BLACK ? generate_general<GenType, BLACK, All>(pos, mlist, sq)
...
ExtMove* generateEvasionMoves(const Position& pos, ExtMove* mlist) {
	return pos.side_to_move() == BLACK ? generate_evasions<BLACK, All>(pos, mlist)
...
ExtMove* generateChecksMoves(const Position& pos, ExtMove* mlist) {
	return pos.side_to_move() == BLACK ? generate_checks<GenType, BLACK, All>(pos, mlist)
...
ExtMove* generateMoves(const Position& pos, ExtMove* mlist, Square recapSq) {
	...
	if (GenType == LEGAL || GenType == LEGAL_ALL) {
		...
		auto last =
		    pos.in_check() ? generateEvasionMoves<All>(pos, mlist) : generateMoves<NON_EVASIONS, All>(pos, mlist);
	...
	// 回避手
	if (GenType == EVASIONS || GenType == EVASIONS_ALL) return generateEvasionMoves<All>(pos, mlist);
