...
struct EvalInfo {
	...
	static EvalInfo build_kpp(T1 kk_, T2 kkp_, T3 kpp_) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KK, 4, 1, FileOrMemory(kk_)));  // KK は4バイト。(手番なしなので1つ)
	...
	static EvalInfo build_kppt32(T1 kk_, T2 kkp_, T3 kpp_) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KK, 4, 2, FileOrMemory(kk_)));  // KK は4バイト。(手番ありなので2つ)
	...
	static EvalInfo build_kppt16(T1 kk_, T2 kkp_, T3 kpp_) {
		EvalInfo ei(81 /* SQ_NB */, 1548 /* EvalKPPT::fe_end */);
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KK, 2, 2, FileOrMemory(kk_)));  // KK は2バイト。(手番ありなので2つ)
	...
	static EvalInfo build_kpp_kkpt32(T1 kk_, T2 kkp_, T3 kpp_) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KK, 4, 2, FileOrMemory(kk_)));  // KK は4バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KKP, 4, 2, FileOrMemory(kkp_)));  // KKPは4バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KPP, 2, 1, FileOrMemory(kpp_)));  // KPPは2バイト。(手番なしなので1つ)
		return ei;
	}
	...
	static EvalInfo build_kppp_kkpt32(T1 kk_, T2 kkp_, T3 kpp_, T4 kppp_, u64 size_of_kppp) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KK, 4, 2, FileOrMemory(kk_)));  // KK  は4バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KKP, 4, 2, FileOrMemory(kkp_)));  // KKP は4バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KPP, 2, 1, FileOrMemory(kpp_)));  // KPP は2バイト。(手番なしなので1つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(VAR, 2, size_of_kppp / 2, FileOrMemory(kppp_)));  // KPPPは2バイト。(手番なしなので1つ)
		return ei;
	}
	...
	static EvalInfo build_kpppt32(T1 kk_, T2 kkp_, T3 kpp_, T4 kppp_, u64 size_of_kppp) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KKP, 4, 2, FileOrMemory(kkp_)));  // KKP は4バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KPP, 2, 2, FileOrMemory(kpp_)));  // KPP は2バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(VAR, 2, size_of_kppp / 2, FileOrMemory(kppp_)));  // KPPPは2バイト。(手番ありなので2つ)
		return ei;
	}
	...
	static EvalInfo build_kkpp_kkpt32(T1 kk_, T2 kkp_, T3 kpp_, T4 kkpp_, u64 size_of_kkpp) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KPP, 2, 1, FileOrMemory(kpp_)));  // KPP は2バイト。(手番なしなので1つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(VAR, 2, size_of_kkpp / 2, FileOrMemory(kkpp_)));  // KKPPは2バイト。(手番なしなので1つ)
		return ei;
	}
	...
	static EvalInfo build_kkppt32(T1 kk_, T2 kkp_, T3 kpp_, T4 kkpp_, u64 size_of_kkpp) {
		...
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(KPP, 2, 2, FileOrMemory(kpp_)));  // KPP は2バイト。(手番ありなので2つ)
		ei.eval_info_array.emplace_back(
		    EvalArrayInfo(VAR, 2, size_of_kkpp / 2, FileOrMemory(kkpp_)));  // KKPPは2バイト。(手番ありなので2つ)
		return ei;
	}
