...
bool eval_convert(const EvalInfo& input, const EvalInfo& output, const std::vector<u16>* map) {
	...
	for (int i = 0; i < size; ++i) {
		...
		auto calc_block_size = [](u64 element_num, u64 element_size, EvalFeature feature, u64 sq_nb, u64 fe_end) {
			...
			switch (feature) {
				case KK:
					block_size *= (sq_nb) * (sq_nb);
					break;
				case KKP:
					block_size *= (sq_nb) * (sq_nb) * (fe_end);
					break;
				case KPP:
					block_size *= (sq_nb) * (fe_end) * (fe_end);
					break;
				case PP:
					block_size *= (fe_end) * (fe_end);
					break;
				case KKPP:
					block_size *= (sq_nb) * (sq_nb) * (fe_end) * (fe_end);
					break;
				case KPPP:
					block_size *= (sq_nb) * (fe_end) * (fe_end) * (fe_end);
					break;
				case VAR:
					block_size *= 1;
					break;
		...
		auto input_block_size =
		    calc_block_size(in_.element_num, in_.element_size, in_.feature, input.sq_nb, input.fe_end);
		auto output_block_size =
		    calc_block_size(out_.element_num, out_.element_size, out_.feature, output.sq_nb, output.fe_end);

		// 型変換の必要があるかどうかの判定
		// inとoutの型が全く同じでかつ変換mapが指定されていないのであれば、
		// 単にメモリコピー、ファイル入出力などで済む。このとき、オーバーヘッドは最も小さくて済む。
		bool convertless = in_.element_size == out_.element_size && in_.element_num == out_.element_num &&
		                   input.sq_nb == output.sq_nb && input.fe_end == output.fe_end && map == nullptr;
		...
		if (convertless) {
			...
			// file to memory
				...
				if (FileOperator::ReadFileToMemory(in_.file_or_memory.filename, [&](u64 size) {
					    if (size != input_block_size) {
						    std::cout << "info string Error! : file size incorrect , file = "
						              << in_.file_or_memory.filename << " , actual size = " << size
						              << " , needed_size = " << input_block_size << std::endl;
						    return (void*)nullptr;
					    }
					...
					if (Options["SkipLoadingEval"]) {
						std::cout << "info string read file error , file = " << in_.file_or_memory.filename
						          << " , but SkipLoadingEval == true , so ignore this." << std::endl;
						memset(out_.file_or_memory.ptr, 0, input_block_size);
					} else
#endif
					{
						std::cout << "info string Error! : read file error , file = " << in_.file_or_memory.filename
						          << std::endl;
						return false;
					}
				}
			...
			// memory to file
				...
				if (FileOperator::WriteMemoryToFile(out_.file_or_memory.filename, in_.file_or_memory.ptr,
				                                    output_block_size)
				        .is_not_ok()) {
			...
			// file to file
				...
				if (ifs)
					ifs.read(reinterpret_cast<char*>(&buffer[0]), input_block_size);
				else {
				...
				if (ofs)
					ofs.write(reinterpret_cast<char*>(&buffer[0]), output_block_size);
				else {
					std::cout << "info string write file error , file = " << out_.file_or_memory.filename << std::endl;
					return false;
				}
			...
			if (in_ptr == nullptr) {
				...
				if (FileOperator::ReadFileToMemory(in_.file_or_memory.filename, [&](u64 file_size) {
					    if (file_size != input_block_size) {
						    std::cout << "info string Error! file_size = " << file_size
						              << " , input_block_size = " << input_block_size << std::endl;
						    return (void*)nullptr;
					    }
					    return in_ptr;
				    }).is_not_ok())
				...
				case KK:
					for (u64 k1 = 0; k1 < output.sq_nb; ++k1)
						for (u64 k2 = 0; k2 < output.sq_nb; ++k2) {
							...
							conv((u8*)in_ptr + input_index * input_feature_size,
							     (u8*)out_ptr + output_index * output_feature_size);
						}
					break;
					...
					for (u64 k1 = 0; k1 < output.sq_nb; ++k1)
						for (u64 k2 = 0; k2 < output.sq_nb; ++k2)
							for (u64 p1 = 0; p1 < output.fe_end; ++p1) {
								...
								if (input_p1 >= input.fe_end) input_p1 = 0;
					...
					for (u64 k1 = 0; k1 < output.sq_nb; ++k1)
						for (u64 p1 = 0; p1 < output.fe_end; ++p1) {
							...
							for (u64 p2 = 0; p2 < output.fe_end; ++p2) {
								...
								if (input_p2 >= input.fe_end) input_p2 = 0;
						...
						for (u64 k2 = 0; k2 < output.sq_nb; ++k2)
							for (u64 p1 = 0; p1 < output.fe_end; ++p1) {
								...
								for (u64 p2 = 0; p2 < output.fe_end; ++p2) {
									...
									if (input_p2 >= input.fe_end) input_p2 = 0;
									u64 input_index =
									    ((k1 * input.sq_nb + k2) * input.fe_end + (input_p1)) * input.fe_end + input_p2;
									u64 output_index =
									    ((k1 * output.sq_nb + k2) * output.fe_end + (p1)) * output.fe_end + p2;
									conv((u8*)in_ptr + input_index * input_feature_size,
									     (u8*)out_ptr + output_index * output_feature_size);
								}
							}
						...
						for (u64 p1 = 0; p1 < output.fe_end; ++p1) {
							...
							for (u64 p2 = 0; p2 < output.fe_end; ++p2) {
								...
								for (u64 p3 = 0; p3 < output.fe_end; ++p3) {
									...
									if (input_p3 >= input.fe_end) input_p3 = 0;
									u64 input_index =
									    (((k1)*input.fe_end + (input_p1)) * input.fe_end + input_p2) * input.fe_end +
									    input_p3;
									u64 output_index =
									    (((k1)*output.fe_end + (p1)) * output.fe_end + p2) * output.fe_end + p3;
									conv((u8*)in_ptr + input_index * input_feature_size,
									     (u8*)out_ptr + output_index * output_feature_size);
								}
							}
			...
			if (out_.file_or_memory.ptr == nullptr) {
				if (FileOperator::WriteMemoryToFile(out_.file_or_memory.filename, out_ptr, output_block_size)
				        .is_not_ok()) {
					std::cout << "info string write file error , file = " << out_.file_or_memory.filename << std::endl;
					return false;
				}
