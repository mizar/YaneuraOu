...
Value compute_eval(const Position& pos) {
	...
	for (auto sq : SQ) {
		...
		for (auto color : COLOR) {
			...
			// scoreは先手から見たスコアなので、colorが先手の時は、(s1-s2) をscoreに加算。colorが後手の時は、(s2-s1)
			// を加算。
...
void init() {
	...
	// 利きは最大で10個のことがある。
	for (int m = 0; m < 11; ++m) multi_effect_value[m] = m == 0 ? 0 : int(6365 - std::pow(0.8525, m - 1) * 5341);
}
...
Value compute_eval(const Position& pos) {
	...
	for (auto sq : SQ) {
		// 盤上の升の利きの価値
		score += effect_table[pos.king_square(BLACK)][pos.king_square(WHITE)][sq][pos.board_effect[BLACK].effect(sq)]
		                     [pos.board_effect[WHITE].effect(sq)];
