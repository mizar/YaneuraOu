...
void compute_eval_impl(const Position& pos) {
	...
	for (i = 0; i < length; ++i) {
		...
		for (j = 0; j < i; ++j) {
			...
			__m128i tmp;
			tmp = _mm_set_epi32(0, 0, *reinterpret_cast<const int32_t*>(&pkppw[l1][0]),
			                    *reinterpret_cast<const int32_t*>(&pkppb[l0][0]));
			// この命令SSE4.1の命令のはず..
...
std::array<s32, 2> do_a_black(const Position& pos, const ExtBonaPiece ebp) {
	...
	for (int i = 1; i < length; ++i) sum += pkppb[list0[i]];
	return sum;
}
...
std::array<s32, 2> do_a_white(const Position& pos, const ExtBonaPiece ebp) {
	...
	for (int i = 1; i < length; ++i) sum += pkppw[list1[i]];
	return sum;
}
