...
Value material(const Position& pos) {
	...
	for (auto i : SQ) v = v + PieceValue[pos.piece_on(i)];
...
void print_softname(u64 check_sum) {
	...
	if (list.count(check_sum)) softname = list[check_sum];

	sync_cout << "info string Eval Check Sum = " << std::hex << check_sum << std::dec << " , Eval File = " << softname
	          << sync_endl;
}
#endif
...
bool EvalList::is_valid(const Position& pos) {
	...
	for (int i = 0; i < length(); ++i) {
		...
		// 範囲外
		if (!(0 <= fb && fb < fe_end)) return false;
			...
			for (auto c : COLOR)
				for (PieceType pr = PAWN; pr < KING; ++pr) {
					...
					if (s <= fb && fb < s + hand_max[pr]) {
						...
						int n = (int)(fb - s) + 1;
						if (hand_count(pos.hand_of(c), pr) < n) return false;
