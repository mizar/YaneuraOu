...
// 学習率のスケールを取得する
double GetGlobalLearningRateScale() { return global_learning_rate_scale; }
...
void InitializeTraining(double eta1, u64 eta1_epoch, double eta2, u64 eta2_epoch, double eta3) {
	std::cout << "Initializing NN training for " << GetArchitectureString() << std::endl;
...
// 学習率のスケールを設定する
void SetGlobalLearningRateScale(double scale) { global_learning_rate_scale = scale; }
...
void AddExample(Position& pos, Color rootColor, const Learner::PackedSfenValue& psv, double weight) {
	...
	for (const auto color : COLOR) {
		...
		for (const auto base_index : active_indices[color]) {
			static_assert(Features::Factorizer<RawFeatures>::GetDimensions() < (1 << TrainingFeature::kIndexBits), "");
			Features::Factorizer<RawFeatures>::AppendTrainingFeatures(base_index, &training_features);
		}
		std::sort(training_features.begin(), training_features.end());

		auto& unique_features = example.training_features[color];
		for (const auto& feature : training_features) {
			if (!unique_features.empty() && feature.GetIndex() == unique_features.back().GetIndex()) {
...
void UpdateParameters(u64 epoch) {
	...
	const auto learning_rate = static_cast<LearnFloatType>(get_eta() / batch_size);
	...
	while (examples.size() >= batch_size) {
		...
		for (std::size_t b = 0; b < batch.size(); ++b) {
			const auto shallow =
			    static_cast<Value>(Round<std::int32_t>(batch[b].sign * network_output[b] * kPonanzaConstant));
...
// 学習に問題が生じていないかチェックする
void CheckHealth() { SendMessages({{"check_health"}}); }
...
// 現在のetaを取得する
double get_eta() { return NNUE::GetGlobalLearningRateScale() * EvalLearningTools::Weight::eta; }
