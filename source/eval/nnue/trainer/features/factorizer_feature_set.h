...
class Factorizer<FeatureSet<FirstFeatureType, RemainingFeatureTypes...>> {
   ...
   public:
	...
	// 学習用特徴量の次元数を取得する
	static constexpr IndexType GetDimensions() { return Head::GetDimensions() + Tail::GetDimensions(); }
			...
			for (auto i = start; i < training_features->size(); ++i) {
				...
				ASSERT_LV5(index < Head::GetDimensions() ||
				           (index >= base_dimensions &&
				            index < base_dimensions + Head::GetDimensions() - Head::kBaseDimensions));
...
class Factorizer<FeatureSet<FeatureType>> {
   public:
	...
	// 学習用特徴量の次元数を取得する
	static constexpr IndexType GetDimensions() { return Factorizer<FeatureType>::GetDimensions(); }
