...
#pragma omp parallel for
		for (IndexType b = 0; b < batch.size(); ++b) {
			...
			for (IndexType c = 0; c < 2; ++c) {
				...
				for (const auto& feature : batch[b].training_features[c]) {
					...
					cblas_saxpy(kHalfDimensions, (float)feature.GetCount(), &weights_[weights_offset], 1,
				...
				for (const auto& feature : batch[b].training_features[c]) {
					const IndexType weights_offset = kHalfDimensions * feature.GetIndex();
					for (IndexType i = 0; i < kHalfDimensions; ++i) {
						output_[output_offset + i] += feature.GetCount() * weights_[weights_offset + i];
					}
				}
#endif
			}
	...
	void Backpropagate(const LearnFloatType* gradients, LearnFloatType learning_rate) {
		...
		for (IndexType b = 0; b < batch_->size(); ++b) {
			...
			for (IndexType i = 0; i < kOutputDimensions; ++i) {
				const IndexType index = batch_offset + i;
				gradients_[index]     = gradients[index] * ((output_[index] > kZero) * (output_[index] < kOne));
			}
		}
		...
		for (IndexType b = 0; b < batch_->size(); ++b) {
			...
			for (IndexType c = 0; c < 2; ++c) {
				const IndexType output_offset = batch_offset + kHalfDimensions * c;
				cblas_saxpy(kHalfDimensions, 1.0, &gradients_[output_offset], 1, biases_diff_, 1);
			}
		}
		cblas_saxpy(kHalfDimensions, -local_learning_rate, biases_diff_, 1, biases_, 1);
#pragma omp parallel
		{
#if defined(_OPENMP)
...
#endif
						const IndexType weights_offset = kHalfDimensions * feature.GetIndex();
						const auto scale = static_cast<LearnFloatType>(effective_learning_rate / feature.GetCount());
						cblas_saxpy(kHalfDimensions, -scale, &gradients_[output_offset], 1, &weights_[weights_offset],
						            1);
					}
				}
			}
		...
		for (IndexType b = 0; b < batch_->size(); ++b) {
			...
			for (IndexType c = 0; c < 2; ++c) {
				...
				for (const auto& feature : (*batch_)[b].training_features[c]) {
					...
					const auto scale = static_cast<LearnFloatType>(effective_learning_rate / feature.GetCount());
					for (IndexType i = 0; i < kHalfDimensions; ++i) {
						weights_[weights_offset + i] -= scale * gradients_[output_offset + i];
					}
				}
			}
		}
   ...
   private:
	...
	Trainer(LayerType* target_layer)
		...
		std::fill(std::begin(min_activations_), std::end(min_activations_), std::numeric_limits<LearnFloatType>::max());
...
#pragma omp parallel for private(training_features)
		for (IndexType j = 0; j < RawFeatures::kDimensions; ++j) {
			...
			Features::Factorizer<RawFeatures>::AppendTrainingFeatures(j, &training_features);
			for (IndexType i = 0; i < kHalfDimensions; ++i) {
				double sum = 0.0;
	...
	void DequantizeParameters() {
		...
		std::fill(std::begin(weights_), std::end(weights_), +kZero);
		for (IndexType i = 0; i < kHalfDimensions * RawFeatures::kDimensions; ++i) {
			weights_[i] = static_cast<LearnFloatType>(target_layer_->weights_[i] / kWeightScale);
		}
	...
	void ClearUnobservedFeatureWeights() {
		for (IndexType i = 0; i < kInputDimensions; ++i) {
			if (!observed_features.test(i)) {
				std::fill(std::begin(weights_) + kHalfDimensions * i, std::begin(weights_) + kHalfDimensions * (i + 1),
				          +kZero);
			}
		}
		QuantizeParameters();
	...
	// 学習に問題が生じていないかチェックする
	void CheckHealth() {
		std::cout << "INFO: observed " << observed_features.count() << " (out of " << kInputDimensions << ") features"
		          << std::endl;

		constexpr LearnFloatType kPreActivationLimit =
		    std::numeric_limits<typename LayerType::WeightType>::max() / kWeightScale;
		std::cout << "INFO: (min, max) of pre-activations = " << min_pre_activation_ << ", " << max_pre_activation_
		          << " (limit = " << kPreActivationLimit << ")" << std::endl;

		const auto largest_min_activation = *std::max_element(std::begin(min_activations_), std::end(min_activations_));
		const auto smallest_max_activation =
		    *std::min_element(std::begin(max_activations_), std::end(max_activations_));
		std::cout << "INFO: largest min activation = " << largest_min_activation
		          << ", smallest max activation = " << smallest_max_activation << std::endl;
	...
	// 入出力の次元数
	static constexpr IndexType kInputDimensions  = Features::Factorizer<RawFeatures>::GetDimensions();
	...
	alignas(kCacheLineSize) LearnFloatType weights_[kHalfDimensions * kInputDimensions];
