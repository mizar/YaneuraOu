...
class SharedInputTrainer {
   public:
	// ファクトリ関数
	static std::shared_ptr<SharedInputTrainer> Create(FeatureTransformer* feature_transformer) {
	...
	void Backpropagate(const LearnFloatType* gradients, LearnFloatType learning_rate) {
		...
		if (++num_calls_ == num_referrers_) {
			feature_transformer_trainer_->Backpropagate(gradients_.data(), learning_rate);
			num_calls_         = 0;
			current_operation_ = Operation::kNone;
		}
	}
   ...
   private:
	...
	SharedInputTrainer(FeatureTransformer* feature_transformer)
	    : batch_size_(0),
	      ...
	      num_calls_(0),
	      current_operation_(Operation::kNone),
	      feature_transformer_trainer_(Trainer<FeatureTransformer>::Create(feature_transformer)),
	      output_(nullptr) {}

	// 入出力の次元数
	static constexpr IndexType kInputDimensions = FeatureTransformer::kOutputDimensions;
	...
	// 入力特徴量変換器のTrainer
	const std::shared_ptr<Trainer<FeatureTransformer>> feature_transformer_trainer_;
...
class Trainer<Layers::InputSlice<OutputDimensions, Offset>> {
   ...
   public:
	...
	// ハイパーパラメータなどのオプションを設定する
	void SendMessage(Message* message) { shared_input_trainer_->SendMessage(message); }
...
#if defined(USE_BLAS)
			cblas_scopy(kOutputDimensions, &input[input_offset + Offset], 1, &output_[output_offset], 1);
#else
   ...
   private:
	...
	Trainer(FeatureTransformer* feature_transformer)
	    : batch_size_(0), shared_input_trainer_(SharedInputTrainer::Create(feature_transformer)) {}
