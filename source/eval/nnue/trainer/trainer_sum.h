...
#if defined(USE_BLAS)
		cblas_saxpy(kOutputDimensions * batch_size_, 1.0, head_output, 1, output, 1);
#else
   ...
   private:
	...
	Trainer(LayerType* target_layer, FeatureTransformer* feature_transformer)
	    : Tail(target_layer, feature_transformer),
	      batch_size_(0),
	      previous_layer_trainer_(
	          Trainer<FirstPreviousLayer>::Create(&target_layer->previous_layer_, feature_transformer)),
...
class Trainer<Layers::Sum<PreviousLayer>> {
   ...
   public:
	...
	// ハイパーパラメータなどのオプションを設定する
	void SendMessage(Message* message) { previous_layer_trainer_->SendMessage(message); }
