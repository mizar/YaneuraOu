...
inline IndexType HalfKPE9<AssociatedKing>::MakeIndex(Square sq_k, BonaPiece p, int effect1, int effect2) {
	return (static_cast<IndexType>(fe_end) * static_cast<IndexType>(sq_k) + p) + (static_cast<IndexType>(fe_end) * static_cast<IndexType>(SQ_NB) * (effect1 * 3 + effect2));
}
...
void HalfKPE9<AssociatedKing>::AppendActiveIndices(
	...
	for (PieceNumber i = PIECE_NUMBER_ZERO; i < PIECE_NUMBER_KING; ++i) {
		...
		active->push_back(MakeIndex(sq_target_k, p, GetEffectCount(pos, sq_p, perspective, perspective, false), GetEffectCount(pos, sq_p, perspective, ~perspective, false)));
	}
}
...
void HalfKPE9<AssociatedKing>::AppendChangedIndices(
	...
	for (int i = 0; i < dp.dirty_num; ++i) {
		...
		removed->push_back(MakeIndex(sq_target_k, old_p, GetEffectCount(pos, old_sq_p, perspective, perspective, true), GetEffectCount(pos, old_sq_p, perspective, ~perspective, true)));
		...
		added->push_back(MakeIndex(sq_target_k, new_p, GetEffectCount(pos, new_sq_p, perspective, perspective, false), GetEffectCount(pos, new_sq_p, perspective, ~perspective, false)));
	}
