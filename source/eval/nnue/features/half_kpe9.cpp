...
// 駒の情報を取得する
template <Side AssociatedKing>
inline void HalfKPE9<AssociatedKing>::GetPieces(const Position& pos, Color perspective, BonaPiece** pieces,
                                                Square* sq_target_k) {
...
template <Side AssociatedKing>
void HalfKPE9<AssociatedKing>::AppendActiveIndices(const Position& pos, Color perspective, IndexList* active) {
	...
	for (PieceNumber i = PIECE_NUMBER_ZERO; i < PIECE_NUMBER_KING; ++i) {
		...
		active->push_back(MakeIndex(sq_target_k, p, GetEffectCount(pos, sq_p, perspective, perspective, false),
...
template <Side AssociatedKing>
void HalfKPE9<AssociatedKing>::AppendChangedIndices(const Position& pos, Color perspective, IndexList* removed,
                                                    IndexList* added) {
	BonaPiece* pieces;
	Square     sq_target_k;
	GetPieces(pos, perspective, &pieces, &sq_target_k);
	...
	for (int i = 0; i < dp.dirty_num; ++i) {
		...
		removed->push_back(MakeIndex(sq_target_k, old_p, GetEffectCount(pos, old_sq_p, perspective, perspective, true),
		...
		added->push_back(MakeIndex(sq_target_k, new_p, GetEffectCount(pos, new_sq_p, perspective, perspective, false),
