...
// 駒の情報を取得する
inline void PE9::GetPieces(const Position& pos, Color perspective, BonaPiece** pieces) {
	*pieces = (perspective == BLACK) ? pos.eval_list()->piece_list_fb() : pos.eval_list()->piece_list_fw();
}

// 特徴量のうち、値が1であるインデックスのリストを取得する
void PE9::AppendActiveIndices(const Position& pos, Color perspective, IndexList* active) {
	...
	for (PieceNumber i = PIECE_NUMBER_ZERO; i < PIECE_NUMBER_KING; ++i) {
		...
		active->push_back(MakeIndex(p, GetEffectCount(pos, sq_p, perspective, perspective, false),
		                            GetEffectCount(pos, sq_p, perspective, ~perspective, false)));
	}
}

// 特徴量のうち、一手前から値が変化したインデックスのリストを取得する
void PE9::AppendChangedIndices(const Position& pos, Color perspective, IndexList* removed, IndexList* added) {
	BonaPiece* pieces;
	GetPieces(pos, perspective, &pieces);
	...
	for (int i = 0; i < dp.dirty_num; ++i) {
		...
		removed->push_back(MakeIndex(old_p, GetEffectCount(pos, old_sq_p, perspective, perspective, true),
		...
		added->push_back(MakeIndex(new_p, GetEffectCount(pos, new_sq_p, perspective, perspective, false),
