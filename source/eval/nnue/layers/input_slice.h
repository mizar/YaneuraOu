...
class InputSlice {
   public:
	...
	static std::string GetStructureString() {
		return "InputSlice[" + std::to_string(kOutputDimensions) + "(" + std::to_string(Offset) + ":" +
		       std::to_string(Offset + kOutputDimensions) + ")]";
	}

	// パラメータを読み込む
	bool ReadParameters(std::istream& /*stream*/) { return true; }

	// パラメータを書き込む
	bool WriteParameters(std::ostream& /*stream*/) const { return true; }
