...
class Sum : public Sum<RemainingPreviousLayers...> {
   ...
   public:
	...
	static constexpr std::size_t kBufferSize = std::max(Head::kBufferSize + kSelfBufferSize, Tail::kBufferSize);
	...
	const OutputType* Propagate(const TransformedFeatureType* transformed_features, char* buffer) const {
		...
		const auto head_output = previous_layer_.Propagate(transformed_features, buffer + kSelfBufferSize);
   ...
   protected:
	...
	static std::string GetSummandsString() { return Head::GetStructureString() + "," + Tail::GetSummandsString(); }
...
class Sum<PreviousLayer> {
   ...
   protected:
	// 和を取る対象となる層のリストを表す文字列
	static std::string GetSummandsString() { return PreviousLayer::GetStructureString(); }
