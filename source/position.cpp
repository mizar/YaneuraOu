...
void Position::init() {
	...
	// 64bit hash keyは256bit hash keyの下位64bitという解釈をすることで、256bitと64bitのときとでhash
	// keyの下位64bitは合致するようにしておく。 これは定跡DBなどで使うときにこの性質が欲しいからである。
	// またpc==NO_PIECEのときは0であることを保証したいのでSET_HASHしない。
	// psqは、C++の規約上、事前にゼロであることは保証される。
	for (auto pc : Piece())
		for (auto sq : SQ)
			if (pc)
				SET_HASH(Zobrist::psq[sq][pc], rng.rand<Key>() & ~1ULL, rng.rand<Key>(), rng.rand<Key>(),
				         rng.rand<Key>());
	...
	for (auto c : COLOR)
		for (PieceType pr = NO_PIECE_TYPE; pr < PIECE_HAND_NB; ++pr)
			if (pr)
				SET_HASH(Zobrist::hand[c][pr], rng.rand<Key>() & ~1ULL, rng.rand<Key>(), rng.rand<Key>(),
				         rng.rand<Key>());
	...
	for (auto pc : Piece()) {
		auto pt = type_of(pc);
		if (!(pt == PAWN || pt == LANCE || pt == KNIGHT || pt == SILVER || pt == GOLD || pt == BISHOP || pt == ROOK ||
		      pt == KING || pt == PRO_PAWN || pt == PRO_LANCE || pt == PRO_KNIGHT || pt == PRO_SILVER || pt == HORSE ||
		      pt == DRAGON))
			continue;
...
std::string pretty(Piece pc) {
	...
	std::string result;
	if (pc != NO_PIECE) result = (color_of(pc) == BLACK) ? "\\e[32;40;1m" : "\\e[33;40;1m";
	result += USI_PIECE_KANJI[pc];
	if (pc != NO_PIECE) result += "\\e[m";
...
void Position::set(std::string sfen, StateInfo* si, Thread* th) {
	...
	PieceNumber piece_no_count[KING] = {PIECE_NUMBER_ZERO,   PIECE_NUMBER_PAWN,   PIECE_NUMBER_LANCE,
	                                    PIECE_NUMBER_KNIGHT, PIECE_NUMBER_SILVER, PIECE_NUMBER_BISHOP,
	                                    PIECE_NUMBER_ROOK,   PIECE_NUMBER_GOLD};
...
const std::string Position::sfen() const {
	...
	for (Rank r = RANK_1; r <= RANK_9; ++r) {
		for (File f = FILE_9; f >= FILE_1; --f) {
			...
			for (emptyCnt = 0; f >= FILE_1 && piece_on(f | r) == NO_PIECE; --f) ++emptyCnt;
...
void Position::set_state(StateInfo* si) const {
	...
	for (auto c : COLOR)
		for (PieceType pr = PAWN; pr < PIECE_HAND_NB; ++pr)
			si->hand_key_ +=
			    Zobrist::hand[c][pr] * (int64_t)hand_count(hand[c], pr);  // 手駒はaddにする(差分計算が楽になるため)
...
std::ostream& operator<<(std::ostream& os, const Position& pos) {
	// 盤面
	for (Rank r = RANK_1; r <= RANK_9; ++r) {
		for (File f = FILE_9; f >= FILE_1; --f) os << pretty(pos.board[f | r]);
		os << endl;
	}
...
Bitboard Position::attackers_to(Square sq, const Bitboard& occ) const {
	...
	return
	    // 先手の歩・桂・銀・金・HDK
	    (((pawnEffect(WHITE, sq) & pieces(PAWN)) | (knightEffect(WHITE, sq) & pieces(KNIGHT)) |
	      (silverEffect(WHITE, sq) & pieces(SILVER_HDK)) | (goldEffect(WHITE, sq) & pieces(GOLDS_HDK))) &
	     pieces(BLACK)) |

	    // 後手の歩・桂・銀・金・HDK
	    (((pawnEffect(BLACK, sq) & pieces(PAWN)) | (knightEffect(BLACK, sq) & pieces(KNIGHT)) |
	      (silverEffect(BLACK, sq) & pieces(SILVER_HDK)) | (goldEffect(BLACK, sq) & pieces(GOLDS_HDK))) &
	     pieces(WHITE))

	    // 先後の角・飛・香
	    | (bishopEffect(sq, occ) & pieces(BISHOP_HORSE)) |
	    (rookEffect(sq, occ) &
	     (pieces(ROOK_DRAGON) | (pieces(BLACK, LANCE) & lanceStepEffect(WHITE, sq)) |
...
bool Position::gives_check(Move m) const {
	...
	return !is_drop(m) && (((blockers_for_king(~sideToMove) /*& pieces(sideToMove)*/) & from) &&
...
Bitboard Position::pinned_pieces(Color c, Square avoid) const {
	...
	while (pinners) {
		...
		if (!more_than_one(b)) result |= b & pieces(c);
	}
	return result;
}
...
bool Position::legal_drop(const Square to) const {
	...
	while (escape_bb) {
		...
		if (!attackers_to(us, king_to, occ)) return true;  // 退路が見つかったので打ち歩詰めではない。
	}
	...
	if (~a8_effet_us & a8_them_movable) return true;
...
bool Position::pseudo_legal_s(const Move m) const {
	...
	if (is_drop(m)) {
		...
		if (moved_piece_after(m) != Piece(pr + ((us == WHITE) ? u32(PIECE_WHITE) : 0) + PIECE_DROP)) return false;
#endif
		...
		if (is_promote(m)) {
			...
			if (moved_piece_after(m) != Piece(pc + PIECE_PROMOTE)) return false;
#endif
			...
			if (All) {
				...
				if (pt == PAWN || pt == LANCE)
					if ((us == BLACK && rank_of(to) == RANK_1) || (us == WHITE && rank_of(to) == RANK_9)) return false;
			} else {
				// 歩の不成と香の2段目への不成を禁止。
				// 大駒の不成を禁止
				switch (pt) {
		...
		if (checkers()) {
			...
			if (type_of(pc) != KING) {
				...
				if (!((between_bb(checkers().pop(), king_square(us)) | checkers()) & to)) return false;
			}
...
bool Position::legal(Move m) const {
	if (is_drop(m))
		...
		if (type_of(piece_on(from)) == KING) return !effected_to(~us, move_to(m), from);

		// blockers_for_king()は、pinされている駒(自駒・敵駒)を表現するが、fromにある駒は自駒であることは
		// わかっているのでこれで良い。
		return !(blockers_for_king(us) & from) || aligned(from, to_sq(m), square<KING>(us));
	}
}
...
Move Position::to_move(Move16 m16) const {
	...
	return Move(
	    u16(m) + ((is_drop(m)      ? (Piece)(make_piece(sideToMove, move_dropped_piece(m)) + PIECE_DROP)
	               : is_promote(m) ? (Piece)(piece_on(move_from(m)) | PIECE_PROMOTE)
...
void Position::do_move_impl(Move m, StateInfo& new_st, bool givesCheck) {
	...
	if (is_drop(m)) {
		...
		if (type_of(moved_pc) == KING) kingSquare[Us] = to;
#endif
		...
		if (givesCheck) {
			...
			if (discovered(from, to, ksq, prevSt->blockersForKing[~Us] & from)) {
				...
				switch (pop_directions(directions)) {
						...
						// つまりfromから上下を見ると、敵玉と、自分の開き王手をしている遠方駒(飛車 or
						// 香)があるはずなのでこれを追加する。
					...
					case DIRECT_U:
					case DIRECT_D:
						st->checkersBB |= rookFileEffect(from, pieces()) & pieces(Us);
						break;
					...
					case DIRECT_R:
					case DIRECT_L:
						st->checkersBB |= rookRankEffect(from, pieces()) & pieces(Us);
						break;
					...
					case DIRECT_RU:
					case DIRECT_LD:
						st->checkersBB |= bishopEffect0(from, pieces()) & pieces(Us);
						break;

					case DIRECT_RD:
					case DIRECT_LU:
						st->checkersBB |= bishopEffect1(from, pieces()) & pieces(Us);
						break;
#else
					...
					case DIRECT_RD:
					case DIRECT_LU:
						st->checkersBB |= bishopEffect(ksq, pieces()) & pieces(Us, BISHOP_HORSE);
						break;
...
PieceType min_attacker(const Position& pos, const Square& to, const Bitboard& stmAttackers, Bitboard& occupied,
                       Bitboard& attackers) {
	...
	Bitboard b;
	b = stmAttackers & pos.pieces(PAWN);
	if (b) goto found;
	b = stmAttackers & pos.pieces(LANCE);
	if (b) goto found;
	b = stmAttackers & pos.pieces(KNIGHT);
	if (b) goto found;
	b = stmAttackers & pos.pieces(SILVER);
	if (b) goto found;
	b = stmAttackers & pos.pieces(GOLDS);
	if (b) goto found;
	b = stmAttackers & pos.pieces(BISHOP);
	if (b) goto found;
	b = stmAttackers & pos.pieces(ROOK);
	if (b) goto found;
	b = stmAttackers & pos.pieces(HORSE);
	if (b) goto found;
	b = stmAttackers & pos.pieces(DRAGON);
	if (b) goto found;
...
bool Position::see_ge(Move m, Value threshold) const {
	...
	if (balance < VALUE_ZERO) return false;
	...
	if (balance >= VALUE_ZERO) return true;
	...
	while (true) {
		...
		if (!(st->pinners[~stm] & occupied)) stmAttackers &= ~st->blockersForKing[stm];
		...
		if (balance >= VALUE_ZERO) {
			if (nextVictim == KING && (attackers & pieces(stm))) stm = ~stm;
			break;
		}
	...
	for (int i = 4; i <= end; i += 2) {
		...
		if (stp->board_key() == st->board_key()) {
			...
			if (stp->hand == st->hand) {
				...
				if (hand_is_equal_or_superior(st->hand, stp->hand)) return REPETITION_SUPERIOR;
				if (hand_is_equal_or_superior(stp->hand, st->hand)) return REPETITION_INFERIOR;
			}
		}
	}
	...
	for (int i = 3; i <= end; i += 2) {
		...
		if ((j = H1(moveKey), cuckoo[j] == moveKey) || (j = H2(moveKey), cuckoo[j] == moveKey)) {
			...
			if (!(between_bb(s1, s2) & pieces())) {
				if (plies_from_root > i) return true;
				...
				for (int k = i + 2; k <= end; k += 2) {
					...
					if (next_stp->key() == stp->key()) return true;
				}
			}
		}
	}
	...
	switch (rule) {
		...
		case EKR_27_POINT:  // 27点法 == CSAルール
		{
			...
			*/
			...
			// (b)宣言側の玉が敵陣三段目以内に入っている。
			if (!(ef & king_square(us))) return MOVE_NONE;

			// (e)宣言側の玉に王手がかかっていない。
			if (checkers()) return MOVE_NONE;

			// (d)宣言側の敵陣三段目以内の駒は、玉を除いて10枚以上存在する。
			int p1 = (pieces(us) & ef).pop_count();
			// p1には玉も含まれているから11枚以上ないといけない
			if (p1 < 11) return MOVE_NONE;
			...
			// ・後手の場合27点以上の持点がある。
			Hand h     = hand[us];
			int  score = p1 + p2 * 4 - 1 + hand_count(h, PAWN) + hand_count(h, LANCE) + hand_count(h, KNIGHT) +
			            hand_count(h, SILVER) + hand_count(h, GOLD) + (hand_count(h, BISHOP) + hand_count(h, ROOK)) * 5;

			// rule==EKR_27_POINTならCSAルール。rule==EKR_24_POINTなら24点法(30点以下引き分けなので31点以上あるときのみ勝ち扱いとする)
			if (score < (rule == EKR_27_POINT ? (us == BLACK ? 28 : 27) : 31)) return MOVE_NONE;
		...
		case EKR_TRY_RULE: {
			...
			if (!(kingEffect(king_sq) & king_try_sq)) return MOVE_NONE;

			// 2) トライする升に自駒がないか。
			if (pieces(us) & king_try_sq) return MOVE_NONE;

			// 3) トライする升に移動させたときに相手に取られないか。
			if (effected_to(~us, king_try_sq, king_sq)) return MOVE_NONE;
...
bool Position::pos_is_ok() const {
	...
	// 3) 王手している駒
	if (st->checkersBB != attackers_to(~sideToMove, king_square(sideToMove))) return false;

	// 4) 相手玉が取れるということはないか
	if (effected_to(sideToMove, king_square(~sideToMove))) return false;

	// 5) occupied bitboardは合っているか
	if ((pieces() != (pieces(BLACK) | pieces(WHITE))) || (pieces(BLACK) & pieces(WHITE))) return false;
