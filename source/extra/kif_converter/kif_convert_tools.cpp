	...
	static Bitboard dir_bb_(Color color, Square sq, Effect8::Direct dir, bool or_flag) {
		...
		switch (dir) {
			case Effect8::DIRECT_R:
				for (File f = File(file_of(sq) - offset); f >= FILE_1; f--)
					bb |= FILE_BB[f];
				break;
			case Effect8::DIRECT_L:
				for (File f = File(file_of(sq) + offset); f <= FILE_9; f++)
					bb |= FILE_BB[f];
				break;
			case Effect8::DIRECT_U:
				for (Rank r = Rank(rank_of(sq) - offset); r >= RANK_1; r--)
					bb |= RANK_BB[r];
				break;
			case Effect8::DIRECT_D:
				for (Rank r = Rank(rank_of(sq) + offset); r <= RANK_9; r++)
					bb |= RANK_BB[r];
				break;
			default:
				ASSERT_LV1(false);
