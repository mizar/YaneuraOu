...
struct CsaStringBuilder {
	...
	void append(Color c, const KifFormat& fmt) {
		if (fmt.color_type == ColorFmt_CSA) ss << ((c == BLACK) ? constStr.csa_color_black : constStr.csa_color_white);
	}
	...
	void append(const Position& pos, const KifFormat& fmt) {
		...
		for (Rank r = RANK_1; r <= RANK_9; ++r) {
			...
			for (File f = FILE_9; f >= FILE_1; --f) ss << constStr.csa_pos_piece[pos.piece_on(f | r)];
			ss << std::endl;
		}
		// 持駒
		for (Color c : {BLACK, WHITE}) {
			if (pos.hand_of(c) == HAND_ZERO) continue;
			...
			for (Piece pc : {ROOK, BISHOP, GOLD, SILVER, KNIGHT, LANCE, PAWN}) {
				...
				for (int i = 0; i < cnt; ++i) ss << constStr.csa_cap_sq << constStr.csa_piece[pc];
			}
			ss << std::endl;
		}
		// 手番
...
struct KifStringBuilder {
		...
		switch (m) {
			...
			default:
				...
				if (is_ok(prev_m) && move_to(prev_m) == move_to(m)) {
					...
					if (fmt.samepos_type == SamePosFmt_Verbose) append(move_to(m), fmt);
					ss << constStr.kif_move_samepos;
					// SamePosFmt_KIFsp の場合、「同」の後ろに全角空白を入れる
					// SamePosFmt_KI2sp の場合、成り指し手、成香・成桂・成銀の指し手では全角空白を入れない
					if (fmt.samepos_type == SamePosFmt_KIFsp ||
					    (fmt.samepos_type == SamePosFmt_KI2sp && !(is_promote(m)) &&
					     type_of(movedPieceType) != PRO_LANCE && type_of(movedPieceType) != PRO_KNIGHT &&
					     type_of(movedPieceType) != PRO_KNIGHT))
				...
				if (is_drop(m))
					...
					if (is_promote(m)) ss << constStr.kif_move_promote;
	...
	void append(Position& pos, const KifFormat& fmt) {
		...
		if (pos.hand_of(WHITE) == HAND_ZERO)
			...
			for (Piece pc : {ROOK, BISHOP, GOLD, SILVER, KNIGHT, LANCE, PAWN}) {
				...
				if (cnt > 0) ss << constStr.bod_hand_piece[pc] << constStr.bod_hand_num[cnt] << constStr.bod_hand_pad;
			}
		ss << std::endl;
		// 盤面
		...
		for (Rank r = RANK_1; r <= RANK_9; ++r) {
			...
			for (File f = FILE_9; f >= FILE_1; --f) ss << constStr.bod_piece[pos.piece_on(f | r)];
		...
		// TODO: 現在手数、直前の指し手出力
		// 後手番のみ追加行
		if (pos.side_to_move() == WHITE) ss << constStr.bod_turn_white << std::endl;
	}
		...
		switch (m) {
			...
			default:
				...
				if (is_ok(prev_m) && move_to(prev_m) == to) {
					// SamePosFmt_Verbose の場合、座標＋「同」～の表記にする
					if (fmt.samepos_type == SamePosFmt_Verbose) append(to, fmt);
					ss << constStr.kif_move_samepos;
					// SamePosFmt_KIFsp の場合、「同」の後ろに全角空白を入れる
					// SamePosFmt_KI2sp の場合、成り指し手、成香・成桂・成銀の指し手では全角空白を入れない
					if (fmt.samepos_type == SamePosFmt_KIFsp ||
					    (fmt.samepos_type == SamePosFmt_KI2sp && !(is_promote(m)) && pt != PRO_LANCE &&
					     pt != PRO_KNIGHT && pt != PRO_KNIGHT))
						ss << constStr.kif_fwsp;
				} else
					append(to, fmt);
				...
				if (!is_drop(m)) {
					...
					if (attackerBB != fromBB) {
						...
						if (from_rank == to_rank) {
							// 真横に寄る同種駒がそれだけなら「寄」
							if ((attackerBB & RANK_BB[to_rank]) == fromBB) ss << constStr.kif_move_slide;
							// 右から寄るなら「右」
							else if ((fromBB & dir_bb(DIRECT_R)) == fromBB) {
								ss << constStr.kif_move_right;
								// 右から動く駒がそれだけでは無いならさらに「寄」
								if ((attackerBB & dir_bb(DIRECT_R)) != fromBB) ss << constStr.kif_move_slide;
							}
							// 左から寄るなら「左」
							else if ((fromBB & dir_bb(DIRECT_L)) == fromBB) {
								ss << constStr.kif_move_left;
								// 左から動く駒がそれだけでは無いならさらに「寄」
								if ((attackerBB & dir_bb(DIRECT_L)) != fromBB) ss << constStr.kif_move_slide;
							}
						}
						// 前に上がるMoveの場合
						else if ((fromBB & dir_bb(DIRECT_D)) == fromBB) {
							// 前に上がる同種駒がそれだけなら「上」
							if ((attackerBB & dir_bb(DIRECT_D)) == fromBB) ss << constStr.kif_move_upper;
							// 真っ直ぐ上がるMoveの場合
							else if (from_file == to_file) {
								// 金・銀・成金なら「直」
								if (is_like_goldsilver) ss << constStr.kif_move_straight;
							...
							// 右から上がるMoveの場合
								...
								ss << constStr.kif_move_right;
								// 右から動ける駒が他にもあればさらに「上」
								if ((attackerBB & dir_bb(DIRECT_R)) != fromBB) ss << constStr.kif_move_upper;
							}
							// 左から上がるMoveの場合
							else if ((fromBB & dir_bb(DIRECT_L)) == fromBB) {
								ss << constStr.kif_move_left;
								// 左から動ける駒が他にもあればさらに「上」
								if ((attackerBB & dir_bb(DIRECT_L)) != fromBB) ss << constStr.kif_move_upper;
							}
						}
						// 後ろに引くMoveの場合
						else if ((fromBB & ForwardRanksBB[c][to_rank]) == fromBB) {
							// 後ろに引く同種駒がそれだけなら
							if ((attackerBB & ForwardRanksBB[c][to_rank]) == fromBB) ss << constStr.kif_move_lower;
							// 真っ直ぐ引くMoveの場合
							else if (from_file == to_file) {
								// 同じ筋・より右の筋に他に動ける駒がないなら「右」
								if ((attackerBB & dir_or_bb(DIRECT_R)) == fromBB) ss << constStr.kif_move_right;
								// 同じ筋・より左の筋に他に動ける駒がないなら「左」
								else if ((attackerBB & dir_or_bb(DIRECT_L)) == fromBB)
									ss << constStr.kif_move_left;
								// 「右引」の判定
							...
							// 右から引くMoveの場合
								...
								ss << constStr.kif_move_right;
								// 右から動ける駒が他にもあればさらに「引」
								if ((attackerBB & dir_bb(DIRECT_R)) != fromBB) ss << constStr.kif_move_lower;
							}
							// 左から引くMoveの場合
							else if ((fromBB & dir_bb(DIRECT_L)) == fromBB) {
								ss << constStr.kif_move_left;
								// 左から動ける駒が他にもあればさらに「引」
								if ((attackerBB & dir_bb(DIRECT_L)) != fromBB) ss << constStr.kif_move_lower;
							}
						}
					}
   ...
   private:
	...
	static Bitboard dir_bb_(Color color, Square sq, Effect8::Direct dir, bool or_flag) {
		...
		Bitboard bb = ZERO_BB;
		switch (dir) {
			case Effect8::DIRECT_R:
				for (File f = File(file_of(sq) - offset); f >= FILE_1; f--) bb |= FILE_BB[f];
				break;
			case Effect8::DIRECT_L:
				for (File f = File(file_of(sq) + offset); f <= FILE_9; f++) bb |= FILE_BB[f];
				break;
			case Effect8::DIRECT_U:
				for (Rank r = Rank(rank_of(sq) - offset); r >= RANK_1; r--) bb |= RANK_BB[r];
				break;
			case Effect8::DIRECT_D:
				for (Rank r = Rank(rank_of(sq) + offset); r <= RANK_9; r++) bb |= RANK_BB[r];
				break;
			default:
				ASSERT_LV1(false);
		}
