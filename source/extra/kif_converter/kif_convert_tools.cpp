...
namespace KifConvertTools
{
	...
	private:
		...
		std::basic_stringstream<typename constT::char_type> ss;
	};
	...
	struct KifStringBuilder
	{
		...
		void append(Position& pos, const KifFormat& fmt)
		{
			...
			for (Rank r = RANK_1; r <= RANK_9; ++r)
			{
				...
				ss << constStr.bod_vline << std::endl;
			}
			ss << constStr.bod_hline << std::endl;
			// 先手の持駒
		...
		string_type to_kif_string(Position& pos, const KifFormat& fmt)
		{
			...
			append(pos, fmt);
			ss << constStr.kiflist_head;
			while (states.size())
			{
				auto& top = states.top();
				Move m = top->lastMove;
				ss << pos.game_ply() << constStr.kiflist_pad; // TODO: 手数の整形処理
				append(m, pos.moved_piece_after(m), pos.side_to_move(), fmt);
				// TODO: 指し手の適切なパディング
				// TODO: 消費時間等の出力
				ss << std::endl;
