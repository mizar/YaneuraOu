...
struct BitStream {
	...
	FORCE_INLINE void write_one_bit(int b) {
		if (b) data[bit_cursor / 8] |= 1 << (bit_cursor & 7);
...
struct SfenPacker {
	...
	void pack(const Position& pos) {
		...
		// 先手玉、後手玉の位置、それぞれ7bit
		for (auto c : COLOR) stream.write_n_bit(pos.king_square(c), 7);
		...
		for (auto c : COLOR)
			for (PieceType pr = PAWN; pr < KING; ++pr) {
				...
				// この駒、n枚持ってるよ
				for (int i = 0; i < n; ++i) write_hand_piece_to_stream(make_piece(c, pr));
			}
	...
	string unpack() {
		...
		// まず玉の位置
		for (auto c : COLOR) board[stream.read_n_bit(7)] = make_piece(c, KING);

		// 盤上の駒
		for (auto sq : SQ) {
			// すでに玉がいるようだ
			if (type_of(board[sq]) == KING) continue;
	...
	void write_board_piece_to_stream(Piece pc) {
		...
		// 成りフラグ
		// (金はこのフラグはない)
		if (pr != GOLD) stream.write_one_bit((PIECE_PROMOTE & pc) ? 1 : 0);
	...
	void write_hand_piece_to_stream(Piece pc) {
		...
		if (pr != GOLD) stream.write_one_bit(false);
	...
	Found:;
		if (pr == NO_PIECE_TYPE) return NO_PIECE;
	...
	Piece read_hand_piece_from_stream() {
		...
		while (true) {
			...
			for (pr = PAWN; pr < KING; ++pr)
				if ((huffman_table[pr].code >> 1) == code && (huffman_table[pr].bits - 1) == bits) goto Found;
		}
	Found:;
		ASSERT_LV3(pr != NO_PIECE_TYPE);

		// 金以外であれば成りフラグを1bit捨てる
		if (pr != GOLD) stream.read_one_bit();
...
Tools::Result Position::set_from_packed_sfen(const PackedSfen& sfen, StateInfo* si, Thread* th, bool mirror,
                                             int gamePly_ /* = 0 */) {
	SfenPacker packer;
	...
	// まず玉の位置
	if (mirror) {
		for (auto c : COLOR) board[Mir((Square)stream.read_n_bit(7))] = make_piece(c, KING);
	} else {
		for (auto c : COLOR) board[stream.read_n_bit(7)] = make_piece(c, KING);
	}
	...
	for (auto sq : SQ) {
		...
		if (stream.get_cursor() > 256) return Tools::Result(Tools::ResultCode::SomeError);
		// ASSERT_LV3(stream.get_cursor() <= 256);
	}
