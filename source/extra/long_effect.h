...
template <Color c>
Direct direct_knight_of(Square sq1, Square sq2) {
...
const Square DirectToDelta_[DIRECT_NB_PLUS4] = {
    SQ_RU, SQ_R, SQ_RD, SQ_U, SQ_D, SQ_LU, SQ_L, SQ_LD, SQ_RUU, SQ_LUU, SQ_RDD, SQ_LDD,
};
inline Square DirectToDelta(Direct d) {
	ASSERT_LV3(is_ok(d));
	return DirectToDelta_[d];
}
...
const uint16_t long_effect16_table[PIECE_NB] = {
    ...
    0,
    0,
    uint16_t(DIRECTIONS_D << 8) /*香*/,
    0,
    0,
    uint16_t(BISHOP_DIR << 8),
    uint16_t(ROOK_DIR << 8),
    0,
    0,
    0,
...
struct WordBoard {
	...
	template <Color Us>
	Effect24::Directions long_effect24_to_around9(Square sq) const {
		// This algorithm is developed by tanuki- and yaneurao in 2016.
		...
		u32       bits0  = PEXT32(
            (ymm((u8*)&le16[sq - SQ_33 + 0] + offset) & Effect24::ymm_direct_to_around8[0]).eq(ymm_zero).to_uint32(),
            0b00000101010101000000000101010101);  // 16升中10升
		u32 bits1 = PEXT32(
		    (ymm((u8*)&le16[sq - SQ_33 + 16] + offset) & Effect24::ymm_direct_to_around8[1]).eq(ymm_zero).to_uint32(),
		    0b01010101010000000001010001010000);  // 16升中 9升
		u32 bits2 = PEXT32(
		    (ymm((u8*)&le16[sq - SQ_33 + 32] + offset) & Effect24::ymm_direct_to_around8[2]).eq(ymm_zero).to_uint32(),
		    0b00000000000000010101010100000000);  // 16升中 5升
...
template <Color Us>
void update_by_dropping_piece(Position& pos, Square to, Piece moved_pc);
...
template <Color Us>
void rewind_by_dropping_piece(Position& pos, Square to, Piece moved_pc);
template <Color Us>
