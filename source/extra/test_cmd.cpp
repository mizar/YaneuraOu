...
void effect_check(Position& pos) {
	...
	for (auto c : COLOR)
		for (auto sq : SQ)
			if (bb[c].effect(sq) != pos.board_effect[c].effect(sq)) {
				cout << "Error effect count of " << c << " at " << sq << endl
				     << pos << "wrong\n"
				     << bb[c] << endl
				     << "correct\n"
	...
	for (auto sq : SQ)
		if (wb.long_effect16(sq) != pos.long_effect.long_effect16(sq)) {
			cout << "Error long effect at " << sq << endl
			     << pos << "wrong\n"
			     << wb << endl
			     << "correct\n"
					...
					if (m != MOVE_NONE) {
						...
						if (!pos.is_mated()) {
							...
							if ((mate_found % 10000) == 0) {
								cout << endl
								     << "mate found = " << mate_found << " , mate miss = " << mate_missed
								     << " , mate found rate  = "
								     << double(100 * mate_found) / (mate_found + mate_missed) << "%" << endl;
							}
						}
					} else {
						// 1手詰め判定で漏れた局面を探す
						for (auto m : MoveList<LEGAL_ALL>(pos)) {
							...
							if (pos.is_mated()) {
								...
								// cout << endl << pos << "mated = " << m.move << ", but mate1ply() = MOVE_NONE." <<
								// endl;
								pos.mate1ply();
...
void test_genchecks(Position& pos, uint64_t loop_max) {
	...
	for (uint64_t i = 0; i < loop_max; ++i) {
		...
		// 局面を巻き戻してみる(undo_moveの動作テストを兼ねて)
		while (ply > 0) pos.undo_move(moves[--ply]);
...
void cooperation_mate(Position& pos, int depth) {
	...
	for (auto m : MoveList<LEGAL_ALL>(pos)) {
		...
		if (pos.is_mated()) {
			...
			// 残り探索深さがあるなら再帰的に探索する。
			if (depth > 1) cooperation_mate(pos, depth - 1);
		}
...
void generate_moves_cmd(Position& pos) {
	...
	if (pos.checkers())
		for (auto m : MoveList<EVASIONS>(pos)) cout << m.move << ' ';
	else
		for (auto m : MoveList<NON_EVASIONS>(pos)) cout << m.move << ' ';
	...
	for (int i = 0; i < 128; ++i)
		for (int j = 0; j < 128; ++j) {
			...
			bool is_equal_or_superior =
			    hand_count(h1, PAWN) >= hand_count(h2, PAWN) && hand_count(h1, LANCE) >= hand_count(h2, LANCE) &&
			    hand_count(h1, KNIGHT) >= hand_count(h2, KNIGHT) && hand_count(h1, SILVER) >= hand_count(h2, SILVER) &&
			    hand_count(h1, BISHOP) >= hand_count(h2, BISHOP) && hand_count(h1, ROOK) >= hand_count(h2, ROOK) &&
...
void test_read_record(Position& pos, istringstream& is) {
	...
	while (Tools::getline(fs, line)) {
		...
		if (token == "startpos") {
			...
			while (ss >> token && token != "moves") sfen += token;
...
void test_timeman() {
	...
	auto simulate = [](Search::LimitsType limits) {
		...
		cout << "initial setting "
		     << " time = " << limits.time[BLACK] << ", byoyomi = " << limits.byoyomi[BLACK]
		     << ", inc = " << limits.inc[BLACK] << ", NetworkDelay = " << delay << ", NetworkDelay2 = " << delay2
		     << ", MinimumThinkingTime = " << Options["MinimumThinkingTime"]
		     << ", max_game_ply = " << limits.max_game_ply << ", rtime = " << limits.rtime << endl;
		...
		for (int ply = 1; ply <= limits.max_game_ply; ply += 2) {
			...
			cout << "ply = " << ply << " , minimum = " << time.minimum() << " , optimum = " << time.optimum()
			...
			if (used_time <= 2000) used_time = 1000;
...
void unit_test(Position& pos, istringstream& is) {
	...
	// -- 色んな関数
	{
		...
		for (auto pc : Piece()) {
			auto pt = type_of(pc);
			success &=
			    (pt == LANCE || pt == BISHOP || pt == ROOK || pt == HORSE || pt == DRAGON) == (has_long_effect(pc));
		}
		check(success);
	}
...
#if defined(MATE_1PLY) && defined(LONG_EFFECT_LIBRARY)
	{
		...
		MateProblem mp[] = {
		    ...
		    // 駒打ちによる詰み
		    {"ln1g1gs1l/1r3p3/pppp1bppp/4pkn+R1/3Ns4/4PP1P1/PPPP1SP1P/5G1B1/L1SGK1L2 b N 1",
		     make_move_drop(KNIGHT, SQ_36)},
...
void book_check(Position& pos, Color rootTurn, Book::MemoryBook& book, string sfen, ofstream& of) {
	...
	if (it != nullptr) {
		...
		for (size_t i = 0; i < n; ++i) {
			...
			if (pos.game_ply() >= 5 && move_list[0].value - move_list[i].value >= 50) continue;
#endif
...
void book_check_cmd(Position& pos, istringstream& is) {
	...
	if (turn == "all") {
		for (auto rootTurn : COLOR) book_check(pos, rootTurn, book, sfen, of);
	} else if (turn == "black") {
...
#define KKPP_BIN "KKPP_synthesized.bin"
	...
	void read(string dir) {
		...
		if (!EvalIO::eval_convert(input, output, nullptr)) goto Error;
	...
	void to_kkpt() {
		...
		for (int p1 = 0; p1 < fe_end; ++p1)
			for (int p2 = 0; p2 < fe_end; ++p2) {
				int sum = 0;
				for (auto sq : SQ) sum += (*kpp_)[sq][p1][p2][1];
				...
				for (auto sq : SQ) (*kpp_)[sq][p1][p2][1] = z;
...
void eval_convert(istringstream& is) {
	...
	auto is_valid_format = [](std::string format) {
		...
		if (!result) cout << "Error! Unknow format , format = " << format << endl;
		return result;
	};
	if (!is_valid_format(input_format) || !is_valid_format(output_format)) return;
...
void dump_sfen(Position& pos, istringstream& is) {
	...
	StateInfo si;
	while (!fs.eof()) {
		if (!fs.read((char*)&sfen, sizeof(Learner::PackedSfenValue))) break;

		// 指定番号になるまでskip
		if (num < start_number) goto NEXT;
...
void test_kif_convert_tools(Position& pos, istringstream& is) {
	...
	while (true) {
		...
		if (token == "")
			break;
		else if (token == "sfenrec") {
			all     = false;
			sfenrec = true;
		} else if (token == "csarec") {
			all    = false;
			csarec = true;
		} else if (token == "kifrec") {
			all    = false;
			kifrec = true;
		} else if (token == "kif2rec") {
			all     = false;
			kif2rec = true;
		} else if (token == "sfen") {
			all  = false;
			...
			is >> fmti;
			if (fmti >= 0 && fmti < (int)KifConvertTools::ColorFmt_NB) colorfmt = (KifConvertTools::ColorFormat)fmti;
		} else if (token == "squareformat") {
			is >> fmti;
			if (fmti >= 0 && fmti < (int)KifConvertTools::SqFmt_NB) sqfmt = (KifConvertTools::SquareFormat)fmti;
		} else if (token == "sameposformat") {
			is >> fmti;
			if (fmti >= 0 && fmti < (int)KifConvertTools::SamePosFmt_NB) spfmt = (KifConvertTools::SamePosFormat)fmti;
		}
	}
	...
	if (all || sfenrec) {
		std::cout << "sfenrec:" << std::endl << KifConvertTools::to_sfen_string(pos);
	}
	if (all || csarec) {
		std::cout << "csarec:" << std::endl << KifConvertTools::to_csa_string(pos, KifConvertTools::CsaFmt);
	}
	if (all || kifrec) {
		KifConvertTools::KifFormat fmt(colorfmt, sqfmt, spfmt);
		std::cout << "kifrec:" << std::endl << KifConvertTools::to_kif_string(pos, KifConvertTools::KifFmtKn1);
	}
	if (all || kif2rec) {
		KifConvertTools::KifFormat fmt(colorfmt, sqfmt, spfmt);
		std::cout << "kif2rec:" << std::endl << KifConvertTools::to_kif2_string(pos, KifConvertTools::KifFmtK2);
	}
	if (all || sfen) {
		std::cout << "sfen:";
	...
	if (all || kif) {
		KifConvertTools::KifFormat fmt(colorfmt, sqfmt, spfmt);
		std::cout << "kif:";
		for (auto m : MoveList<LEGAL_ALL>(pos)) std::cout << " " << KifConvertTools::to_kif_string(pos, m.move, fmt);
		std::cout << std::endl;
	}
	if (all || kif2) {
		KifConvertTools::KifFormat fmt(colorfmt, sqfmt, spfmt);
		std::cout << "kif2:";
		for (auto m : MoveList<LEGAL_ALL>(pos)) std::cout << " " << KifConvertTools::to_kif2_string(pos, m.move, fmt);
		std::cout << std::endl;
	}
}
...
void test_cmd(Position& pos, istringstream& is) {
	...
	is >> param;
	if (param == "unit")
		unit_test(pos, is);  // 単体テスト
	else if (param == "rp")
		random_player_cmd(pos, is);  // ランダムプレイヤー
	else if (param == "rpbench")
		random_player_bench_cmd(pos, is);  // ランダムプレイヤーベンチ
	else if (param == "cm")
		cooperation_mate_cmd(pos, is);  // 協力詰めルーチン
	else if (param == "checks")
		test_genchecks(pos, is);  // 王手生成ルーチンのテスト
	else if (param == "hand")
		test_hand();  // 手駒の優劣関係などのテスト
	else if (param == "records")
		test_read_record(pos, is);  // 棋譜の読み込みテスト
	else if (param == "autoplay")
		auto_play(pos, is);  // 思考ルーチンを呼び出しての連続自己対戦
	else if (param == "timeman")
		test_timeman();  // TimeManagerのテスト
	else if (param == "exambook")
		exam_book(pos);  // 定跡の精査用コマンド
	else if (param == "bookcheck")
		book_check_cmd(pos, is);  // 定跡のチェックコマンド
#if defined(EVAL_LEARN)
	else if (param == "search")
		test_search(pos, is);  // 現局面からLearner::search()を呼び出して探索させる
	else if (param == "dumpsfen")
		dump_sfen(pos, is);  // gensfenコマンドで生成した教師局面のダンプ
	else if (param == "evalsave")
		Eval::save_eval("");  // 現在の評価関数のパラメーターをファイルに保存
#endif
#if defined(EVAL_KPPT) || defined(EVAL_KPP_KKPT)
	else if (param == "evalmerge")
		eval_merge(is);  // 評価関数の合成コマンド
	else if (param == "evalconvert")
		eval_convert(is);  // 評価関数の変換コマンド
	else if (param == "evalexam")
		eval_exam(is);  // 評価関数ファイルの調査用
	else if (param == "evalresolve")
		eval_resolve(is);  // 評価関数ファイルの調査用
#if defined(EVAL_LEARN)
	else if (param == "regkk")
		regularize_kk_cmd(is);  // 評価関数のKKの正規化
#endif
#endif
#if defined(USE_KIF_CONVERT_TOOLS)
	else if (param == "kifconvert")
		test_kif_convert_tools(pos, is);  // 現局面からの全合法手を各種形式で出力チェック
#endif
#if defined(EVAL_NNUE)
	else if (param == "nnue")
