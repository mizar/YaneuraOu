...
void AperyBook::init() {
	...
	for (Piece p = PIECE_ZERO; p < PIECE_NB - 1; ++p) {
		for (Square sq = SQ_ZERO; sq < SQ_NB; ++sq) ZobPiece[p][sq] = mt64bit_.random() * (p != NO_PIECE);
	}
	for (Piece hp = PIECE_ZERO; hp < PIECE_HAND_NB - 1; ++hp) {
		for (int num = 0; num < 19; ++num) ZobHand[hp][num] = mt64bit_.random();
	}
...
AperyBook::AperyBook(const std::string& filename) {
	...
	if (!file.is_open()) sync_cout << "info string could not open an apery book file. " << filename << sync_endl;
...
Key AperyBook::bookKey(const Position& pos) {
	...
	for (PieceType hp = PAWN; hp < PIECE_HAND_NB; ++hp) key ^= ZobHand[hand_piece_map[hp]][hand_count(hand, hp)];
	if (pos.side_to_move() == WHITE) key ^= ZobTurn;
	return key;
}
