...
void MultiThinkBook::thread_worker(size_t thread_id) {
	...
	while ((id = get_next_loop_count()) != UINT64_MAX) {
		...
		for (size_t i = 0; i < m; ++i) {
			...
			BookPos bp(rootMoves.pv[0], nextMove, rootMoves.score, search_depth, int(atof(ENGINE_VERSION) * 100));
		...
		sync_cout << "[" << get_done_count() << "/" << get_loop_max() << ":" << thread_id << "] " << Tools::now_string()
		          << " : " << sfen << sync_endl;
#endif
	}
}
...
void makebook_cmd(Position& pos, istringstream& is) {
	...
	if (makebook2019(pos, is, token)) return;
	...
	if (from_sfen || from_thinking) {
		...
		if (from_sfen) cout << "read sfen moves " << moves << endl;
		if (from_thinking)
			cout << "read sfen moves from " << start_moves << " to " << moves << " , depth = " << depth
			     << " , nodes = " << nodes << " , cluster = " << cluster_id << "/" << cluster_num << endl;
		...
		if (!bw_files) {
			...
			for (auto& sfen : tmp_sfens) sfens.push_back(SfenAndColor(sfen, COLOR_NB));
		} else {
			...
			for (auto c : COLOR) {
				...
				// 読み込まないという指定になる。
				if (filename == "no_file") continue;

				vector<string> tmp_sfens;
				FileOperator::ReadAllLines(filename, tmp_sfens);
				for (auto& sfen : tmp_sfens) sfens.push_back(SfenAndColor(sfen, c));
			}
		}
		...
		for (size_t k = 0; k < sfens.size(); ++k) {
			...
			if (hirate) pos.set_hirate(&state, Threads.main());
			...
			for (int i = 0; i < (int)sf.size() - (from_sfen ? 1 : 0); ++i) {
				if (i < start_moves - 1) continue;

				// 現局面の手番が望むべきものではないので解析をskipする。
				if (!sf[i].second /* sf[i].is_valid */) continue;
				...
				if (from_sfen) {
					...
					if (thinking_sfens.count(sfen) == 0) thinking_sfens.insert(sfen);
				}
			}
		...
		if (from_thinking) {
			...
			if (cluster_id != 1 || cluster_num != 1) {
				...
				for (int i = 0; i < (int)sfens_.size(); ++i) {
					if ((i % cluster_num) == cluster_id - 1) a.push_back(sfens_[i]);
				}
				sfens_ = a;
			...
			multi_think.callback_func    = [&]() {
                ...
                if (multi_think.appended) {
                    ...
                    sync_cout << "Save start : " << Tools::now_string() << " , book_name = " << write_book_name
                              << sync_endl;
		...
		for (int i = 0; i < 2; ++i) {
			if (book[i].read_book(book_name[i]).is_not_ok()) return;
		}
		...
		cout << "same nodes = " << same_nodes << " , different nodes =  " << diffrent_nodes1 << " + " << diffrent_nodes2
		     << endl;
...
Tools::Result MemoryBook::read_book(const std::string& filename, bool on_the_fly_) {
	...
	if (pure_filename == kAperyBookName) {
		...
		auto calc_prob = [&] {
			...
			u64 num_sum = 0;
			for (auto& it : move_list) num_sum += it.num;
			num_sum = std::max(num_sum, UINT64_C(1));  // ゼロ除算対策
			for (auto& it : move_list) it.prob = float(it.num) / num_sum;
			num_sum = 0;
		};
		...
		while (reader.ReadLine(line).is_ok()) {
			...
			if (line.length() >= 5 && line.substr(0, 5) == "sfen ") {
				...
				if (ignoreBookPly) StringExtension::trim_number_inplace(sfen);  // 末尾の数字除去
...
Tools::Result MemoryBook::write_book(const std::string& filename /*, bool sort*/) const {
	...
	// (Options["BookOnTheFly"]==true かつ Options["IgnoreBookPly"] == true
	// のときに、手数違いのものがヒットするだとか、そういう問題と、
	...
	for (auto& it : vectored_book) {
		...
		auto sfen_left = StringExtension::trim_number(sfen);  // 末尾にplyがあるはずじゃろ
		int  ply       = StringExtension::to_int(sfen.substr(sfen_left.length()), 0);
		if (book_ply[sfen_left] != ply) continue;
		...
		if (fs.fail()) return Tools::Result(Tools::ResultCode::FileWriteError);
	}
...
BookType::iterator MemoryBook::find(const std::string& sfen) { return book_body.find(trim(sfen)); }
...
PosMoveListPtr MemoryBook::find(const Position& pos) {
	...
	if (pure_book_name == "no_book") return PosMoveListPtr();
	...
	if (pure_book_name == kAperyBookName) {
		...
		if (!on_the_fly && book_body.size() == 0) return PosMoveListPtr();
		...
		if (on_the_fly) {
			...
			while (true) {
				...
				if (sfen2 == "" || sfen < sfen2) {  // 左(それより小さいところ)を探す
					e = m;
				} else if (sfen > sfen2) {  // 右(それより大きいところ)を探す
					s = current_pos();
				} else {
					// 見つかった！
					break;
			...
			auto calc_prob = [&] {
				...
				std::stable_sort(move_list.begin(), move_list.end());
				num_sum = std::max(num_sum, UINT64_C(1));  // ゼロ除算対策
				for (auto& bp : move_list) bp.prob = float(bp.num) / num_sum;
				num_sum = 0;
			};
...
Tools::Result MemoryBook::read_apery_book(const std::string& filename) {
	// 読み込み済であるかの判定
	if (book_name == filename) return Tools::Result::Ok();
	...
	auto report = [&]() {
		cout << "# seen positions = " << seen.size() << ", size of converted book = " << book_body.size()
		     << ", # hash collisions detected = " << collisions << endl;
	};
...
void BookMoveSelector::init(USI::OptionsMap& o) {
	...
	std::vector<std::string> book_list = {
	    "no_book",          "standard_book.db", "yaneura_book1.db", "yaneura_book2.db", "yaneura_book3.db",
	    "yaneura_book4.db", "user_book1.db",    "user_book2.db",    "user_book3.db",    "book.bin"};
...
string BookMoveSelector::pv_builder(Position& pos, Move16 m16, int depth) {
	...
	if (pos.pseudo_legal(m) && pos.legal(m)) {
		...
		if (!probe_impl(pos, true, bestMove16, ponderMove16, true /* 強制的にhitさせる */)) goto UNDO;

		if (depth > 0) result = pv_builder(pos, bestMove16, depth - 1);  // さらにbestMoveで指し手を進める。
...
bool BookMoveSelector::probe_impl(Position& rootPos, bool silent, Move16& bestMove, Move16& ponderMove, bool forceHit) {
	if (!forceHit) {
		...
		if (!forceHit && rootPos.game_ply() > book_ply) return false;
	}

	auto it = memory_book.find(rootPos);
	if (it == nullptr || it->size() == 0) return false;
	...
	if (!silent) {
		...
		for (size_t i = 0; i < move_list.size(); ++i) {
			...
			if (pv_moves <= 1)
				...
				pv_string = (rest != "") ? (it.bestMove.to_usi_string() + rest)
			...
			sync_cout << "info"
			          ...
			          << " multipv " << (i + 1)
#endif
			          << " score cp " << it.value << " depth " << it.depth << " pv " << pv_string << " (" << fixed
			          << std::setprecision(2) << (100 * it.prob) << "%)"  // 採択確率
			          << sync_endl;
	...
	if (forceHit) {
		...
		auto it_end =
		    std::remove_if(move_list.begin(), move_list.end(), [&](Book::BookPos& m) { return m.value < value_limit; });
		...
		if (narrowBook) {
			...
			// 出現確率10%未満のものを取り除く。
			auto it_end =
			    std::remove_if(move_list.begin(), move_list.end(), [](Book::BookPos& m) { return m.prob < 0.1; });
			move_list.erase(it_end, move_list.end());

			// 1手でも取り除いたなら、定跡から取り除いたことをGUIに出力
			if (!silent && (n != move_list.size()))
				sync_cout << "info string NarrowBook : " << n << " moves to " << move_list.size() << " moves."
				          << sync_endl;
		}

		if (move_list.size() == 0) return false;
		...
		if ((depth_limit != 0 && move_list[0].depth < depth_limit)) {
			if (!silent) sync_cout << "info string BookDepthLimit is lower than the depth of this node." << sync_endl;
			move_list.clear();
		} else {
			...
			auto it_end = std::remove_if(move_list.begin(), move_list.end(),
			                             [&](Book::BookPos& m) { return m.value < value_limit; });
			move_list.erase(it_end, move_list.end());
...

	{
		...
		if (forceHit || Options["ConsiderBookMoveCount"]) {
			...
			// 採用回数の合計。
			u64 sum = 0;
			for (auto& move : move_list) sum += move.num;
...
Move BookMoveSelector::probe(Position& pos) {
	...
	Move16     bestMove16, ponderMove16;
	if (!probe_impl(pos, silent, bestMove16, ponderMove16)) return MOVE_NONE;
	...
	if (!pos.pseudo_legal(bestMove) || !pos.legal(bestMove)) {
		...
		cout << "Error : illigal move in book" << endl << pos.sfen() << endl << "Move = " << bestMove << endl;
		return MOVE_NONE;
	}
...
bool BookMoveSelector::probe(Thread& th, Search::LimitsType& Limits) {
	...
	if (!Options["USI_OwnBook"]) return false;
