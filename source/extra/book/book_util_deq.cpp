...
namespace BookUtil
{
	...
	int deqBook::read_book(const std::string & filename, bool sfen_n11n, Thread* th)
	{
		// 読み込み済であるかの判定
		...
		while (idx1 > 0)
		{
			...
			// BookEntry 構築
			BookEntry be(buffer + idx0 + 5, idx1 - idx0 - 5, sfen_n11n, th);
	...
	MoveListPtr deqBook::get_entries(const Position & pos)
	{
		auto it = find(pos);
		if (it == end())
			return MoveListPtr();
