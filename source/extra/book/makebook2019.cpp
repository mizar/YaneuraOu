...
std::vector<std::string> start_sfens = {
    ...
    /*public static readonly string HANDICAP_RIGHT_KYO = */
    ...
    /*public static readonly string HANDICAP_HISYA_KYO = */
...
struct BookTreeBuilder {
   ...
   private:
	...
	void do_move(Position& pos, Move m, StateInfo& si) {
...
void BookTreeBuilder::output_progress() {
	if ((total_node % 1000) == 0) {
		...
		if (total_write_node) cout << "|" << total_write_node;
	}
	if ((total_node % 10) == 0) cout << ".";
	++total_node;
}
...
VMD_Pair BookTreeBuilder::build_tree_nega_max(Position& pos, MemoryBook& read_book, MemoryBook& write_book) {
	...
	// 現局面で詰んでいる
	if (pos.is_mated()) return VMD_Pair(mated_in(0), MOVE_NONE, 0);

	// 現局面で宣言勝ちできる。
	// 定跡ファイルにMOVE_WINが紛れたときの解釈を規定していないのでここでは入れないことにする。
	if (pos.DeclarationWin() != MOVE_NONE) return VMD_Pair(mate_in(1), MOVE_NONE, 0);
		...
		switch (draw_type) {
			...
			case REPETITION_DRAW:
				// 千日手局面を実際に見つけて、その直後の指し手を取得する。
				{
					...
					// 例)
					// 4手前の局面とkey()が同じなら4手前から循環して千日手が成立。すなわち、lastMovesの後ろから5つ目の指し手で千日手局面に突入しているので
			...
			case REPETITION_INFERIOR:
				return VMD_Pair(-VALUE_SUPERIOR, MOVE_NONE, 0);
			case REPETITION_SUPERIOR:
				return VMD_Pair(VALUE_SUPERIOR, MOVE_NONE, 0);
			case REPETITION_WIN:
				return VMD_Pair(mate_in(MAX_PLY), MOVE_NONE, 0);
			case REPETITION_LOSE:
				return VMD_Pair(mated_in(MAX_PLY), MOVE_NONE, 0);
	...
	if (it_write != vmd_write_cache.end() && it_write->second.gamePly <= gamePly) return it_write->second.vmd_pair;
	...
	auto add_list = [&](Book::BookPos& bp, Color c /* このnodeのColor */, bool update_list) {
		...
		if (update_list) list->push_back(bp);
		...
		if (best[c].value < vmd.value) best[c] = vmd;
	};
	...
	for (const auto& m : MoveList<LEGAL_ALL>(pos)) {
		...
		for (auto color : COLOR) {
			...
			// 子nodeの探索結果を取り出す。
			// depthは、この先にbestMoveを辿っていくときleaf
			// nodeまで何手あるかという値なのでここで定跡が途切れるならDEPTH_ZERO。
			...
			if (value == VALUE_NONE) {
				...
				auto it =
				    std::find_if(it_read->begin(), it_read->end(), [m](const auto& x) { return x.bestMove == m.move; });
				if (it != it_read->end()) {
...
void BookTreeBuilder::extend_tree_sub(Position& pos, MemoryBook& read_book, fstream& fs, const string& sfen,
                                      bool book_hit) {
	// 千日手に到達した局面は思考対象としてはならない。
	auto draw_type = pos.is_repetition(MAX_PLY);
	if (draw_type == REPETITION_DRAW || pos.game_ply() > this->max_game_ply) return;
	// →　それ以外の反復は大きなスコアがつくはずで、除外されるはず。

	// 詰み、宣言勝ちの局面も思考対象としてはならない。
	if (pos.is_mated() || pos.DeclarationWin() != MOVE_NONE) return;
	// →　直前のnodeで大きなスコアがつくはずだから除外されるはずだが。

	// このnodeを処理済みであるか
	// →　この処理遅いかなぁ..pos.key()で十分なような気もするが…。
	string this_sfen = pos.sfen();
	if (done_sfen.find(this_sfen) != done_sfen.end()) return;
	...
	for (const auto& m : MoveList<LEGAL_ALL>(pos)) {
		...
		if (book_hit) {
			...
			if ((turn == BLACK && it->value >= black_eval_limit) || (turn == WHITE && it->value >= white_eval_limit))
...
void BookTreeBuilder::extend_tree(Position& pos, istringstream& is) {
	...
	if (enable_extend_range) cout << "extend_range = [" << extend_range1 << "," << extend_range2 << "]" << endl;
	...
	for (int i = 0; i < (int)lines.size(); ++i) {
		...
		if (std::find(sp.begin(), sp.end(), "moves") == sp.end()) line = line + " moves";
...
/*
    ...
    例)
    makebook endless_extend_tree book/book_test.db book/kadai_sfen book/think_sfen.txt depth 8 startmoves 1 moves 32
   loop 10 nodes 100000
*/
void BookTreeBuilder::endless_extend_tree(Position& pos, istringstream& is) {
	...
	for (int i = 0; i < iteration; ++i) {
...
#endif
		{
			...
			command =
			    "extend_tree " + read_book_name + " " + read_sfen_name + " " + think_sfen_name + " black_eval_limit " +
			    to_string(black_eval_limit) + " white_eval_limit " + to_string(white_eval_limit) +
			...
			command = "think " + think_sfen_name + " " + read_book_name + " depth " + to_string(depth) +
			          " startmoves " + to_string(start_moves) + " moves " + to_string(end_moves) + " nodes " +
			          to_string(nodes);
			do_command(command);
		}
	}
