...
namespace BookUtil
{
	struct ThinkEntry
	{
		ThinkEntry(const std::string & sfen_, int depth_, int multipv_) : sfen(sfen_), depth(depth_), multipv(multipv_) {}
		std::string sfen;
		int depth;
		size_t multipv;
	};
	...
	{
		MultiThinkBook(deqBook & book_)
			: book(book_), appended(false), progress_type(0) {}

		virtual void thread_worker(size_t thread_id);

		// 定跡を作るために思考する局面
		std::vector<ThinkEntry> thinks;
	...
	void MultiThinkBook::thread_worker(size_t thread_id)
	{
		...
		{
			auto thinkentry = thinks[id];

			auto th = Threads[thread_id];
			auto & pos = th->rootPos;
			pos.set(thinkentry.sfen, th);

			if (pos.is_mated())
				continue;

			// depth手読みの評価値とPV(最善応手列)
			Learner::search(pos, thinkentry.depth, thinkentry.multipv);

			// MultiPVで局面を足す、的な

			MoveListPtr move_list(new MoveListType);

			size_t multi_pv = std::min(thinkentry.multipv, th->rootMoves.size());
			for (size_t i = 0; i < multi_pv; ++i)
			{
				...
				BookPos bp(th->rootMoves[i].pv[0], nextMove, th->rootMoves[i].score
					, thinkentry.depth, int(atof(ENGINE_VERSION) * 100));
...

			{
				...
				// 前のエントリーは上書きされる。
				BookEntry be(split_sfen(thinkentry.sfen), move_list);
				book.add(be, true);
...
namespace BookUtil
{
	...
	void bookutil_cmd(Position & pos, std::istringstream & is)
	{
		...
		if (from_sfen || from_thinking)
		{
			...
			size_t multipv_black = Options["MultiPV"];
			size_t multipv_white = Options["MultiPV"];
			...
			while (true)
			{
				...
				{
					is >> depth_black;
					depth_white = depth_black;
				}
				else if (token == "depth_black")
					is >> depth_black;
				else if (token == "depth_white")
					is >> depth_white;
				else if (token == "multipv")
				{
					is >> multipv_black;
					multipv_white = multipv_black;
				}
				else if (token == "multipv_black")
					is >> multipv_black;
				else if (token == "multipv_white")
					is >> multipv_white;
			...
			if (from_thinking)
				std::cout << "read sfen moves from " << start_moves << " to " << moves
					<< " , depth = " << depth_black << "," << depth_white
					<< " , multipv = " << multipv_black << "," << multipv_white
			...
			// 思考すべき局面のsfen
			std::unordered_set<std::string> thinking_sfens[COLOR_NB] = {};
			...
			for (auto c : COLOR)
			{
				...
				for (size_t k = 0; k < sfens[c].size(); ++k)
				{
					...
					std::vector<std::pair<std::string, Color>> sf; // 初手から(moves+0)手までのsfen文字列格納用
					...
					for (int i = plyinit; i <= moves + (from_sfen ? 1 : 0); ++i)
					{
						...
						if (token == "")
						{
							...
							if (!from_sfen)
								sf.emplace_back(pos.sfen(), pos.side_to_move());
							break;
						}
					...
					for (size_t i = 0; i + (from_sfen ? 1 : 0) < sf.size(); ++i)
					{
						Color c = sf[i].second;
						if ((int)i + plyinit < start_moves || !(c == BLACK ? enable_black : enable_white))
							continue;
						...
						if (from_sfen)
						{
							...
							auto ss = split_sfen(sf[i].first);
							book.insert_book_pos(ss, bp);
						} else if (from_thinking)
						{
							// posの局面で思考させてみる。(あとでまとめて)
							if (thinking_sfens[c].count(sf[i].first) == 0)
								thinking_sfens[c].insert(sf[i].first);
						}
					}
			...
			// 入力が空でも初期局面は含めるように
			if (from_thinking && !whiteonly && start_moves == 1 && thinking_sfens[BLACK].empty() && thinking_sfens[WHITE].empty())
			{
				thinking_sfens[BLACK].insert(SFEN_HIRATE);
			}
			std::cout << "done." << std::endl;
			...
			if (from_thinking)
			{
				...
				MultiThinkBook multi_think(book);

				// 進捗表示形式
				multi_think.progress_type = progress_type;

				auto & thinks = multi_think.thinks;
				for (Color c : { BLACK, WHITE })
				{
					int _depth = (c == BLACK ? depth_black : depth_white);
					size_t _multipv = (c == BLACK ? multipv_black : multipv_white);
					for (auto & s : thinking_sfens[c])
					{
						...
						if (it == book.end())
							thinks.emplace_back(s, _depth, _multipv);
						else
						{
							const auto& mlp = it->move_list;
							if (mlp->front().depth < _depth || // 今回の探索depthのほうが深い
								(
									mlp->front().depth == _depth && mlp->size() < _multipv && // 探索深さは同じだが今回のMultiPVのほうが大きい
									mlp->size() < MoveList<LEGAL>((pos_.set(s, Threads.main()), pos_)).size() // かつ、合法手の数のほうが大きい
								)
							)
								thinks.emplace_back(s, _depth, _multipv);
						}
				...
				// 思考対象node数の出力。
				std::cout << "total " << thinks.size() << " nodes " << std::endl;

				// クラスターの指定に従い、間引く。
				if (cluster_id != 1 || cluster_num != 1)
				{
					std::vector<ThinkEntry> a;
					for (int i = 0; i < (int)thinks.size(); ++i)
					{
						if ((i % cluster_num) == cluster_id - 1)
							a.push_back(thinks[i]);
					}
					thinks = a;

					// このPCに割り振られたnode数を表示する。
					std::cout << "for my PC : " << thinks.size() << " nodes " << std::endl;
				}

				multi_think.set_loop_max(thinks.size());
		...
		else if (kifexport)
		{
			...
			while (true)
			{
				...
				else if (token == "evaldiff")
				{
					...
					is >> evalwhitediff;
				else if (token == "evaldiff_black")
					is >> evalblackdiff;
				else if (token == "evaldiff_white")
					is >> evalwhitediff;
				else if (token == "evallimit")
				{
					...
					is >> evalwhitelimit;
				else if (token == "evallimit_black")
					is >> evalblacklimit;
				else if (token == "evallimit_white")
					is >> evalwhitelimit;
			...
			std::cout << "export "
				...
				<< ":"
				<< " moves " << moves
				<< " evaldiff " << evalblackdiff << "," << evalwhitediff
				<< " evallimit " << evalblacklimit << "," << evalwhitelimit
		...
		else if (from_apery)
		{
			...
			{
				const auto& mlp = apery_book.get_entries(pos);
				if (mlp.get() == nullptr || mlp->empty()) { return BOOKRES_UNFILLED; }
				BookEntry be(pos, mlp);
	...
	MoveListPtr OnTheFlyBook::get_entries(const Position & pos)
	{
		if (!on_the_fly)
			return {};
