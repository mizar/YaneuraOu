...
namespace Book
{
	...
	int OnTheFlyBook::read_book(const string & filename)
	{
		if (fs.is_open())
			fs.close();
	...
	int deqBook::read_book(const string & filename)
	{
		return read_book(filename, false);
	}
	int deqBook::read_book(const string & filename, bool sfen_n11n)
	{
		// 読み込み済であるかの判定
	...
	// 定跡ファイルの書き出し
	int deqBook::write_book(const string & filename)
	{
		// 全体のソートと重複局面の除去
		intl_uniq();
		...
		// バッファ領域サイズ
		size_t renderbufsize = entrymaxsize * min(renderbufmulti, book_body.size() + 1);
		// バッファ払い出し閾値
