...
void init() {
	...
	for (auto sq : SQ) board_mask_table[sq] = around8(kingEffect(sq), sq);
...
Directions around8(const Bitboard& b, Square sq) {
	...
	auto t = uint32_t((sq < SQ_32) ? (b.p[0] << int(SQ_32 - sq))
	                               : ((b.p[0] >> int(sq - SQ_32)) |
	                                  (b.p[1] << int(SQ_92 + SQ_L - sq))));  // p[1]のSQ_92の左の升は、p[0]のSQ_32相当。
...
void init() {
	...
	for (auto sq : SQ) board_mask_table[sq] = around24(around24_bb(sq), sq);
	...
	// -- ymm_direct_to_around8[]の初期化
	{
		...
		for (auto sq : SQ) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (!is_ok(sqww)) continue;
				if (sq33_around9 & sqww_to_sq(sqww)) d |= Effect8::to_directions(dir);
			}
			d81[sq] = d;
		}
		for (int i = 0; i < 3; ++i) ymm_direct_to_around8[i] = ymm(&d81[i * 16]);  // 32byteずつ
	}
}
	...
	for (auto r : Rank()) {
		for (File f = FILE_9; f >= FILE_1; --f) {
			...
			if (e < 16)
				...
				os << "[" << e
				   << "]";  // プログラムのバグで-1(255)などになっているのだろうが、その値が表示されていて欲しい。
		}
		os << std::endl;
	}
	return os;
...
void calc_effect(Position& pos) {
	...
	for (auto sq : pos.pieces()) {
		...
		Color c      = color_of(pc);
		for (auto to : effect) ADD_BOARD_EFFECT(c, to, 1);
		if (has_long_effect(pc)) {
			// ただし、馬・龍に対しては、長い利きは、角・飛車と同じ方向だけなので成り属性を消して、利きを求め直す。
			if (type_of(pc) != LANCE) effect = effects_from(Piece(pc & ~PIECE_PROMOTE), sq, pos.pieces());
...
inline Bitboard short_effects_from(Piece pc, Square sq) {
	switch (pc) {
		...
		case B_KNIGHT:
			return knightEffect(BLACK, sq);
		case W_KNIGHT:
			return knightEffect(WHITE, sq);
...
// これは
// 1)
// toの地点にUsの駒を移動させるなら、toの地点で発生する利き(dir_bw_us)以外は、遮断された利きであるから、このray上の利きは減るべき。
// 2)
// toの地点からUsの駒を移動させるなら、toの地点から取り除かれる利き(dir_bw_us)以外は、遮断されていたものが回復する利きであるから、このray上の利きは増えるべき。
...
#define UPDATE_LONG_EFFECT_FROM(to, dir_bw_us, dir_bw_others, p) \
	{ UPDATE_LONG_EFFECT_FROM_(ADD_BOARD_EFFECT_BOTH, to, dir_bw_us, dir_bw_others, p); }

// undo_move()で巻き戻すときに使う用。(利きの更新関数が違う)
#define UPDATE_LONG_EFFECT_FROM_REWIND(to, dir_bw_us, dir_bw_others, p) \
	{ UPDATE_LONG_EFFECT_FROM_(ADD_BOARD_EFFECT_BOTH_REWIND, to, dir_bw_us, dir_bw_others, p); }

// Usの手番で駒pcをtoに配置したときの盤面の利きの更新
template <Color Us>
void update_by_dropping_piece(Position& pos, Square to, Piece dropped_pc) {
	auto& board_effect = pos.board_effect;
	...
	// trick a)
	// (右側から)左方向への長い利きがあり、toの地点でそれを遮ったとして、しかしtoの地点に持って行った駒が飛車であれば、
	// この左方向の長い利きは遮ったことにはならならず、左方向への長い利きの更新処理は不要である。
	// このようにtoの地点の升での現在の長い利きと、toの地点に持って行った駒から発生する長い利きとをxorすることで
	// この利きの相殺処理がうまく行える。

	// trick b)
	// (右側から)左方向への後手の長い利きをtoの升で遮断し、toの升に持って行った駒が飛車で左方向の長い利きが発生した場合、
...
template <Color Us>
void update_by_capturing_piece(Position& pos, Square from, Square to, Piece moved_pc, Piece moved_after_pc,
                               Piece captured_pc) {
	auto& board_effect = pos.board_effect;
	auto& long_effect  = pos.long_effect;
...
template <Color Us>
void update_by_no_capturing_piece(Position& pos, Square from, Square to, Piece moved_pc, Piece moved_after_pc) {
	auto& board_effect = pos.board_effect;
	auto& long_effect  = pos.long_effect;
...
template <Color Us>
void rewind_by_dropping_piece(Position& pos, Square to, Piece dropped_pc) {
	auto& board_effect = pos.board_effect;
...
template <Color Us>
void rewind_by_capturing_piece(Position& pos, Square from, Square to, Piece moved_pc, Piece moved_after_pc,
                               Piece captured_pc) {
	auto& board_effect = pos.board_effect;
	auto& long_effect  = pos.long_effect;
	...
	while (inc_target) {
		auto sq = inc_target.pop();
		ADD_BOARD_EFFECT_REWIND(Us, sq, +1);
	}
	while (dec_target) {
		auto sq = dec_target.pop();
		ADD_BOARD_EFFECT_REWIND(Us, sq, -1);
...
template <Color Us>
void rewind_by_no_capturing_piece(Position& pos, Square from, Square to, Piece moved_pc, Piece moved_after_pc) {
	auto& board_effect = pos.board_effect;
	auto& long_effect  = pos.long_effect;
...
template void update_by_capturing_piece<BLACK>(Position& pos, Square from, Square to, Piece moved_pc,
                                               Piece moved_after_pc, Piece captured_pc);
template void update_by_capturing_piece<WHITE>(Position& pos, Square from, Square to, Piece moved_pc,
                                               Piece moved_after_pc, Piece captured_pc);
template void update_by_no_capturing_piece<BLACK>(Position& pos, Square from, Square to, Piece moved_pc,
                                                  Piece moved_after_pc);
template void update_by_no_capturing_piece<WHITE>(Position& pos, Square from, Square to, Piece moved_pc,
                                                  Piece moved_after_pc);
...
template void rewind_by_capturing_piece<BLACK>(Position& pos, Square from, Square to, Piece moved_pc,
                                               Piece moved_after_pc, Piece captured_pc);
template void rewind_by_capturing_piece<WHITE>(Position& pos, Square from, Square to, Piece moved_pc,
                                               Piece moved_after_pc, Piece captured_pc);
template void rewind_by_no_capturing_piece<BLACK>(Position& pos, Square from, Square to, Piece moved_pc,
                                                  Piece moved_after_pc);
template void rewind_by_no_capturing_piece<WHITE>(Position& pos, Square from, Square to, Piece moved_pc,
                                                  Piece moved_after_pc);
