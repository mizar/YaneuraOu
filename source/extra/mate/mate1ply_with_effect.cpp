...
// 駒移動の場合 =>
// 移動させることによって移動元から利きが損なわれなく、かつ、移動先の升で大駒の利きが遮断されなければこれで詰む
	...
	if (hk & HAND_KIND_##DROP_PIECE) {                                                                        \
		...
		while (directions) {                                                                                  \
			...
			while (cut_dirs_from_king) {                                                                      \
				...
				if (board_effect[Us].e[to2] <= 1) goto Next##DROP_PIECE;                                      \
			}                                                                                                 \
...
Move Position::mate1ply_impl() const {
	...
	if (!(mi.directions | mi.hand_kind)) return MOVE_NONE;
...
MOVE_MATE:
		...
		while (drop_target) {
			...
			if (!board_effect[them].effect(to)) {
				...
				if ((ourHand & HAND_KIND_KNIGHT) && !(pieces() & to)) {
					...
					// 遮断していないならこれにて詰み
					if (!cut_directions) return make_move_drop(KNIGHT, to);
					...
					if (!dec_effect) return make_move_drop(KNIGHT, to);
				...
				while (froms) {
					...
					if (!(pinned & from)) {
						auto cut_directions = long_effect.le16[to].dirs[Us];
						// 遮断していないならこれにて詰み
						if (!cut_directions) return make_move(from, to);

						// 敵玉から見てここの利きを損失するのでここの利きが2以上でないといけない
						auto direct     = direct_knight_of<Us>(themKing, to);
						auto dec_effect = cutoff_directions(direct, cut_directions);
						// 敵玉の8近傍に影響を及ぼす遮断された利きがないのでこれにて詰み
						if (!dec_effect) return make_move(from, to);
	...
	auto is_mated_by_from_to = [&](Square from, Square to, Direct to_direct) {
		...
		// 9) fromの升で、fromからtoの方向の敵の長い利きがないこと。(back
		// attackによりtoに移動させたときに取られてしまう) 10) toの地点において大駒の利きが遮断されず 11)
		// 移動元から利きが消失しても敵玉8近傍の攻め方の利きが存在すること。
		...
		{
			if (type_of(pc) == KING) goto NextCandidate;  // 14)
			...
			auto effect_us_not = (type_of(pro_pc) == DRAGON && is_diag(to_direct))
			                         ? ~around8(dragonEffect(to, pieces()), themKing)
			                         : piece_effect_not(pro_pc, to_direct);  // 12)
			...
			auto cut_dirs =
			    cutoff_directions(to_direct, long_effect.directions_of(Us, to) & ~LongEffect::long_effect_of(pc));

			// 上の2つの条件の升で、toにその駒を移動させたときに利きがない升に対して調べる。
			// toの地点はすでに条件を満たしているので調べない。(影の利きで王手する場合などがあるので調べてはならない)
			auto dec =
			    (dec_around8 | cut_dirs) & effect_us_not & a8_board_mask & a8_them_movable & ~to_directions(to_direct);
	...
	NonProCheck:;

		{
			...
			if (pt == PAWN || pt == BISHOP || pt == ROOK) goto NextCandidate;

			// 香で2段目での成で詰まないなら不成で詰むことはない。(1手詰めにおいては)
			if (pt == LANCE && (rank_of(to) == ((Us == BLACK) ? RANK_2 : RANK_8))) goto NextCandidate;
		}
	...
	NonProCheck2:;
...

		{
			...
			if (discovered(from, to, ourKing, pinned)) goto NextCandidate;

			if (!(piece_check_around8(pc) & (1 << to_direct))) goto NextCandidate;

			auto effect_us_not = (type_of(pc) == DRAGON && is_diag(to_direct))
			                         ? ~around8(dragonEffect(to, pieces()), themKing)
			                         : piece_effect_not(pc, to_direct);

			if (effect_us_not & info2) goto NextCandidate;

			if (directions_of(from, to) & long_effect.le16[from].dirs[~Us]) goto NextCandidate;
			...
			while (dec) {
				...
				uint8_t dec_num    = ((dec_around8 >> cut_direct) & 1) + ((cut_dirs >> cut_direct) & 1);
				Square  to2        = themKing + DirectToDelta(cut_direct);
				if (board_effect[Us].effect(to2) <= dec_num) goto NextCandidate;
			}
			return make_move(from, to);
		}
	NextCandidate:;
	...
	Bitboard   froms;
	Directions to_candicate =
	    a8_effect_us_gt1 & a8_board_mask & a8_us_movable & mi.directions & ~a8_effect_them;  // 1) & 2) & 3) & 4) & 5)
	...
	while (froms24) {
		...
		while (target) {
			...
			if (!(directions_of(from, to) & long_effect.directions_of(Us, from))) continue;

			// c) toの升が利きが1つでなければNG。(2つ以上あるところはチェック済み)
			// d) toの升に(玉を除く)敵の利きがあってはならない。
			if (board_effect[Us].effect(to) != 1 || board_effect[them].effect(to) != 1) continue;
...
Move Position::mate1ply() const { return sideToMove == BLACK ? mate1ply_impl<BLACK>() : mate1ply_impl<WHITE>(); }
...
void init() {
	for (auto c : COLOR)
		for (uint32_t info = 0; info < 0x10000; ++info) {
			...
			//  bit  8..15 : (2) 王が移動可能な升(攻め方の利きがなく、受け方の駒もない)
			//  壁は0(玉はそこに移動できないので)
			...
			for (int i = 0; i < 8; ++i) {
				...
				for (auto pt : drop_pieces) {
					...
					if (!droppable && pt != QUEEN) continue;
				...
				// 王に行き場がないなら桂で詰む
				if (!info2) hk |= HandKind(1 << (KNIGHT - 1));
			}
