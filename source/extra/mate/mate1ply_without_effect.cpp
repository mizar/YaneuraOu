...
inline Bitboard check_cand_bb(Color us, PieceTypeCheck pc, Square sq_king) { return CHECK_CAND_BB[sq_king][pc][us]; }

// 敵玉8近傍の利きに関係する自駒の候補のbitboardを返す。ここになければ玉周辺に利きをつけない。
// pt = PAWN～HDK
inline Bitboard check_around_bb(Color us, PieceType pt, Square sq_king) { return CHECK_AROUND_BB[sq_king][pt - 1][us]; }
				...
				switch ((int)p) {
					...
					case PIECE_TYPE_CHECK_LANCE:
						...
						if (enemy_field(c) ^ sq) {
							...
							if (file_of(sq) != FILE_1) bb |= lanceStepEffect(~c, sq + SQ_R);
							if (file_of(sq) != FILE_9) bb |= lanceStepEffect(~c, sq + SQ_L);
						}
					...
					case PIECE_TYPE_CHECK_SILVER:
						...
						// 玉のひとつ下の升の2つとなりの升
						{
							...
							if (r == rank_of(sq)) {
								...
								// 2升隣。
								if (file_of(to) >= FILE_3) bb |= (to + SQ_R * 2);
								if (file_of(to) <= FILE_7) bb |= (to + SQ_L * 2);
							}

							// 5段目の玉に成りでのバックアタック的な..
							if (rank_of(sq) == RANK_5) bb |= knightEffect(c, sq);
						}
						break;
			...
			for (auto c : COLOR) {
				...
				switch (p) {
					...
					case LANCE:
						...
						bb = lanceStepEffect(~c, sq);
						if (file_of(sq) != FILE_1) bb |= lanceStepEffect(~c, sq + SQ_R) | (sq + SQ_R);
						if (file_of(sq) != FILE_9) bb |= lanceStepEffect(~c, sq + SQ_L) | (sq + SQ_L);
						break;
	...
	for (auto s1 : SQ)
		for (auto s2 : SQ) {
			...
			if (queenStepEffect(s1) & s2) {
				...
				// 盤面の範囲外に出ていないかのテスト
				if (is_ok(s3f) && is_ok(s3r)) next_sq = s3f | s3r;
			}
...
bool can_king_escape(const Position& pos, Color us, Square to, const Bitboard& bb_avoid, const Bitboard& slide_) {
	...
	*/
	...
	while (bb) {
		...
		if (!pos.attackers_to(~us, escape, slide)) return true;
		// 何も破壊していないので即座に返って良い。
	}
	return false;
}
...
bool can_king_escape(const Position& pos, Color us, Square from, Square to, const Bitboard& bb_avoid,
                     const Bitboard& slide_) {
	// toには駒が置かれているのでこれにより利きの遮断は発生している。(attackers_to()で利きを見るときに重要)
	Bitboard slide = slide_ | to;
...
// またfromからは駒が除去されているものとして考える。
// ただしtoには行けるものとする。
bool can_king_escape_cangoto(const Position& pos, Color us, Square from, Square to, const Bitboard& bb_avoid,
                             const Bitboard& slide_) {
	// toには駒が置かれているのでこれにより利きの遮断は発生している。(attackers_to()で利きを見るときに重要)
	Bitboard slide = slide_ | to;
...
bool can_piece_capture(const Position& pos, Color us, Square to, const Bitboard& pinned, const Bitboard& slide) {
	...
	while (sum) {
		...
		if (!pinned || !(pinned & from) || aligned(from, to, sq_king)) return true;
	}
...
bool can_piece_capture(const Position& pos, Color us, Square to, Square avoid, const Bitboard& pinned,
...
Move is_mate_in_1ply_imp(const Position& pos) {
	...
	if (hand_count(ourHand, ROOK)) {
		...
		while (bb) {
			to = bb.pop();
			// toに対して自駒が利いてないと意味ない
			if (!pos.attackers_to(Us, to)) continue;

			// このtoに飛車を打つものとして…この十字方向には逃げられないわけだから…そこは駄目ですよ、と。
			bb_attacks = rookStepEffect(to);

			if (can_king_escape(pos, them, to, bb_attacks, pos.pieces())) {
				continue;
			}
			if (can_piece_capture(pos, them, to, pinned, pos.pieces())) {
				continue;
			}
	...
	if (hand_count(ourHand, GOLD)) {
		...
		if (hand_count(ourHand, ROOK)) bb &= ~pawnEffect(Us, sq_king);
	...
	if (hand_count(ourHand, SILVER)) {
		...
		if (hand_count(ourHand, GOLD)) {
			...
			if (hand_count(ourHand, BISHOP)) goto SILVER_DROP_END;
...
SILVER_DROP_END:;
	...
	if (hand_count(ourHand, KNIGHT)) {
		...
		while (bb) {
			...
			if (can_king_escape(pos, them, to, ZERO_BB, pos.pieces())) {
				continue;
			}
			if (can_piece_capture(pos, them, to, pinned, pos.pieces())) {
				continue;
	...
	while (bb) {
		...
		while (bb_check) {
			...
			if (!(pos.attackers_to(Us, to, slide) ^ from)) {
				continue;
			}
			...
			if (can_king_escape(pos, them, from, to, bb_attacks, slide)) {
				continue;
			}
	...
	while (bb) {
		...
		while (bb_check) {
			...
			if (pos.discovered(from, to, pos.king_square(Us), our_pinned)) {
				continue;
			}
			if (can_king_escape(pos, them, from, to, bb_attacks, slide)) {
				continue;
	...
	if (hand_count(themHand, PAWN) == (int)themHand) {
		...
		while (bb2) {
			...
			if (type_of(pos.piece_on(to)) == PAWN && file_of(to) == file_of(one) && hand_count(themHand, PAWN) >= 1)
				continue;
			...
			if (dr & DIRECTIONS_DIAG) {
				...
				// 斜めなら角を持ってなきゃ
				if (hand_count(ourHand, BISHOP) == 0) goto NEXT2;
			} else {
				pt = ROOK;
			...
			{
				if (!(~bishopStepEffect(to) & escape_bb)) return make_move_drop(pt, to);
			} else  // if (pt == ROOK || pt==LANCE)
			{
				// LANCEの場合もtoの地点からの横の利きでは玉の8近傍に到達しないので同列に扱って良い。
				if (!(~rookStepEffect(to) & escape_bb)) return make_move_drop(pt, to);
			}
			...
			if (esc_count <= 2) {
				...
				if (dr & DIRECTIONS_DIAG)  // pt == BISHOP
				{
					if (!(~bishopStepEffect(nextTo) & escape_bb)) return make_move_drop(pt, nextTo);
				} else  // if (pt == ROOK || pt==LANCE)
				{
					if (!(~rookStepEffect(nextTo) & escape_bb)) return make_move_drop(pt, nextTo);
				}
			}
		...
		NEXT2:;
			...
			if (!(dr & DIRECTIONS_DIAG))  // (pt == ROOK || pt == LANCE)
			{
				...
				if (is_rook || is_dragon || is_lance) {
					// 落ちてるっぽい。移動可能かどうか調べる。
					bb = ZERO_BB;
					if (is_rook) bb = rookEffect(to, pos.pieces()) & pos.pieces(Us, ROOK_DRAGON);
					if (is_dragon) bb |= kingEffect(to) & pos.pieces(Us, DRAGON);
					if (is_lance) bb |= lanceEffect(them, to, pos.pieces()) & pos.pieces(Us, LANCE);
					...
					while (bb) {
						...
						if (!(kingEffect(sq_king) &
						      ~(pos.pieces(them) | AttacksAroundKingInAvoiding<~Us>(pos, from, new_slide) |
						        bb_attacks))) {
							// これで詰みが確定した
							// 香は不成りでの王手
							if (type_of(pos.piece_on(from)) != LANCE && canPromote(Us, from, to) &&
							    !(pos.piece_on(from) & PIECE_PROMOTE))
								return make_move_promote(from, to);
							else
								return make_move(from, to);
						}
				...
				if (is_bishop || is_horse) {
					// 落ちてるっぽい。移動可能かどうか調べる。
					bb = ZERO_BB;
					if (is_bishop) bb = bishopEffect(to, pos.pieces()) & pos.pieces(Us, BISHOP_HORSE);
					if (is_horse) bb |= kingEffect(to) & pos.pieces(Us, HORSE);
...
NEXT1:;
	...
	bb = check_cand_bb(Us, PIECE_TYPE_CHECK_NON_SLIDER, sq_king) & (pos.pieces(Us, GOLDS, SILVER, KNIGHT, PAWN));
	if (!bb) goto DC_CHECK;
	...
	if (check_cand_bb(Us, PIECE_TYPE_CHECK_PAWN_WITH_NO_PRO, sq_king) & pos.pieces(Us, PAWN)) {
		...
		if (pos.piece_on(to) != NO_PIECE && color_of(pos.piece_on(to)) != ~Us) {
			goto SKIP_PAWN;
		}
		...
		if (can_king_escape(pos, them, from, to, ZERO_BB, slide)) {
			goto SKIP_PAWN;
		}
...
DC_CHECK:;
		...
		while (bb) {
			...
			switch (pt) {
				...
				case PAWN: {
					...
					if (file_of(from) == file_of(sq_king)) {
						continue;
					}
			...
			while (bb) {
				...
				// この地点でのこの駒の利きは..
				// bb_attacks = pos.attacks_from(make_piece(Us, pt), to, pos.pieces() ^ sq_king); //
				// sq_kingが除去されて貫通である必要がある。
				...
				switch (pt) {
					...
					case BISHOP:
						bb_attacks = bishopStepEffect(to);
						if (promo) bb_attacks |= kingEffect(to);
						break;
					case HORSE:
						bb_attacks = bishopStepEffect(to) | kingEffect(to);
						break;
				...
				if (!can_king_escape_cangoto(pos, them, from, to, bb_attacks, slide)) {
					if (promo && !(pt & PIECE_PROMOTE) && pt != GOLD) return make_move_promote(from, to);
					return make_move(from, to);
				}
	...
	if (dcCandidates && hand_count(themHand, PAWN) == (int)themHand) {
		...
		while (bb) {
			...
			if (atk) {
				if (atk.pop_count() >= 2) continue;  // 2つ以上利きがあるなら消せないわ
			...
			switch ((int)pt)  // intにcastしとかないとhandleしてない値に対して警告がでる。
			{
				...
				case KNIGHT:
					// 成って詰みはあるか..それだけ見るか..
					if (!((Us == BLACK ? RANK3_5BB : RANK5_7BB) & from)) continue;
			...
			while (bb2) {
				...
				if (can_piece_capture(pos, them, from, to, new_pinned, new_slide)) continue;

				// 玉の8近傍だとcapられる可能性がある。
				if (kingEffect(sq_king) & to) {
					// from以外の駒が利いてない == capられる!!
					if (!(pos.attackers_to(Us, to) ^ from)) continue;
				}
				...
				switch (pt) {
					case SILVER:
						...
						// 成れないならば不成りの判定へ
						if (!canPromote(Us, from, to)) goto DISCOVER_ATTACK_CONTINUE_SILVER;
						bb_attacks = goldEffect(Us, to);
						break;
				...
				if (bb_attacks & sq_king) goto DISCOVER_ATTACK_CONTINUE_SILVER;
			...
			DISCOVER_ATTACK_CONTINUE_SILVER:;
				...
				if (pt == SILVER) {
					...
					// 以下、同様の処理
					bb_attacks = silverEffect(Us, to);
					if (bb_attacks & sq_king) goto DISCOVER_ATTACK_CONTINUE;

					if (kingEffect(sq_king) &
					    ~(bb_attacks | AttacksAroundKingInAvoiding<~Us>(pos, from, new_slide ^ sq_king) |
					      pos.pieces(them)))
					...
					Square s2 = s1 + (nextSquare(s1, from) - from);
					do {
						if (can_piece_capture(pos, them, s2, to, new_pinned, new_slide)) goto DISCOVER_ATTACK_CONTINUE;
						if (s2 != from && pos.piece_on(s2)) break;
						if (can_pawn_drop<~Us>(pos, s2) || (capPawn && file_of(s2) == file_of(to)))
							goto DISCOVER_ATTACK_CONTINUE;
						Square s3 = nextSquare(s1, s2);
						s1        = s2;
