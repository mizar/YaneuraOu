...
bool can_piece_capture(const Position& pos, Color us, Square to, const Bitboard& pinned, const Bitboard& slide) {
	...
	while (sum) {
		...
		if (!pinned || !(pinned & from) || aligned(from, to, sq_king))
			return true;
	}
...
SILVER_DROP_END:;
	...
	if (hand_count(themHand, PAWN) == (int)themHand) {
		...
		while (bb2) {
			...
			// oneに駒の移動合いができない
			if (can_piece_capture(pos, them, one, pinned, pos.pieces()))
				goto NEXT2;
			...
			if (esc_count <= 2) {
				...
				if (nextTo == SQ_NB)
					goto NEXT2;
				if (pos.piece_on(nextTo))
					goto NEXT2;
				if (can_pawn_drop<~Us>(pos, to))
					goto NEXT2;
				if (can_piece_capture(pos, them, nextTo, pinned, pos.pieces()))
					goto NEXT2;
			...
			if (!(dr & DIRECTIONS_DIAG))  // (pt == ROOK || pt == LANCE)
			{
				...
				if (is_rook || is_dragon || is_lance) {
					...
					while (bb) {
						...
						// (将棋にはQUEENがないので)
						// ゆえに古いpinで考えておけばいいや。
						if (can_piece_capture(pos, them, one, to, pinned, slide))
							continue;
	...
	if (dcCandidates) {
		...
		while (bb) {
			...
			switch (pt) {
				...
				case KNIGHT:
					if (!(check_around_bb(Us, KNIGHT, sq_king) & from))
						continue;
				...
				case SILVER:
					// 王手になる見込みがない
					if (!(check_around_bb(Us, SILVER, sq_king) & from))
						continue;
				...
				case GOLD:
					// 王手になる見込みがない
					if (!(check_around_bb(Us, GOLD, sq_king) & from))
						continue;
