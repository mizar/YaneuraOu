...
struct alignas(32) ymm {
	...
	ymm(const uint8_t t) {
		for (int i = 0; i < 32; ++i) ((u8*)this)[i] = t;
	}
...
struct ymm {
	...
	uint32_t to_uint32() const {
		uint32_t r = 0;
		for (int i = 0; i < 32; ++i) r |= uint32_t(m8[i] >> 7) << i;
		return r;
	}
	...
	ymm cmp(const ymm& rhs) const {
		ymm t;
		for (int i = 0; i < 32; ++i) t.m8[i] = ((int8_t)m8[i] > (int8_t)rhs.m8[i]) ? 0xff : 0;
		return t;
	}
	...
	ymm eq(const ymm& rhs) const {
		ymm t;
		for (int i = 0; i < 32; ++i) t.m8[i] = (m8[i] == rhs.m8[i]) ? 0xff : 0;
		return t;
	}
};
