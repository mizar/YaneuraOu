...
std::string pv(const Position& pos, Depth depth, Value alpha, Value beta) {
	...
	for (size_t i = 0; i < multiPV; ++i) {
		...
		if (v == -VALUE_INFINITE) v = VALUE_ZERO;  // この場合でもとりあえず出力は行う。
		...
		ss << "info"
		   << " depth " << d << " seldepth " << rootMoves[i].selDepth << " score " << USI::value(v);

		// これが現在探索中の指し手であるなら、それがlowerboundかupperboundかは表示させる
		if (i == pvIdx) ss << (v >= beta ? " lowerbound" : v <= alpha ? " upperbound" : "");
		...
		if (elapsed > 1000) ss << " hashfull " << TT.hashfull();
		...
		auto out_array_pv = [&]() {
			for (Move m : rootMoves[i].pv) ss << " " << m;
		};
		...
		auto out_tt_pv = [&]() {
			...
			while (ply < MAX_PLY) {
				...
				if (m == MOVE_WIN) {
					...
					if (pos.DeclarationWin() != MOVE_NONE) ss << " " << MOVE_WIN;
			...
			while (ply > 0) pos_->undo_move(moves[--ply]);
		};
...
void is_ready(bool skipCorruptCheck) {
	...
	// スレッド起動待ち
	while (!thread_started) Tools::sleep(100);
...
void getoption_cmd(istringstream& is) {
	...
	if (!all) sync_cout << "No such option: " << name << sync_endl;
}
...
void go_cmd(const Position& pos, istringstream& is, StateListPtr& states) {
	...
	int max_game_ply = 0;
	if (Options.count("MaxMovesToDraw")) max_game_ply = (int)Options["MaxMovesToDraw"];
	...
	while (is >> token) {
		...
		if (token == "searchmoves")
			...
			while (is >> token) limits.searchmoves.push_back(USI::to_move(pos, token));
...
void USI::loop(int argc, char* argv[]) {
	...
	do {
		...
		if (token == "quit" || token == "stop" || token == "gameover") {
			...
			if (token == "gameover") gameover_handler(cmd);
#endif
		...
		// 指し手生成祭りの局面をセットする。
			...
			pos.set("l6nl/5+P1gk/2np1S3/p1p4Pp/3P2Sp1/1PPb2P1P/P5GS1/R8/LN4bKL w GR5pnsg 1", &states->back(),
			        Threads.main());
		...
		// この局面の手番側がどちらであるかを返す。BLACK or WHITE
			...
			cout << (pos.side_to_move() == BLACK ? "black" : "white") << endl;
...
#if defined(MATE_ENGINE)
		else if (token == "test_mate_engine")
			test_mate_engine_cmd(pos, is);
#endif
#endif
		...
		// 定跡を作るコマンド
			...
			Learner::gen_sfen(pos, is);
		else if (token == "learn")
			Learner::learn(pos, is);

#if defined(GENSFEN2019)
		// 開発中の教師局面生成コマンド
		else if (token == "gensfen2019")
			Learner::gen_sfen2019(pos, is);
#endif
...
Square usi_to_sq(char f, char r) {
	...
	if (is_ok(file) && is_ok(rank)) return file | rank;
...
// Square型をUSI文字列に変換する
std::string USI::square(Square s) { return std::string{char('a' + file_of(s)), char('1' + rank_of(s))}; }
...
Move USI::to_move(const Position& pos, const std::string& str) {
	...
	if (str == "win") return MOVE_WIN;

	// パス(null move)入力への対応 {UCI: "0000", GPSfish: "pass"}
	if (str == "0000" || str == "null" || str == "pass") return MOVE_NULL;
	...
	// 合法手かどうかのチェックを入れる。
	if (!MoveList<LEGAL>(pos).contains(move)) sync_cout << "info string Error!! Illegal Move = " << move << sync_endl;
#endif

	if (pos.pseudo_legal(move) && pos.legal(move)) return move;
...
Move16 USI::to_move16(const string& str) {
...

	{
		...
		if (str.length() <= 3) goto END;

		Square to = usi_to_sq(str[2], str[3]);
		if (!is_ok(to)) goto END;
		...
		if (!drop) {
			...
			if (is_ok(from)) move = promote ? make_move_promote(from, to) : make_move(from, to);
		} else {
			for (int i = 1; i <= 7; ++i)
