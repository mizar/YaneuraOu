...
void to_kif2_c32(char32_t ** r, Move m, Position& pos, Move prev_m, SquareFormat fmt)
{
	...
	*s++ = ((~c) ? U'▲' : U'△');
	...
	else
	{
		...
		Bitboard fromB = SquareBB[fromSq], toB = SquareBB[toSq];
		...
		if (!is_drop(m)) {
			if (atkB_pop > 1) {
				if (fromSqR == toSqR) {
					if ((atkB & RANK_BB[toSqR]).pop_count() == 1)
						*s++ = U'寄';
					else if (fromSqF < toSqF) {
						*s++ = ((~c) ? U'左' : U'右');
						if ((atkB & InLeftBB[0][toSqF]).pop_count() != 1)
							*s++ = U'寄';
					}
					else if (fromSqF > toSqF) {
						*s++ = ((~c) ? U'左' : U'右');
						if ((atkB & InRightBB[0][toSqF]).pop_count() != 1)
							*s++ = U'寄';
					}
				}
				else if (fromSqR < toSqR) {
					if ((atkB & InFrontBB[0][toSqR]).pop_count() == 1)
						*s++ = ((~c) ? U'引' : U'上');
					else if (fromSqF == toSqF) {
						if (c && is_like_goldsilver)
							*s++ = U'直';
						else if (atkB_pop - (atkB & InLeftBB[0][fromSqF]).pop_count() == 1)
							*s++ = ((~c) ? U'右' : U'左');
						else if (atkB_pop - (atkB & InRightBB[0][fromSqF]).pop_count() == 1)
							*s++ = ((~c) ? U'左' : U'右');
						else if ((atkB & InLeftBB[0][toSqF] & InFrontBB[0][fromSqR]) == ZERO_BB) {
							*s++ = ((~c) ? U'左' : U'右');
							*s++ = ((~c) ? U'引' : U'上');
						}
						else if ((atkB & InRightBB[0][toSqF] & InFrontBB[0][fromSqR]) == ZERO_BB) {
							*s++ = ((~c) ? U'右' : U'左');
							*s++ = ((~c) ? U'引' : U'上');
						}
					}
						...
						*s++ = ((~c) ? U'右' : U'左');
						if ((atkB & OrRightBB[0][fromSqF]).pop_count() != 1)
							*s++ = ((~c) ? U'引' : U'上');
					}
					else if (fromSqF > toSqF) {
						*s++ = ((~c) ? U'左' : U'右');
						if ((atkB & OrLeftBB[0][fromSqF]).pop_count() != 1)
							*s++ = ((~c) ? U'引' : U'上');
					}
				}
				else if (fromSqR > toSqR) {
					if ((atkB & InBackBB[0][toSqR]).pop_count() == 1)
						*s++ = ((~c) ? U'上' : U'引');
					else if (fromSqF == toSqF) {
						if (~c && is_like_goldsilver)
							*s++ = U'直';
							...
							*s++ = ((~c) ? U'左' : U'右');
						else if ((atkB & InLeftBB[0][fromSqF] & InBackBB[0][toSqR]) == ZERO_BB) {
							*s++ = ((~c) ? U'左' : U'右');
							*s++ = ((~c) ? U'上' : U'引');
						}
						else if ((atkB & InRightBB[0][fromSqF] & InBackBB[0][toSqR]) == ZERO_BB) {
							*s++ = ((~c) ? U'右' : U'左');
							*s++ = ((~c) ? U'上' : U'引');
						}
					}
