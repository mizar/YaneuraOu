...
Value qsearch(Position& pos, Value alpha, Value beta, Depth depth) {
	...
	if (InCheck) {
		...
		if (depth < -3 * ONE_PLY) return alpha;
	}
	...
	if (InCheck && alpha == -VALUE_INFINITE) return mated_in(ply_from_root);
...
Value search(Position& pos, Value alpha, Value beta, Depth depth) {
	...
	while (move = mp.next_move()) {
		...
		if (!fullDepthSearch) {
			...
			value = depth - R < ONE_PLY ? -qsearch<NonPV>(pos, -beta, -alpha, depth - R)
		...
		if (fullDepthSearch)
			value = depth - ONE_PLY < ONE_PLY ? -qsearch<PV>(pos, -beta, -alpha, depth - ONE_PLY)
	...
	if (moveCount == 0) alpha = mated_in(ply_from_root);
	...
	tte->save(
	    key, value_to_tt(alpha, ply_from_root),
