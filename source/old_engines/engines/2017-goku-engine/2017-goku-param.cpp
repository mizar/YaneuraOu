...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	while ((move = mp.next_move()) != MOVE_NONE) {
		...
		bool evasionPrunable = InCheck && (depth != DEPTH_ZERO || moveCount > 2) &&
		                       bestValue > VALUE_MATED_IN_MAX_PLY && !pos.capture(move);
...
moves_loop:
	...
	bool singularExtensionNode =
	    !RootNode && depth >= PARAM_SINGULAR_EXTENSION_DEPTH * ONE_PLY && ttMove != MOVE_NONE &&
	...
	// PV
	// nodeで、置換表にhitして、その内容がBOUND_EXACTであるなら、reduction量を減らす。(もっと先まで読めるので読んだほうが良いはず！)
	...
	while ((move = mp.next_move(skipQuiets)) != MOVE_NONE) {
		...
		if (!RootNode
			...
			// やねうら王の独自のコード。depthの2乗に比例したseeマージン。適用depthに制限なし。
			         ...
			         !pos.see_ge(
			             move, Value(-PARAM_FUTILITY_AT_PARENT_NODE_GAMMA2 * (depth / ONE_PLY) * (depth / ONE_PLY))
		...
		// ttCaptureであるかの判定
		if (move == ttMove && captureOrPawnPromotion) ttCapture = true;
...
ValueAndPV qsearch(Position& pos) {
	...
	auto       bestValue = inCheck ? YaneuraOu2017GOKU::qsearch<PV, true>(pos, ss, -VALUE_INFINITE, VALUE_INFINITE)
