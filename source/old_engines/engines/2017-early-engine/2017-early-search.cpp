...
void update_continuation_histories(Stack* ss, Piece pc, Square s, int bonus) {
	for (int i : {1, 2, 4})
		if (is_ok((ss - i)->currentMove)) (ss - i)->contHistory->update(pc, s, bonus);
}
...
// quietsCnt = ↑の数
inline void update_stats(const Position& pos, Stack* ss, Move move, Move* quiets, int quietsCnt, int bonus) {
	//   killerのupdate
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	while ((move = mp.next_move()) != MOVE_NONE) {
		...
		bool evasionPrunable =
		    InCheck && depth != DEPTH_ZERO && bestValue > VALUE_MATED_IN_MAX_PLY && !pos.capture(move);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
	...
	Move ttMove = RootNode                 ? thisThread->rootMoves[thisThread->PVIdx].pv[0]
	              : ttHit && !excludedMove ? pos.move16_to_move(tte->move())
	                                       : MOVE_NONE;
	...
	if (!PvNode  // PV nodeでは置換表の指し手では枝刈りしない(PV nodeはごくわずかしかないので..)
		...
		if (ttMove) {
			if (ttValue >= beta) {
				...
				// ToDo :
				// Stockfish本家のコード、ここ、pos.piece_on()ではなく、pos.moved_piece()にしたほうが良いのでは…。
			...
			if (PARAM_WEAK_MATE_PLY == 1) {
				...
				if (move != MOVE_NONE) {
					...
					tte->save(posKey, value_to_tt(bestValue, ss->ply), BOUND_EXACT, DEPTH_MAX, move,
					          /* ss->staticEval */ bestValue, TT_GEN(pos));
	...
	if (inCheck) {
		...
		tte->save(posKey, VALUE_NONE, BOUND_NONE, DEPTH_NONE, MOVE_NONE, ss->staticEval, TT_GEN(pos));
		// どうせ毎node評価関数を呼び出すので、evalの値にそんなに価値はないのだが、mate1ply()を
		// 実行したという証にはなるので意味がある。
	}

	// このnodeで指し手生成前の枝刈りを省略するなら指し手生成ループへ。
	if (skipEarlyPruning) goto moves_loop;
	...
	if (!PvNode && depth < 4 * ONE_PLY && eval + razor_margin[depth / ONE_PLY] <= alpha) {
...
moves_loop:
	...
	while ((move = mp.next_move(skipQuiets)) != MOVE_NONE) {
		...
		if (!RootNode
		    ...
		    // cf. Allow inCheck pruning  :
		    // https://github.com/official-stockfish/Stockfish/commit/ab26c61971c2f73d312b003e6d024373fbacf8e6
		...
		if (depth >= 3 * ONE_PLY && moveCount > 1 && (!captureOrPawnPromotion || moveCountPruning)) {
			...
			if (captureOrPawnPromotion)
				...
				ss->statScore = thisThread->mainHistory[from_to(move)][~pos.side_to_move()] +
				                (*contHist[0])[movedSq][movedPiece] + (*contHist[1])[movedSq][movedPiece] +
				                (*contHist[3])[movedSq][movedPiece] - PARAM_REDUCTION_BY_HISTORY;  // 修正項
		...
		if (doFullDepthSearch)
			value = newDepth < ONE_PLY ? givesCheck ? -qsearch<NonPV, true>(pos, ss + 1, -(alpha + 1), -alpha)
		...
		if (PvNode && (moveCount == 1 || (value > alpha && (RootNode || value < beta)))) {
			...
			value = newDepth < ONE_PLY ? givesCheck ? -qsearch<PV, true>(pos, ss + 1, -beta, -alpha)
	...
	// 詰まされていない場合、bestMoveがあるならこの指し手をkiller等に登録する。
		...
		// 1手前は置換表の指し手であるのでNULL MOVEではありえない。
		if ((ss - 1)->moveCount == 1 && !pos.captured_piece())
			update_continuation_histories(ss - 1, pos.moved_piece_after((ss - 1)->currentMove), prevSq,
...
void init_param() {
	...
	{
		std::vector<std::string> param_names = {"PARAM_FUTILITY_MARGIN_ALPHA",
...
void Search::clear() {
	...
	for (int imp = 0; imp <= 1; ++imp)
		for (int d = 1; d < 64; ++d)
			for (int mc = 1; mc < 64; ++mc) {
				...
				if (!imp && Reductions[NonPV][imp][d][mc] >= 2 * ONE_PLY) Reductions[NonPV][imp][d][mc]++;
			}
	...
	for (int d = 0; d < PARAM_PRUNING_BY_MOVE_COUNT_DEPTH; ++d) {
		FutilityMoveCounts[0][d] =
		    int(PARAM_FUTILITY_MOVE_COUNT_ALPHA0 / 100.0 + PARAM_FUTILITY_MOVE_COUNT_BETA0 / 1000.0 * pow(d, 1.78));
		FutilityMoveCounts[1][d] =
		    int(PARAM_FUTILITY_MOVE_COUNT_ALPHA1 / 100.0 + PARAM_FUTILITY_MOVE_COUNT_BETA1 / 1000.0 * pow(d, 2.00));
	}
...
void Thread::search() {
	...
	for (int i = 4; i > 0; i--) (ss - i)->contHistory = &this->counterMoveHistory[SQ_ZERO][NO_PIECE];
	...
	while ((rootDepth += ONE_PLY) < DEPTH_MAX && !Threads.stop &&
		...
		if (idx) {
			int i = (idx - 1) % 20;
			if (((rootDepth / ONE_PLY + rootPos.game_ply() + skipPhase[i]) / skipSize[i]) % 2) continue;
		}
		...
		for (PVIdx = 0; PVIdx < multiPV && !Threads.stop; ++PVIdx) {
			...
			while (true) {
				...
				if (mainThread
				    ...
				    && multiPV == 1 && (bestValue <= alpha || beta <= bestValue) &&
				    Time.elapsed() > 3000
...
void MainThread::think() {
	...
	for (Thread* th : Threads) th->completedDepth = DEPTH_ZERO;
	...
	if (rootMoves.empty()) {
		...
		if (!Limits.silent) sync_cout << USI::pv(rootPos, ONE_PLY, -VALUE_INFINITE, VALUE_INFINITE) << sync_endl;
	...
	if (book.probe(*this, Limits)) goto ID_END;
...
ID_END:;
	...
	if (!this->easyMovePlayed && Options["MultiPV"] == 1 && !Limits.depth && rootMoves[0].pv[0] != MOVE_NONE) {
		...
		for (Thread* th : Threads) {
			...
			if (th->rootMoves.empty()) continue;

			Depth depthDiff = th->completedDepth - bestThread->completedDepth;
			Value scoreDiff = th->rootMoves[0].score - bestThread->rootMoves[0].score;

			// いまよりスコアが優れていて、かつ、探索深さがいまより浅くなければ。
			if (scoreDiff > 0 && depthDiff >= 0) bestThread = th;
		}
	}
...
void MainThread::check_time() {
	...
	if ((Limits.use_time_management() &&
	    ...
	    (Limits.movetime && elapsed >= Limits.movetime) || (Limits.nodes && Threads.nodes_searched() >= Limits.nodes))
		Threads.stop = true;
}
...
void init_for_search(Position& pos, Stack* ss) {
	...
	// this_threadに関して。
	{
		...
		for (int i = 4; i > 0; i--) (ss - i)->contHistory = &th->counterMoveHistory[SQ_ZERO][NO_PIECE];
...
ValueAndPV qsearch(Position& pos) {
	...
	auto       bestValue = inCheck ? YaneuraOu2017Early::qsearch<PV, true>(pos, ss, -VALUE_INFINITE, VALUE_INFINITE)
...
ValueAndPV search(Position& pos, int depth_, size_t multiPV /* = 1*/) {
	...
	if (depth < DEPTH_ZERO) return std::pair<Value, std::vector<Move>>(Eval::evaluate(pos), std::vector<Move>());
