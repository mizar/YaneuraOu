...
struct TTEntry {
	...
	void save(const Key128& key_, uint32_t depth, Move move) {
		depth16     = depth & 0xffff;
		depth_high8 = depth >> 16;
		key64       = key_.p(1);
		move16      = (uint16_t)move;
	}
...
struct TranspositionTable {
	...
	void save(const Key128& key, uint32_t depth, Move move) {
		...
		// 一致するhash
		// keyが格納されているTTEntryが見つからなかったし、空のエントリーも見つからなかったのでどれか一つ犠牲にする。
	...
	int hashfull() const {
		...
		for (int i = 0; i < 1000 / ClusterSize; ++i) {
			...
			for (int j = 0; j < ClusterSize; ++j)
				if ((tte[j].generation() == generation16)) ++cnt;
		}
		return cnt;
	}
	...
	void lock(Cluster& cluster) {
		...
		while (true) {
			uint8_t expected = 0;
			if (lk.compare_exchange_weak(expected, 1)) break;
	...
	void unlock(Cluster& cluster) {
		...
		if (!lk.compare_exchange_weak(expected, 0)) ASSERT_LV1(false);
	}
