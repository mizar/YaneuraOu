...
void id_loop(Position& pos, int thread_id, int thread_num) {
	...
	for (uint32_t depth = 1 + thread_id * 2; depth < MAX_PLY; depth += 2 * thread_num) {
		...
		if (Threads.stop || mate_found) break;

		// 定期的にdepth、nodes、npsを出力する。
		auto end_time      = now();
		auto node_searched = Threads.nodes_searched();  // 全スレッドでの探索合計
		sync_cout << "info  depth " << depth << " nodes " << node_searched << " nps "
		          << (node_searched * 1000 / ((int64_t)(end_time - start_time + 1))) << " hashfull " << TT.hashfull()
		          << sync_endl;
		...
		while (true) {
			...
			if (depth > sd) {
				if (search_depth.compare_exchange_weak(sd, depth)) break;
			} else
				break;  // 下回っているので書き込む価値はない。
		}
	}
}
