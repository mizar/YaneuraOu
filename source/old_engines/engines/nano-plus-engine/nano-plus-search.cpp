...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	if (InCheck && bestValue == -VALUE_INFINITE) {
		...
		tte->save(posKey, value_to_tt(bestValue, ss->ply), BOUND_EXACT, DEPTH_MAX, MOVE_NONE, ss->staticEval,
		          TT.generation());
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	if (InCheck) {
		...
		if ((ss->staticEval = eval = tte->eval()) == VALUE_NONE) ss->staticEval = evaluate(pos);
...

{
	...
	while (move = mp.next_move()) {
		...
		if (depth >= 3 * ONE_PLY && moveCount > 1 && !captureOrPromotion) {
			...
			if (r && pos.capture(move) && type_of(pos.piece_on(move_to(move))) != PAWN  // 歩以外の捕獲
			)
	...
	tte->save(
	    ...
	    : PvNode && bestMove ? BOUND_EXACT
	                         : BOUND_UPPER,
	    // betaを超えているということはbeta
	    // cutされるわけで残りの指し手を調べていないから真の値はまだ大きいと考えられる。
...
void MainThread::think() {
	...
	// ---------------------
	{
		...
		while (++rootDepth < MAX_PLY && !Signals.stop && (!Limits.depth || Threads.main()->rootDepth <= Limits.depth)) {
			...
			// 読み筋を出力しておく。
			if (!Limits.silent) sync_cout << USI::pv(pos, rootDepth, alpha, beta) << sync_endl;
		}
...
ID_END:;  // 反復深化の終了。
	...
	if (!Limits.silent) sync_cout << "bestmove " << bestMove << sync_endl;
}
