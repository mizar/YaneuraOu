...
struct ProcessNegotiator {
	...
	void run(string app_path_) {
		...
		success = ::CreateProcess(NULL,                      // ApplicationName
	...
	string read() {
		...
		BOOL success = ::PeekNamedPipe(child_std_out_read,  // [in]  handle of named pipe
		                               chBuf,               // [out] buffer
...
#undef ERROR_MES
	...
	string read_next() {
		...
		if (it == string::npos) return string();
		// 切り出したいのは"\n"の手前まで(改行コード不要)、このあと"\n"は捨てたいので
		// it+1から最後までが次回まわし。
...
struct EngineState {
	...
	void on_idle() {
		switch (state) {
			...
			case IS_READY:
				// エンジンの初期化コマンドを送ってやる
				for (auto line : engine_config) pn.write(line);
	...
	Move think(const Position& pos, const string& think_cmd, const string& engine_name) {
		...
		while (true) {
			bestmove = pn.read();
			if (bestmove.find("bestmove") != string::npos) break;
...
void MainThread::think() {
	...
	sync_cout << endl
	          << "local game server end : [" << engine_name[0] << "] vs [" << engine_name[1] << "]" << sync_endl;
...
void Thread::search() {
	...
	// プロセスの生成に失敗しているなら終了。
	if (!es[0].pn.success || !es[1].pn.success) return;

	for (int i = 0; i < 2; ++i) es[i].set_engine_config(engine_config_lines[i]);
	...
	if (max_games == 0) max_games = 100;  // デフォルトでは100回
	//  int games = 0;

	// 定跡の手数
	int max_book_move = Search::Limits.time[WHITE];
	if (max_book_move == 0) max_book_move = 32;  // デフォルトでは32手目から
	...
	auto game_start = [&] {
		...
		if (book.size()) {
			...
			while (rootPos.game_ply() < max_book_move) {
				...
				if (!is_ok(m)) {
					//  sync_cout << "Error book.sfen , line = " << book_number << " , moves = " << token << endl <<
					//  rootPos << sync_endl;
					// →　エラー扱いはしない。
					break;
				} else {
	...
	while (/*!Threads.stop &&*/ games < max_games) {
		...
		if (es[0].pn.is_terminated() || es[1].pn.is_terminated()) break;
		...
		if (game_started) {
			...
			auto engine_name =
			    es[player].engine_exe_name();  // engine_name()だとエラーが起きたときにどれだかわからない可能性がある。
			...
			if (m != MOVE_RESIGN && (!rootPos.pseudo_legal(m) || !rootPos.legal(m))) {
				sync_cout << "Error : illigal move , move = " << m << " , engine name = " << engine_name << endl
				          << rootPos << sync_endl;
				m = MOVE_RESIGN;
			} else {
				SetupStates->push(StateInfo());
	...
	if (games == max_games) game_over(false);
