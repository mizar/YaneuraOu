...
bool Progress::Save() {
	...
	if (!ofs.write(reinterpret_cast<char*>(weights_), sizeof(weights_))) {
		sync_cout << "info string Failed to write the progress file. file_path=" << file_path << sync_endl;
		return false;
	}
...
bool Progress::Learn() {
	...
	std::vector<std::vector<Move> > games_for_training(games.begin(), games.begin() + num_games_for_training);
	...
	std::sprintf(loss_file_name, "learn_progress_loss_%I64d.learning_rate=%f.csv", std::time(nullptr), learning_rate);
		...
		for (int game_index = 0; game_index < num_games_for_training; ++game_index) {
			...
			for (int move_index = 0; move_index < num_moves; ++move_index) {
				...
				if (std::isnan(sum_diff2_train)) {
					sync_cout << "game_index: " << game_index << "move_index: " << move_index << sync_endl;
					std::exit(1);
				}
...
#pragma omp parallel for reduction(+ : sum_diff2_test, num_moves_in_test) schedule(dynamic)
		...
		// ロスの出力
		ofs_loss << offset / num_moves_in_train << "," << std::sqrt(sum_diff2_train / num_moves_in_train) << ","
		         << std::sqrt(sum_diff2_test / num_moves_in_test) << std::endl
		         << std::flush;
