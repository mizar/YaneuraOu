...
struct alignas(32) EvalHash {
	...
	EvalHash& operator=(const EvalHash& rhs) {
		this->m = rhs.m;
		return *this;
	}
};
			...
			if (dirty >= PIECE_NO_KING)  // 王と王でないかで場合分け
			{
				if (dirty == PIECE_NO_BKING) {
					...
					for (i = 0; i < PIECE_NO_KING; i++) {
						...
						for (j = 0; j < i; j++) sumBKPP += Eval::kpp[sq_bk0][k0][now_list_fb[j]];
					}
					...
					for (i = 0; i < PIECE_NO_KING; i++) {
						...
						for (j = 0; j < i; j++) sumWKPP -= Eval::kpp[sq_wk1][k1][now_list_fw[j]];
					}
	...
	{                                                     \
		...
		sumWKPP -= Eval::kpp[sq_wk1][W3][now_list_fw[i]]; \
				...
				if (k == 1) {
					...
					// KP値、要らんのでi==dirtyを除く
					for (i = 0; i < dirty; ++i) ADD_BWKPP(k0, k1, k2, k3);
					for (++i; i < PIECE_NO_KING; ++i) ADD_BWKPP(k0, k1, k2, k3);
...
void evaluate_with_no_return(const Position& pos) {
	if (pos.state()->sumKKP == VALUE_NOT_EVALUATED) evaluate(pos);
}
