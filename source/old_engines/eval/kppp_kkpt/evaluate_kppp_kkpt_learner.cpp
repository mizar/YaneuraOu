...
#pragma omp for schedule(dynamic, 20000)
		for (s64 index_ = 0; index_ < (s64)vector_length; ++index_) {
			...
			if (g_kk.is_ok(index) && !freeze_kk) {
				...
				for (int i = 1; i < KPP_LOWER_COUNT; ++i) kpp[a[i].king()][a[i].piece0()][a[i].piece1()] = v;
#else
				...
				for (int i = 0; i < KPPP_LOWER_COUNT; ++i) ids[i] = a[i].toRawIndex();

				LearnFloatType g_sum = zero;
				for (auto id : ids) g_sum += weights_kppp[id].get_grad();
				...
				for (auto id : ids) weights_kppp[id].set_grad(zero);
#else
...
void save_eval(std::string dir_name) {
	{
		...
		auto output = EvalIO::EvalInfo::build_kppp_kkpt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
