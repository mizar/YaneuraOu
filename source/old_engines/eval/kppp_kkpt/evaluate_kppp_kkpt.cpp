...
void evaluate_kppp(const Position& pos) {
	...
	if (kppp_bk_enable) {
		...
		if (file_of(sq_bk_for_kppp) >= FILE_6) {
			//  KK   は、sq_bk,sq_wk に依存するので、これらをまとめてミラーしているからこのコードで合ってる。 KKP
			//  は、sq_bk,sq_wk,    Bから見たPに依存するので、これらをまとめてミラーしているからこのコードで合ってる。
	...
	// 注)
	// PIECE_NUMBER_KPPP_START～PIECE_NUMBER_KINGの範囲だけ入れ替えないと配列の中身(対応する駒)がぐちゃぐちゃになる。
