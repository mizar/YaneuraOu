...
void load_eval_impl() {
	...
	auto input     = EvalIO::EvalInfo::build_kkppt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
...
void compute_eval_impl(const Position& pos, int encoded_eval_kk) {
	...
	if (encoded_eval_kk != -1) {
		...
		for (i = 0; i < PIECE_NUMBER_KING; ++i) {
			...
			// 端数の処理その2
			for (; j < i; ++j) sum.p[0] += kkpp_kp_fb[list_fb[j]];
...
std::array<s32, 2> do_a_black(const Position& pos, const ExtBonaPiece ebp) {
	...
	for (int i = 1; i < PIECE_NUMBER_KING; ++i) sum += kpp_kp_fb[list0[i]];
	return sum;
}
...
std::array<s32, 2> do_a_white(const Position& pos, const ExtBonaPiece ebp) {
	...
	for (int i = 1; i < PIECE_NUMBER_KING; ++i) sum += kpp_kp_fw[list1[i]];
	return sum;
}
...
EvalSum kkpp_do_a_pc(const Position& pos, const ExtBonaPiece ebp, int encoded_eval_kk) {
	...
	// 端数の処理その2
	for (; i < PIECE_LIST_LENGTH; ++i) sum.p[0] += kkpp_kp_fb[list0[i]];
...
void evaluateBody_kpp(const Position& pos) {
	...
	if (dirty >= PIECE_NUMBER_KING) {
		...
		if (dirty == PIECE_NUMBER_WKING) {
			...
			__m128i diffp1_128 =
			    _mm_add_epi32(_mm256_extracti128_si256(diffp1, 0), _mm256_extracti128_si256(diffp1, 1));
			...
			__m128i diffp0_128 =
			    _mm_add_epi32(_mm256_extracti128_si256(diffp0, 0), _mm256_extracti128_si256(diffp0, 1));
...
void kk_stat() {
	...
	for_all_sq([](Square sq) {
		...
		for (auto p1 = BONA_PIECE_ZERO; p1 < fe_end; ++p1)
			for (auto p2 = BONA_PIECE_ZERO; p2 < fe_end; ++p2) sum_kpp += kpp_ksq_pcpc(sq, p1, p2);
		...
		for (auto p1 = BONA_PIECE_ZERO; p1 < fe_end; ++p1)
			for (auto p2 = BONA_PIECE_ZERO; p2 < fe_end; ++p2)
				...
				if (!is_zero(kpp_ksq_pcpc(sq, p1, p2)) && p1 != p2 && p1 && p2)
					kpp_ksq_pcpc(sq, p1, p2) =
					    (ValueKpp)(kpp_ksq_pcpc(sq, p1, p2) -
					               ValueKpp{(int16_t)(kpp_offset[sq][0]), (int16_t)(kpp_offset[sq][1])});
	}
}
