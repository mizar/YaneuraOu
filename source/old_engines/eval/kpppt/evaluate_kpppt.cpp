...
void load_eval_impl() {
	...
	auto input     = EvalIO::EvalInfo::build_kpppt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
...
void compute_eval_impl(const Position& pos) {
	...
	if (kppp_bk_enable) {
		...
		if (file_of(sq_bk_for_kppp) >= FILE_6) {
			...
			for (int i = 0; i < PIECE_NUMBER_KING; ++i) list_fb[i] = EvalLearningTools::mir_piece(list_fb[i]);
		}
		// 玉の位置を詰める
	...
	if (kppp_wk_enable) {
		...
		if (file_of(sq_wk_for_kppp) >= FILE_6) {
			...
			for (int i = 0; i < PIECE_NUMBER_KING; ++i) list_fw[i] = EvalLearningTools::mir_piece(list_fw[i]);
		}
...
EvalSum do_a_pc(const Position& pos, const ExtBonaPiece ebp) {
	...
	*/
	...
	sum.m[0] = _mm_set_epi32(0, 0, *reinterpret_cast<const s32*>(&pkppw[list1[0]][0]),
	                         *reinterpret_cast<const s32*>(&pkppb[list0[0]][0]));
	sum.m[0] = _mm_cvtepi16_epi32(sum.m[0]);
	for (int i = 1; i < PIECE_NUMBER_KING; ++i) {
		__m128i tmp;
		tmp = _mm_set_epi32(0, 0, *reinterpret_cast<const s32*>(&pkppw[list1[i]][0]),
		                    *reinterpret_cast<const s32*>(&pkppb[list0[i]][0]));
...
void kk_stat() {
	...
	for_all_sq([](Square sq) {
		...
		for (auto p1 = 0; p1 < fe_end; ++p1)
			for (auto p2 = 0; p2 < fe_end; ++p2) sum_kpp += kpp[sq][p1][p2];
	...
	for (Square sq = SQ_ZERO; sq < SQ_NB; ++sq) {
		...
		for (auto p1 = 0; p1 < fe_end; ++p1)
			for (auto p2 = 0; p2 < fe_end; ++p2)
				...
				if (kpp[sq][p1][p2][0] && p1 != p2 && p1 && p2) kpp[sq][p1][p2] -= kpp_offset[sq];
	}
}
