...
#pragma omp for schedule(dynamic, 20000)
		for (s64 index_ = 0; index_ < (s64)vector_length; ++index_) {
			...
			if (g_kk.is_ok(index) && !freeze_kk) {
				...
				for (auto id : ids) weights_kpp[id].set_grad(zero_t);
				...
				for (auto id : ids) weights_kppp[id].set_grad(zero_t);
#else
...
void save_eval(std::string dir_name) {
	{
		...
		auto output    = EvalIO::EvalInfo::build_kpppt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
