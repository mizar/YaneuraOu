...
void load_eval_impl() {
	...
	auto input     = EvalIO::EvalInfo::build_kppp_kkpt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
                                                     make_name(KPPP_BIN), size_of_kppp);
...
void init_kppp_bona_piece_table() {
	...
	for (int i = 1; i < fe_end; ++i) kppp_bona_piece_table[i] = BONA_PIECE_NOT_INIT;

	int j = 1;  // 番号は1から使っていく。
	for (auto v : arr)
		for (int i = (int)v.first; i < (int)v.first + v.second; ++i) kppp_bona_piece_table[i] = (BonaPiece)(j++);
...
void load_eval() {
	...
	WaitForSingleObject(hMutex, INFINITE);
	{
		...
		auto hMap = CreateFileMapping(INVALID_HANDLE_VALUE, NULL,
		                              ...
		                              (u32)(size_of_eval_ >> 32), (u32)size_of_eval_, mapped_file_name.c_str());
...
BonaPiece to_kppp_bona_piece(BonaPiece p) { return kppp_bona_piece_table[p]; }

// kppp用に写像されたBonaPieceに対するミラーしたBonaPieceを求める。
BonaPiece kppp_mir_piece(BonaPiece p) { return kppp_mir_piece_table[p]; }
...
EvalSum do_a_pc(const Position& pos, const ExtBonaPiece ebp) {
	...
	*/
	...
	// Nodes/second    : 784886
	// Function	Samples	% of Hotspot Samples	Module
	// Eval::do_a_pc(struct Position const &, struct Eval::ExtBonaPiece)	5870	11.9399996	YaneuraOu - 2017 -
	// early.exe
	//
	// NPSが2.3%程度向上した
	...
	for (; i + 8 < PIECE_NUMBER_KING; i += 8) {
		...
		__m256i w0 =
		    _mm256_set_epi32(pkppb[list0[i + 7]], pkppb[list0[i + 6]], pkppb[list0[i + 5]], pkppb[list0[i + 4]],
		                     pkppb[list0[i + 3]], pkppb[list0[i + 2]], pkppb[list0[i + 1]], pkppb[list0[i + 0]]);
		__m256i w1 =
		    _mm256_set_epi32(pkppw[list1[i + 7]], pkppw[list1[i + 6]], pkppw[list1[i + 5]], pkppw[list1[i + 4]],
		                     pkppw[list1[i + 3]], pkppw[list1[i + 2]], pkppw[list1[i + 1]], pkppw[list1[i + 0]]);
	...
	// 端数の6要素分の処理
	{
		__m256i w0 = _mm256_set_epi32(0, 0, pkppb[list0[i + 5]], pkppb[list0[i + 4]], pkppb[list0[i + 3]],
		                              pkppb[list0[i + 2]], pkppb[list0[i + 1]], pkppb[list0[i + 0]]);
		__m256i w1 = _mm256_set_epi32(0, 0, pkppw[list1[i + 5]], pkppw[list1[i + 4]], pkppw[list1[i + 3]],
		                              pkppw[list1[i + 2]], pkppw[list1[i + 1]], pkppw[list1[i + 0]]);
		sum0       = _mm256_add_epi32(sum0, w0);
		sum1       = _mm256_add_epi32(sum1, w1);
	}
		...
		if (dirty == PIECE_NUMBER_WKING) {
			...
			for (int i = 0; i < PIECE_NUMBER_KING; ++i) {
				...
				for (; j + 8 < i; j += 8) {
					// 1要素が16-bitでvgatherdd命令が使えないため
					// 通常のメモリアクセスで評価値をロードする
					__m256i w1 = _mm256_set_epi32(pkppw[list1[j + 7]], pkppw[list1[j + 6]], pkppw[list1[j + 5]],
					                              pkppw[list1[j + 4]], pkppw[list1[j + 3]], pkppw[list1[j + 2]],
					                              pkppw[list1[j + 1]], pkppw[list1[j + 0]]);
					sum1_256   = _mm256_add_epi32(sum1_256, w1);
				}

				for (; j + 4 < i; j += 4) {
					__m128i w1 = _mm_set_epi32(pkppw[list1[j + 3]], pkppw[list1[j + 2]], pkppw[list1[j + 1]],
					                           pkppw[list1[j + 0]]);
			...
			for (int i = 0; i < PIECE_NUMBER_KING; ++i) {
				...
				for (; j + 8 < i; j += 8) {
					// 1要素が16-bitでvgatherdd命令が使えないため
					// 通常のメモリアクセスで評価値をロードする
					__m256i w1 = _mm256_set_epi32(pkppb[list0[j + 7]], pkppb[list0[j + 6]], pkppb[list0[j + 5]],
					                              pkppb[list0[j + 4]], pkppb[list0[j + 3]], pkppb[list0[j + 2]],
					                              pkppb[list0[j + 1]], pkppb[list0[j + 0]]);
					sum0_256   = _mm256_add_epi32(sum0_256, w1);
				}

				for (; j + 4 < i; j += 4) {
					__m128i w1 = _mm_set_epi32(pkppb[list0[j + 3]], pkppb[list0[j + 2]], pkppb[list0[j + 1]],
					                           pkppb[list0[j + 0]]);
...
void kk_stat() {
	...
	for_all_sq([](Square sq) {
		...
		for (auto sq2 = 0; sq2 < SQ_NB; ++sq2) {
			sum_kk += kk[sq][sq2];
			for (auto p = 0; p < fe_end; ++p) sum_kkp += kkp[sq][sq2][p];
		}
		for (auto p1 = 0; p1 < fe_end; ++p1)
			for (auto p2 = 0; p2 < fe_end; ++p2) sum_kpp[0] += kpp[sq][p1][p2];
