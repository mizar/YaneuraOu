...
#pragma omp for schedule(dynamic, 20000)
		for (s64 index_ = 0; index_ < (s64)vector_length; ++index_) {
			...
			if (index < KPP::max_index() && !min_index_flag[index]) continue;
			...
			if (KK::is_ok(index) && !freeze_kk) {
				...
				if (file_of((Square)x.king()) >= FILE_6) continue;
				...
				BonaPiece p_list[3] = {kppp_mir_piece(x.piece2()), kppp_mir_piece(x.piece1()),
				                       kppp_mir_piece(x.piece0())};
				my_insertion_sort(p_list, 0, 3);
				a[1] = g_kppp.fromKPPP(Mir((Square)x.king()), p_list[2], p_list[1], p_list[0]);

				u64 ids[MY_KPPP_LOWER_COUNT];
				for (int i = 0; i < MY_KPPP_LOWER_COUNT; ++i) ids[i] = a[i].toRawIndex();
