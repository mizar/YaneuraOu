...
void compute_eval_impl(const Position& pos) {
	...
	if (!dp.updated()) dp.do_update(eval_list);
...
Value evaluate(const Position& pos) {
	...
	if (entry.key == keyExcludeTurn) {
		...
		if (!dp.updated()) dp.do_update(*(const_cast<EvalList*>(pos.eval_list())));
...
auto bp_str = [](BonaPiece p) {
	...
	if (p >= fe_old_end) {
		...
		for (auto q : a) ss << (q.f | q.r) << pretty(q.pc) << " ";
	} else
		ss << p;
...
void print_eval_list(Position& pos) {
	...
	for (int i = 0; i < length; ++i) cout << int(list_fb[i]) << " = " << bp_str(list_fb[i]) << endl;
...
void print_eval_stat(Position& pos) {
	...
	// 38枚の駒を表示
	for (i = 0; i < length; ++i)
		cout << int(list_fb[i]) << " = " << bp_str(list_fb[i]) << " , " << int(list_fw[i]) << " =  "
		     << bp_str(list_fw[i]) << endl;
	...
	for (i = 0; i < length; ++i) {
		...
		cout << "KKP : " << sq_bk << " " << sq_wk << " " << bp_str(k0) << " = " << kkp[sq_bk][sq_wk][k0][0] << " + "
		...
		if (c == BLACK) {
			...
			if (RANK_5 <= r && r <= RANK_7)  // この3段にいるならば
			{
				...
				if (f <= FILE_5) {
					...
					if (v) remove_list.push_back((BonaPiece)(v + fe_nabla_pawn));
					v += r2 << ((int)f * 2);
					add_list.push_back((BonaPiece)(v + fe_nabla_pawn));
				}
				if (f >= FILE_5) {
					auto& v = state()->nabla_work[1];
					if (v) remove_list.push_back((BonaPiece)(v + 1024 + fe_nabla_pawn));
					v += r2 << ((8 - (int)f) * 2);
			...
			if (RANK_3 <= r && r <= RANK_5)  // この3段にいるならば
			{
				...
				if (f >= FILE_5) {
					...
					if (v) remove_list.push_back((BonaPiece)(v + 1024 * 2 + fe_nabla_pawn));
					v += r2 << ((8 - (int)f) * 2);
					add_list.push_back((BonaPiece)(v + 1024 * 2 + fe_nabla_pawn));
				}
				if (f <= FILE_5) {
					auto& v = state()->nabla_work[3];
					if (v) remove_list.push_back((BonaPiece)(v + 1024 * 3 + fe_nabla_pawn));
					v += r2 << ((int)f * 2);
		...
		if (c == BLACK) {
			...
			if (RANK_5 <= r && r <= RANK_7)  // この3段にいるならば
			{
				...
				if (f <= FILE_5) {
					...
					if (v) add_list.push_back((BonaPiece)(v + fe_nabla_pawn));
				}
				if (f >= FILE_5) {
					auto& v = state()->nabla_work[1];
					...
					v -= r2 << ((8 - (int)f) * 2);
					if (v) add_list.push_back((BonaPiece)(v + 1024 + fe_nabla_pawn));
				}
			} else
				goto NORMAL;  // 対象外。通常処理
		} else {
			...
			if (RANK_3 <= r && r <= RANK_5)  // この3段にいるならば
			{
				...
				if (f >= FILE_5) {
					...
					ASSERT_LV3(v >= (r2 << ((8 - (int)f) * 2)));
					v -= r2 << ((8 - (int)f) * 2);
					if (v) add_list.push_back((BonaPiece)(v + 1024 * 2 + fe_nabla_pawn));
				}
				if (f <= FILE_5) {
					auto& v = state()->nabla_work[3];
					...
					ASSERT_LV3(v >= (r2 << ((int)(f)*2)));
					v -= r2 << ((int)(f)*2);
					if (v) add_list.push_back((BonaPiece)(v + 1024 * 3 + fe_nabla_pawn));
				}
			} else
				goto NORMAL;  // 対象外。通常処理
		}
...
void DirtyPiece::add_piece(Color c, Piece pt, int i) { add_list.push_back(BonaPiece(kpp_hand_index[c][pt].fb + i)); }
