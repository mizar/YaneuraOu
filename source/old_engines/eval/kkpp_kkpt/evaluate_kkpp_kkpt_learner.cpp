...
void correct_eval() {
	...
	// 駒落ちなどではここを利用したいので0クリアすべき。
	{
		...
		for (auto sq1 : SQ)
			for (auto sq2 : SQ) kkp[sq1][sq2][0] = kkp_zero;
	...
	{
		for (auto sq : SQ) kk[sq][Inv(sq)][0] = 0;
	}
#endif
}
...
void add_grad(Position& pos, Color rootColor, double delta_grad, const std::array<bool, 4>& freeze) {
	...
	if (encoded_eval_kk != -1) {
		...
		if (file_of(sq_bk) > FILE_5) {
			...
			for (int i = 0; i < PIECE_NUMBER_KING; ++i) list_fb[i] = mir_piece(list_fb[i]);
		}
...
#pragma omp for schedule(dynamic, 20000)
		for (s64 index_ = 0; index_ < (s64)vector_length; ++index_) {
			...
			if (index < g_kpp.max_index() && !min_index_flag[index]) continue;
			...
			if (g_kk.is_ok(index) && !freeze_kk) {
				...
				for (int i = 0; i < KK_LOWER_COUNT; ++i) ids[i] = a[i].toIndex();
				...
				for (int i = 0; i < KK_LOWER_COUNT; ++i) g_sum += a[i].apply_inverse_sign(weights[ids[i]].get_grad());

				// 次元下げを考慮して、その勾配の合計が0であるなら、一切の更新をする必要はない。
				if (is_zero(g_sum)) continue;

				auto& v = kk[a[0].king0()][a[0].king1()];
				weights[ids[0]].set_grad(g_sum);
				weights[ids[0]].updateFV(v);

				for (int i = 1; i < KK_LOWER_COUNT; ++i) kk[a[i].king0()][a[i].king1()] = a[i].apply_inverse_sign(v);

				// mirrorした場所が同じindexである可能性があるので、gのクリアはこのタイミングで行なう。
				// この場合、毎回gを通常の2倍加算していることになるが、AdaGradは適応型なのでこれでもうまく学習できる。
				for (auto id : ids) weights[id].set_grad(zero_t);
				...
				for (int i = 0; i < KKP_LOWER_COUNT; ++i) ids[i] = a[i].toIndex();

				std::array<LearnFloatType, 2> g_sum = zero_t;
				for (int i = 0; i < KKP_LOWER_COUNT; ++i) g_sum += a[i].apply_inverse_sign(weights[ids[i]].get_grad());
				...
				for (int i = 0; i < KPP_LOWER_COUNT; ++i) ids[i] = a[i].toRawIndex();

				// KPPに関してはinverseの次元下げがないので、inverseの判定は不要。

				// KPPTとの違いは、ここに手番がないというだけ。
				LearnFloatType g_sum = zero;
				for (auto id : ids) g_sum += weights_kpp[id].get_grad();
				...
				for (int i = 1; i < KPP_LOWER_COUNT; ++i) kpp_ksq_pcpc(a[i].king(), a[i].piece0(), a[i].piece1()) = v;
#else
...
#endif
#endif
				for (auto id : ids) weights_kpp[id].set_grad(zero);
				...
				for (int i = 0; i < KPP_LOWER_COUNT; ++i) {
					...
					for (Square wk : SQ) {
						...
						if (encoded_eval_kk_from_bk == -1) continue;
						...
						vec_bk.push_back(&kkpp_ksq_pcpc(mir_encoded_eval_kk_from_bk, mir_piece(a[i].piece0()),
						                                mir_piece(a[i].piece1())));
						vec_bk.push_back(&kkpp_ksq_pcpc(mir_encoded_eval_kk_from_bk, mir_piece(a[i].piece1()),
						                                mir_piece(a[i].piece0())));
						...
						vec_wk.push_back(&kkpp_ksq_pcpc(encoded_eval_kk_from_wk, inv_piece(a[i].piece0()),
						                                inv_piece(a[i].piece1())));
						vec_wk.push_back(&kkpp_ksq_pcpc(encoded_eval_kk_from_wk, inv_piece(a[i].piece1()),
						                                inv_piece(a[i].piece0())));

						int mir_encoded_eval_kk_from_wk = encode_to_eval_kk(Mir(Inv(wk)), Mir(Inv(bk)));
						ASSERT_LV3(mir_encoded_eval_kk_from_wk != -1);

						vec_wk.push_back(&kkpp_ksq_pcpc(mir_encoded_eval_kk_from_wk,
						                                mir_piece(inv_piece(a[i].piece0())),
						                                mir_piece((inv_piece(a[i].piece1())))));
						vec_wk.push_back(&kkpp_ksq_pcpc(mir_encoded_eval_kk_from_wk,
						                                mir_piece(inv_piece(a[i].piece1())),
						                                mir_piece((inv_piece(a[i].piece0())))));
					}
				...
				for (auto ptr : vec_bk) *ptr += diff;
				for (auto ptr : vec_wk) *ptr -= diff;
...
// isreadyコマンドで評価関数ファイルを読み込み、"test
// evalsave"コマンドでsave_eval()を呼び出すと変換したものが書き出されるという考え。
...
void save_eval(std::string dir_name) {
...

	{
		...
		auto output = EvalIO::EvalInfo::build_kkpp_kkpt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
...
// 現在のetaを取得する。
double get_eta() { return Weight::eta; }
