...
void load_eval_impl() {
	...
	auto input     = EvalIO::EvalInfo::build_kkpp_kkpt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN),
                                                     make_name(KKPP_BIN), size_of_kkpp);
...
u64 calc_check_sum() {
	...
	auto add_sum = [&](u16* ptr, size_t t) {
		for (size_t i = 0; i < t; ++i) sum += ptr[i];
	};
...
void load_eval() {
	...
	{
		// ファイルマッピングオブジェクトの作成
		auto hMap = CreateFileMapping(INVALID_HANDLE_VALUE, NULL,
		                              PAGE_READWRITE,  // | /**SEC_COMMIT/**/ /*SEC_RESERVE/**/,
		                              (u32)(size_of_eval >> 32), (u32)size_of_eval, mapped_file_name.c_str());
...
void compute_eval_impl(const Position& pos, int encoded_eval_kk) {
	...
	if (encoded_eval_kk != -1) {
		...
		for (i = 0; i < PIECE_NUMBER_KING; ++i) {
			...
			// 端数の処理その1
			if (j + 4 < i) {
				__m256i w0 = _mm256_set_epi32(0, 0, 0, 0, kkpp_kp_fb[list_fb[j + 3]], kkpp_kp_fb[list_fb[j + 2]],
				                              kkpp_kp_fb[list_fb[j + 1]], kkpp_kp_fb[list_fb[j + 0]]);
				sum0       = _mm256_add_epi32(sum0, w0);
				j += 4;
			}
			// 端数の処理その2
			for (; j < i; ++j) sum.p[0][0] += kkpp_kp_fb[list_fb[j]];
		...
		for (i = 0; i < PIECE_NUMBER_KING; ++i) {
			...
			// 端数の処理その1
			if (j + 4 < i) {
				__m256i w0 = _mm256_set_epi32(0, 0, 0, 0, kpp_kp_fb[list_fb[j + 3]], kpp_kp_fb[list_fb[j + 2]],
				                              kpp_kp_fb[list_fb[j + 1]], kpp_kp_fb[list_fb[j + 0]]);
				__m256i w1 = _mm256_set_epi32(0, 0, 0, 0, kpp_kp_fw[list_fw[j + 3]], kpp_kp_fw[list_fw[j + 2]],
				                              kpp_kp_fw[list_fw[j + 1]], kpp_kp_fw[list_fw[j + 0]]);
				sum0       = _mm256_add_epi32(sum0, w0);
				sum1       = _mm256_add_epi32(sum1, w1);
				j += 4;
			}
...
EvalSum do_a_pc(const Position& pos, const ExtBonaPiece ebp) {
	...
	*/
	...
	// Eval::do_a_pc(struct Position const &, struct Eval::ExtBonaPiece)	6509	14.6000004	YaneuraOu - 2017 -
	// early.exe Eval::evaluateBody(struct Position const &)	6201	13.9099998	YaneuraOu - 2017 - early.exe
	...
	// Eval::do_a_pc(struct Position const &, struct Eval::ExtBonaPiece)	5100	12.1800003	YaneuraOu - 2017 -
	// early.exe
	...
	if (i + 4 < PIECE_LIST_LENGTH) {
		__m256i w0 = _mm256_set_epi32(0, 0, 0, 0, kpp_kp_fb[list0[i + 3]], kpp_kp_fb[list0[i + 2]],
		                              kpp_kp_fb[list0[i + 1]], kpp_kp_fb[list0[i + 0]]);
		__m256i w1 = _mm256_set_epi32(0, 0, 0, 0, kpp_kp_fw[list1[i + 3]], kpp_kp_fw[list1[i + 2]],
		                              kpp_kp_fw[list1[i + 1]], kpp_kp_fw[list1[i + 0]]);
		sum0       = _mm256_add_epi32(sum0, w0);
		sum1       = _mm256_add_epi32(sum1, w1);
		i += 4;
	}
...
EvalSum kkpp_do_a_pc(const Position& pos, const ExtBonaPiece ebp, int encoded_eval_kk) {
	...
	// 端数の処理その1
	if (i + 4 < PIECE_LIST_LENGTH) {
		__m256i w0 = _mm256_set_epi32(0, 0, 0, 0, kkpp_kp_fb[list0[i + 3]], kkpp_kp_fb[list0[i + 2]],
		                              kkpp_kp_fb[list0[i + 1]], kkpp_kp_fb[list0[i + 0]]);
		sum0       = _mm256_add_epi32(sum0, w0);
		i += 4;
	}
	...
	// 端数の処理その2
	for (; i < PIECE_LIST_LENGTH; ++i) sum.p[0][0] += kkpp_kp_fb[list0[i]];
...
// prefetchする関数も用意しておく。
void prefetch_evalhash(const Key key) { prefetch(g_evalTable[key >> 1]); }
...
void evaluateBody_kpp(const Position& pos) {
	...
	if (dirty >= PIECE_NUMBER_KING) {
		...
		if (moved_piece_num == 1) {
			...
			diff.p[1][0] -=
			    kpp_ksq_pcpc(Inv(sq_wk), dp.changed_piece[0].new_piece.fw, dp.changed_piece[1].new_piece.fw);
			...
			diff.p[1][0] +=
			    kpp_ksq_pcpc(Inv(sq_wk), dp.changed_piece[0].old_piece.fw, dp.changed_piece[1].old_piece.fw);
			list0[listIndex_cap] = dp.changed_piece[1].new_piece.fb;
			list1[listIndex_cap] = dp.changed_piece[1].new_piece.fw;
		}
...
void evaluateBody_kkpp(const Position& pos, int last_encoded_eval_kk, int encoded_eval_kk) {
	...
	if (dirty >= PIECE_NUMBER_KING) {
		...
		if (moved_piece_num == 1) {
			...
			diff.p[0][0] -=
			    kkpp_ksq_pcpc(encoded_eval_kk, dp.changed_piece[0].new_piece.fb, dp.changed_piece[1].new_piece.fb);
			...
			diff.p[0][0] +=
			    kkpp_ksq_pcpc(encoded_eval_kk, dp.changed_piece[0].old_piece.fb, dp.changed_piece[1].old_piece.fb);
			list0[listIndex_cap] = dp.changed_piece[1].new_piece.fb;
		}
...
void evaluateBody(const Position& pos) { compute_eval_impl(pos); }
...
Value evaluate(const Position& pos) {
	...
	if (sum.evaluated()) return Value(sum.sum(pos.side_to_move()) / FV_SCALE);
...
#if !defined(USE_EVAL_MAKE_LIST_FUNCTION)
	// まだ評価値が計算されていないなら
	if (!pos.state()->sum.evaluated()) evaluate(pos);
#else
...
void kk_stat() {
	...
	for_all_sq([](Square sq) {
		...
		for (auto sq2 = 0; sq2 < SQ_NB; ++sq2) {
			sum_kk += kk[sq][sq2];
			for (auto p = BONA_PIECE_ZERO; p < fe_end; ++p) sum_kkp += kkp[sq][sq2][p];
		}
		for (auto p1 = BONA_PIECE_ZERO; p1 < fe_end; ++p1)
			for (auto p2 = BONA_PIECE_ZERO; p2 < fe_end; ++p2) sum_kpp[0] += kpp_ksq_pcpc(sq, p1, p2);
		...
		cout << "{" << (int)sum_kk[0] << ":" << (int)sum_kkp[0] << ":" << (int)sum_kpp[0] << "," << (int)sum_kk[1]
		     << ":" << (int)sum_kkp[1] << ":" << (int)sum_kpp[1] << "} ";
	});
	...
	for_all_sq([](Square sq) {
		...
		for (Square sq2 = SQ_ZERO; sq2 < SQ_NB; ++sq2) {
			sum_kk += kk[sq2][sq];
			for (BonaPiece p = BONA_PIECE_ZERO; p < fe_end; ++p) sum_kkp += kkp[sq2][sq][p];
		}
...
void print_eval_stat(Position& pos) {
	...
	for (PieceNumber i = PIECE_NUMBER_ZERO; i < PIECE_NUMBER_NB; ++i) {
		...
		if (fb != fb_new || fw != fw_new)
			std::cout << "PieceNumber = " << i << " , fb = " << (int)fb << ":" << fb << " , fw = " << (int)fw << ":"
			          << fw << " , fb_new = " << (int)fb_new << " , fw_new = " << (int)fw_new
			          ...
			          << " , inv(fw_new) = " << (int)EvalLearningTools::inv_piece(fw_new) << std::endl;
	}
#endif
	...
	for (i = 0; i < PIECE_NUMBER_KING; ++i) {
		...
		cout << "KKP : " << sq_bk << " " << sq_wk << " " << k0 << " = " << kkp[sq_bk][sq_wk][k0][0] << " + "
		     << kkp[sq_bk][sq_wk][k0][1] << endl;
	}
