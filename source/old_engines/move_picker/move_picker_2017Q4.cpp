...
// 通常探索から呼び出されるとき用。
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh, const PieceToHistory** ch,
                       Move cm, Move* killers_p)
    : pos(p), mainHistory(mh), contHistory(ch), countermove(cm), killers{killers_p[0], killers_p[1]}, depth(d) {
...
MovePicker::MovePicker(const Position& p, Move ttm, Value th) : pos(p), threshold(th) {
	...
	ttMove = ttm && pos.pseudo_legal_s<false>(ttm) && pos.capture(ttm) && pos.see_ge(ttm, threshold) ? ttm : MOVE_NONE;
	...
	for (auto& m : *this) {
		if (Type == CAPTURES) {
			...
			m.value =  // (pawn_promo ? (Value)(Eval::ProDiffPieceValue[PAWN]) : VALUE_ZERO) +
			    (Value)Eval::CapturePieceValue[pos.piece_on(to_sq(m))] - LVA(pt);
			...
			m.value = (*mainHistory)[from_to(m)][c] + (*contHistory[0])[movedSq][movedPiece] +
			          (*contHistory[1])[movedSq][movedPiece] + (*contHistory[3])[movedSq][movedPiece];
		} else  // Type == EVASIONS
		{
			// 王手回避の指し手をスコアリングする。
...
Move MovePicker::next_move2() {
	...
	switch (stage) {
		...
		case COUNTERMOVE:
			++stage;
			move = countermove;
			if (move != MOVE_NONE && move != ttMove && move != killers[0] && move != killers[1] &&
		...
		case QUIET:
			while (cur < endMoves && (!skipQuiets || cur->value >= VALUE_ZERO)) {
				move = *cur++;
				if (move != ttMove && move != killers[0] && move != killers[1] && move != countermove) return move;
			}
			++stage;
		...
		case PROBCUT_CAPTURES:
			while (cur < endMoves) {
				move = pick_best(cur++, endMoves);
				if (move != ttMove && pos.see_ge(move, threshold)) return move;
			}
			break;
