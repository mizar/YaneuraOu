...
struct MovePicker {
	...
	MovePicker(const Position& pos_, Move ttMove_, Depth depth_, const HistoryStats& history_,
	           const CounterMoveStats& cmh, const CounterMoveStats& fmh, Move counterMove_, Search::Stack* ss_)
	    : pos(pos_),
	      history(history_),
	      counterMoveHistory(&cmh),
	      followupMoveHistory(&fmh),
	      ss(ss_),
	      counterMove(counterMove_),
	      depth(depth_) {
		// 次の指し手生成の段階
	...
	MovePicker(const Position& p, Move ttMove_, const HistoryStats& h, Value threshold_)
		...
		ttMove =
		    ttMove_ && pos.pseudo_legal_s<false>(ttMove_) && pos.capture(ttMove_) && pos.see_ge(ttMove_, threshold + 1)
		        ? ttMove_
		        : MOVE_NONE;
	...
	void generate_next_stage() {
		...
		switch (++stage) {
			...
			case KILLERS:
				// killer,counter
				// moveを32bitで持つとき、ExtMoveの上位に駒種を格納しておき、取り出したときにチェックする。
		...
		while (true) {
			...
			switch (stage) {
				...
				case SEARCH_QUIETS:
					...
					if (move != ttMove && move != (Move)killers[0] && move != (Move)killers[1] &&
					    move != (Move)killers[2])
						return move;
					break;
   ...
   private:
	...
	void score_quiets() {
		for (auto& m : *this) {
			...
			m.value   = history[move_to(m)][mpc] + (*counterMoveHistory)[move_to(m)][mpc] +
	...
	void score_evasions() {
		for (auto& m : *this)
			...
			if (pos.capture(m)) {
				...
				if (is_promote(m)) m.value += (Eval::ProDiffPieceValue[raw_type_of(pos.moved_piece_after(m))]);
			} else
			// ↓のifがぶら下がっている。
