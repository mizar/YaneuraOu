...
// 通常探索から呼び出されるとき用。
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, Search::Stack* s) : pos(p), ss(s), depth(d) {
...
// 静止探索から呼び出される時用。
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, Square recapSq) : pos(p) {
#ifdef MUST_CAPTURE_SHOGI_ENGINE
	checkMustCapture();
#endif
...
MovePicker::MovePicker(const Position& p, Move ttm, Value th) : pos(p), threshold(th) {
	...
	ttMove =
	    ttm && pos.pseudo_legal_s<false>(ttm) && pos.capture(ttm) && pos.see_ge(ttm, threshold + 1) ? ttm : MOVE_NONE;
...
Move MovePicker::next_move() {
	...
	while (true) {
		...
		// のいずれか
		if (!mustCapture || pos.capture(move)) return move;
	}
}
Move MovePicker::next_move2() {
	...
	switch (stage) {
		...
		case GOOD_CAPTURES:
			...
			move = ss->killers[0];
			if (move != MOVE_NONE && move != ttMove && pos.pseudo_legal_s<false>(move) &&
		...
		case COUNTERMOVE:
			++stage;
			move = countermove;
			if (move != MOVE_NONE && move != ttMove && move != ss->killers[0] && move != ss->killers[1] &&
			    pos.pseudo_legal_s<false>(move) && !pos.capture_or_pawn_promotion(move))
				return move;
		...
		case QUIET_INIT:
			...
			if (depth < 3 * ONE_PLY) {
				ExtMove* goodQuiet =
				    std::partition(cur, endMoves, [](const ExtMove& m) { return m.value > VALUE_ZERO; });
		...
		case QUIET:
			while (cur < endMoves) {
				move = *cur++;
				if (move != ttMove && move != ss->killers[0] && move != ss->killers[1] && move != countermove)
					return move;
			}
			++stage;
			...
			// see()が負の指し手を返す。
		case BAD_CAPTURES:
			if (cur < endBadCaptures) return *cur++;
			break;
		...
		case QCHECKS:
			while (cur < endMoves) {
				move = cur++->move;
				if (move != ttMove && !pos.pawn_promotion(move)) return move;
			}
			break;
