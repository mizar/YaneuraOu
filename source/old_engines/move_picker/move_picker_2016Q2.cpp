...
static const Value LVATable[PIECE_WHITE] = {
    Value(0),        Value(1) /*歩*/, Value(2) /*香*/,     Value(3) /*桂*/, Value(4) /*銀*/, Value(7) /*角*/,
    Value(8) /*飛*/, Value(6) /*金*/, Value(10000) /*王*/, Value(5) /*と*/, Value(5) /*杏*/, Value(5) /*圭*/,
    Value(5) /*全*/, Value(9) /*馬*/, Value(10) /*龍*/,    Value(11) /*成金*/
};
...
MovePicker::MovePicker(const Position& pos_, Move ttMove_, Depth depth_, Search::Stack* ss_)
	...
	countermove = is_ok((ss - 1)->currentMove) ? pos.this_thread()->counterMoves[prevSq][prevPc] : MOVE_NONE;
...
// 静止探索から呼び出される時用。
MovePicker::MovePicker(const Position& pos_, Move ttMove_, Depth depth_, Square recapSq) : pos(pos_) {
...
MovePicker::MovePicker(const Position& pos_, Move ttMove_, Value threshold_) : pos(pos_), threshold(threshold_) {
	...
	ttMove = ttMove_ && pos.pseudo_legal_s<false>(ttMove_) && pos.capture(ttMove_) && pos.see_ge(ttMove_, threshold + 1)
	             ? ttMove_
	             : MOVE_NONE;
...
void MovePicker::generate_next_stage() {
	...
	switch (++stage) {
		...
		case SEARCH_QUIETS:
			...
			if (depth < 3 * ONE_PLY) {
				auto goodQuiet =
				    std::partition(currentMoves, endMoves, [](const ExtMove& m) { return m.value > VALUE_ZERO; });
		...
		case ALL_EVASIONS:
			...
			if (endMoves - moves > 1) score_evasions();
			break;
		...
		case EVASION_START:
		case QSEARCH_WITH_CHECKS_START:
		case QSEARCH_WITHOUT_CHECKS_START:
		case PROBCUT_START:
		case RECAPTURE_START:
		case STOP:
			stage = STOP;
			break;
...
Move MovePicker::next_move() {
	...
	while (true) {
		while (currentMoves == endMoves && stage != STOP) generate_next_stage();
		...
		switch (stage) {
			...
			case PROBCUT_START:
				...
				// また、killerの3つ目はcounter
				// moveでこれは先後の区別がないのでpseudo_legal_s<X,true>()を呼び出す必要がある。
			case KILLERS:
				move = *currentMoves++;
			...
			case GOOD_CAPTURES:
				...
				if (move != ttMove) {
					...
					if (pos.see_ge(move, VALUE_ZERO)) return move;
			...
			case SEARCH_QUIETS:
				...
				if (move != ttMove && move != (Move)killers[0] && move != (Move)killers[1] && move != (Move)killers[2])
					return move;
				break;
#else
				// killerの個数に応じて分岐
			...
			case QUIETS1:
				move = *currentMoves++;
				if (move != (Move)killers[0]) return move;
				break;
			case QUIETS2:
				move = *currentMoves++;
				if (move != (Move)killers[0] && move != (Move)killers[1]) return move;
				break;
			case QUIETS3:
				move = *currentMoves++;
				if (move != (Move)killers[0] && move != (Move)killers[1] && move != (Move)killers[2]) return move;
				break;
			case QUIETS4:
				move = *currentMoves++;
				if (move != (Move)killers[0] && move != (Move)killers[1] && move != (Move)killers[2] &&
				    move != (Move)killers[3])
					return move;
				break;
#endif
			...
			case PROBCUT_CAPTURES:
				move = pick_best(currentMoves++, endMoves);
				if (move != ttMove && pos.see_ge(move, threshold + 1)) return move;
				break;
...
void MovePicker::score_captures() {
	...
	for (auto& m : *this) {
		...
		m.value = (pawn_promo ? (Value)(Eval::ProDiffPieceValue[PAWN]) : VALUE_ZERO) +
		          (Value)Eval::CapturePieceValue[pos.piece_on(move_to(m))] - LVA(pt);
...
void MovePicker::score_quiets() {
	...
	for (auto& m : *this) {
		...
		m.value   = history[move_to(move)][mpc] + (cm ? (*cm)[move_to(move)][mpc] : VALUE_ZERO) +
		          (fm ? (*fm)[move_to(move)][mpc] : VALUE_ZERO) + (f2 ? (*f2)[move_to(move)][mpc] : VALUE_ZERO) +
		          fromTo.get(c, m);
	}
}
