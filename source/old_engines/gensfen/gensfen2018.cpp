	...
	while (!quit) {
		...
		auto flush_psv = [&](s8 lastTurnIsWin) {
			...
			for (auto it = a_psv.rbegin(); it != a_psv.rend(); ++it) {
				...
				if (POPCNT64(loop_count) == 1) {
					sync_cout << endl
					          << "loop_count = " << loop_count << " , node_info_count = " << node_info_count
					          << sync_endl;
		...
		for (int ply = 0; ply < MAX_PLY2; ++ply) {
			...
			if (opening) {
				...
				if (key == entry.key) {
					// 局面、おそらく循環しちゃってるので終了。
					if (pos.game_ply() > entry.game_ply) break;
				...
				for (u64 i = 0; i < length /* && count < GENSFEN_MULTI_PV */; ++i) {
					...
					if (value < -400 || 400 < value) continue;

					moves[count++] = (u16)rm[i].pv[0];
				}
				if (count) {
					for (int i = 0; i < count; ++i) entry.moves[i] = moves[i];
...

			{
				...
				if (abs(value1) >= eval_limit) {
					//					sync_cout << pos << "eval limit = " << eval_limit << " over , move = " << pv1[0] <<
					//sync_endl;
