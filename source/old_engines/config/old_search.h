...
struct LimitsType {
	...
	LimitsType() {
		nodes = time[WHITE] = time[BLACK] = inc[WHITE] = inc[BLACK] = byoyomi[WHITE] = byoyomi[BLACK] = npmsec = depth =
		    movetime = mate = infinite = ponder = rtime = 0;
...
struct Stack {
	...
	Move  excludedMove;  // singular
	                    // extension判定のときに置換表の指し手をそのnodeで除外して探索したいのでその除外する指し手
