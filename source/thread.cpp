...
void Thread::idle_loop() {
	...
	// https://github.com/official-stockfish/Stockfish/commit/1c50d8cbf554733c0db6ab423b413d75cc0c1928 その後、
	// ・fishtestを8スレッドで行うから、9以上にしてくれとのことらしい。
	// cf. NUMA for 9 threads or more :
	// https://github.com/official-stockfish/Stockfish/commit/bc3b148d5712ef9ea00e74d3ff5aea10a4d3cabe
...
void ThreadPool::set(size_t requested) {
	if (size() > 0) {  // いったんすべてのスレッドを解体(NUMA対策)
		...
		while (size() > 0) delete back(), pop_back();
	}

	if (requested > 0) {  // 要求された数だけのスレッドを生成
		push_back(new MainThread(0));

		while (size() < requested) push_back(new Thread(size()));
		clear();
...
void ThreadPool::start_thinking(const Position& pos, StateListPtr& states, const Search::LimitsType& limits,
	...
	if (pos.DeclarationWin() == MOVE_WIN) rootMoves.emplace_back(MOVE_WIN);
#endif
	...
	if (states.get()) setupStates = std::move(states);  // Ownership transfer, states is now empty
	...
	// cf. Fix incorrect StateInfo :
	// https://github.com/official-stockfish/Stockfish/commit/232c50fed0b80a0f39322a925575f760648ae0a5
...
Thread* ThreadPool::get_best_thread() const {
	...
	for (Thread* th : *this) minScore = std::min(minScore, th->rootMoves[0].score);

	// Vote according to score and depth, and select the best thread
	for (Thread* th : *this) {
		votes[th->rootMoves[0].pv[0]] += (th->rootMoves[0].score - minScore + 14) * int(th->completedDepth);

		if (abs(bestThread->rootMoves[0].score) >= VALUE_TB_WIN_IN_MAX_PLY) {
			// Make sure we pick the shortest mate / TB conversion or stave off mate the longest
			if (th->rootMoves[0].score > bestThread->rootMoves[0].score) bestThread = th;
...
void ThreadPool::start_searching() {
	for (Thread* th : *this)
		if (th != front()) th->start_searching();
}
...
void ThreadPool::wait_for_search_finished() const {
	for (Thread* th : *this)
		if (th != front()) th->wait_for_search_finished();
}
