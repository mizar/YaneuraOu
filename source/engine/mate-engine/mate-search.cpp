...
struct MovePicker {
	...
	MovePicker(Position& pos, bool or_node) {
		...
		// ここで除外しておく。
		endMoves = std::remove_if(moves, endMoves, [&pos](const auto& move) { return !pos.legal(move); });
	}

	// 生成された指し手が空であるかの判定
	bool empty() const { return moves == endMoves; }
...
struct TranspositionTable {
	...
	virtual ~TranspositionTable() { Release(); }

	// 置換表のサイズ(メモリクリアするときに必要。単位はbytes。
	size_t Size() const { return sizeof(Cluster) * num_clusters; }
	...
	TTEntry& LookUp(Key key, Color root_color) {
		...
		for (auto& entry : entries.entries)
			if (hash_high == entry.hash_high && entry.generation == generation) return entry;
	...
	TTEntry& LookUp(Position& n, Color root_color) { return LookUp(n.key(), root_color); }
	...
	int hashfull() const {
		...
		for (int cluster_index = 0;; ++cluster_index)
			for (int entry_index = 0; entry_index < Cluster::kNumEntries; ++entry_index) {
				...
				// 世代が同じ時は使用中であるとみなせる。
				if (entry.generation == generation) ++num_used;

				if (++num_checked == 1000) return num_used;
			}
	}
...
void DFPNwithTCA(Position& n, uint32_t thpn, uint32_t thdn, bool inc_flag, bool or_node, uint16_t depth,
	...
	if (nodes_searched && (nodes_searched % 1000000) == 0) {
		...
		auto time_ms      = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - start_time).count();
		time_ms           = std::max(time_ms, decltype(time_ms)(1));
		int64_t nps       = nodes_searched * 1000LL / time_ms;

		sync_cout << "info  time " << time_ms << " nodes " << nodes_searched << " nps " << nps << " hashfull "
		          << transposition_table.hashfull() << sync_endl;
	}
	...
	for (const auto& move : move_picker) {
		...
		if (entry.minimum_distance > child_entry.minimum_distance && child_entry.pn != kInfinitePnDn &&
	...
	while (!Threads.stop.load(std::memory_order_relaxed)) {
		...
		DFPNwithTCA(n, thpn_child, thdn_child, inc_flag, !or_node, depth + 1, root_color, start_time, timeup);
...
// 詰み手順を1つ返す
// 最短の詰み手順である保証はない
bool SearchMatePvFast(bool or_node, Color root_color, Position& pos, std::vector<Move>& moves,
...
void dfpn(Position& r) {
	...
	while (!Threads.stop && (Threads.main()->ponder || Limits.infinite)) Tools::sleep(1);
...
void USI::extra_option(USI::OptionsMap& o) { o[MateEngine::kMorePreciseMatePv] << USI::Option(true); }
...
void Thread::search() {
	...
	if (Search::Limits.mate == 0) {
		// "go infinite"に対してはstopが送られてくるまで待つ。
		while (!Threads.stop && Limits.infinite) Tools::sleep(1);
