...
Value futility_margin(Depth d, bool improving) { return Value(PARAM_FUTILITY_MARGIN_ALPHA1 /*224*/ * (d - improving)); }
	...
	while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
		...
		if (!rootNode
			...
			if (!captureOrPawnPromotion && !givesCheck) {
				...
				if (lmrDepth < PARAM_FUTILITY_AT_PARENT_NODE_DEPTH /*7*/
				    && !ss->inCheck &&
				    ss->staticEval + PARAM_FUTILITY_AT_PARENT_NODE_MARGIN1 /*266*/ +
		...
		// clang-format on
		{
			...
			if (!captureOrPawnPromotion) {
				...
				if (!givesCheck && ss->staticEval + CapturePieceValue[pos.captured_piece()] + 210 * depth <= alpha)
					r++;
			}
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	while ((move = mp.next_move()) != MOVE_NONE) {
		...
		if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY
		    // !ss->inCheck
		    // これ不要らしい。cf.
		    // https://github.com/official-stockfish/Stockfish/commit/392b529c3f52103ad47ad096b86103c17758cb4f
		    && !pos.see_ge(move))
			continue;
