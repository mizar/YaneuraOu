...
Value futility_margin(Depth d, bool improving) { return Value(PARAM_FUTILITY_MARGIN_ALPHA1 /*223*/ * (d - improving)); }
...
constexpr int futility_move_count(bool improving, int depth) { return (3 + depth * depth) / (2 - improving); }
	...
	explicit ThreadHolding(Thread* thisThread, Key posKey, int ply) {
		...
		if (location) {
			...
			if (tmp == nullptr) {
				...
				owning = true;
			} else if (tmp != thisThread && (*location).key.load(std::memory_order_relaxed) == posKey)
				otherThread = true;
		}
	}
...
uint64_t perft(Position& pos, Depth depth) {
	...
	for (const auto& m : MoveList<LEGAL_ALL>(pos)) {
		...
		if (Root) sync_cout << USI::move(m) << ": " << cnt << sync_endl;
	}
	return nodes;
}
...
void MainThread::search() {
	...
	if (rootMoves.empty()) {
		...
		if (!Limits.silent) sync_cout << USI::pv(rootPos, 1, -VALUE_INFINITE, VALUE_INFINITE) << sync_endl;
	...
	if (book.probe(*this, Limits)) goto SKIP_SEARCH;
...
SKIP_SEARCH:;
	...
	if (Options["MultiPV"] == 1 && !Limits.depth &&
	    !Skill((int)Options["SkillLevel"]).enabled()
	    //&& rootMoves[0].pv[0] != MOVE_NONE //
	    //やねうら王では投了の局面でMOVE_NONEを突っ込まないのでこのチェックは不要。
	...
	if (bestThread != this && !Limits.silent && !Limits.consideration_mode)
		sync_cout << USI::pv(bestThread->rootPos, bestThread->completedDepth, -VALUE_INFINITE, VALUE_INFINITE)
		          << sync_endl;
...
void Thread::search() {
	...
	while (++rootDepth < MAX_PLY && !Threads.stop && !(Limits.depth && mainThread && rootDepth > Limits.depth)) {
		...
		for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx) {
			...
			while (true) {
				...
				if (mainThread
				    ...
				    && multiPV == 1 && (bestValue <= alpha || bestValue >= beta) &&
				    Time.elapsed() > 3000
		...
		if (multiPV == 1) {
			...
			if (Limits.mate && bestValue >= VALUE_MATE_IN_MAX_PLY && VALUE_MATE - bestValue <= Limits.mate)
				Threads.stop = true;

			// 勝ちを読みきっているのに将棋所の表示が追いつかずに、将棋所がフリーズしていて、その間の時間ロスで
			// 時間切れで負けることがある。
			// mateを読みきったとき、そのmateの倍以上、iterationを回しても仕方ない気がするので探索を打ち切るようにする。
			if (!Limits.mate && bestValue >= VALUE_MATE_IN_MAX_PLY && (VALUE_MATE - bestValue) * 2 < (Value)(rootDepth))
				break;
		...
		// とりあえず、無いよりマシだろう。
		if (mainThread->rootMoves[0].pv.size() > 1) mainThread->ponder_candidate = mainThread->rootMoves[0].pv[1];
		...
		if (Limits.use_time_management()) {
			...
			if (!Threads.stop && Time.search_end == 0) {
				...
				double totalTime =
				    rootMoves.size() == 1 ? 0 : Time.optimum() * fallingEval * reduction * bestMoveInstability;
				...
				else if (Threads.increaseDepth && !mainThread->ponder && Time.elapsed() > totalTime * 0.58)
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
	...
	if (/* pos.rule50_count() >= 3
	    &&*/ alpha < ValueDraw && !rootNode && pos.has_game_cycle(ss->ply)) {
	...
	//   https://github.com/official-stockfish/Stockfish/commit/c5f6bd517c68e16c3ead7892e1d83a6b1bb89b69 cf. Use
	//   evaluation trend to adjust futility margin :
	...
	// singularQuietLMR     :
	bool captureOrPawnPromotion, doFullDepthSearch, moveCountPruning, ttCapture, singularQuietLMR;
	...
	if (!rootNode) {
		...
		if (draw_type != REPETITION_NONE) return value_from_tt(draw_value(draw_type, pos.side_to_move()), ss->ply);
	...
	// cf. More robust interaction of singular search and iid :
	...
	thisThread->ttHitAverage =
	    (TtHitAverageWindow - 1) * thisThread->ttHitAverage / TtHitAverageWindow + TtHitAverageResolution * ss->ttHit;
	...
	if (!PvNode  // PV nodeでは置換表の指し手では枝刈りしない(PV nodeはごくわずかしかないので..)
		...
		if (ttMove) {
			if (ttValue >= beta) {
				...
				if (!pos.capture(ttMove)) update_quiet_stats(pos, ss, ttMove, stat_bonus(depth), depth);
...

	{
		...
		if (!ttMove || PvNode) {
			...
			if (m != MOVE_NONE) {
				...
				tte->save(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv, BOUND_EXACT, MAX_PLY, m, ss->staticEval);
		...
		if (!rootNode && !ss->ttHit && !ss->inCheck) {
			if (PARAM_WEAK_MATE_PLY == 1) {
				...
				if (move != MOVE_NONE) {
					...
					tte->save(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv, BOUND_EXACT, MAX_PLY, move,
	...
	if (ss->inCheck) {
		...
		if (eval == VALUE_NONE) ss->staticEval = eval = evaluate(pos);
	...
	if (!rootNode  // The required rootNode PV handling is not available in qsearch
	    && depth == 1 && eval <= alpha - PARAM_RAZORING_MARGIN /* == RazorMargin == 510 */)
	...
	if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < PARAM_NULL_MOVE_MARGIN0 /*22977*/
	...
	if (!PvNode && depth > PARAM_PROBCUT_DEPTH /*4*/
	    ...
	    && !(ss->ttHit && tte->depth() >= depth - (PARAM_PROBCUT_DEPTH - 1) && ttValue != VALUE_NONE &&
	         ttValue < probCutBeta)) {
		...
		if (ss->ttHit && tte->depth() >= depth - (PARAM_PROBCUT_DEPTH - 1) && ttValue != VALUE_NONE &&
		    ttValue >= probCutBeta && ttMove && pos.capture_or_promotion(ttMove))
			return probCutBeta;
		...
		while ((move = mp.next_move()) != MOVE_NONE && probCutCount < 2 + 2 * cutNode) {
			if (move != excludedMove && pos.legal(move)) {
				...
				ss->currentMove         = move;
				ss->continuationHistory = &thisThread->continuationHistory[ss->inCheck][captureOrPawnPromotion]
				                                                          [to_sq(move)][pos.moved_piece_after(move)];
				...
				if (value >= probCutBeta)
					value = -search<NonPV>(pos, ss + 1, -probCutBeta, -probCutBeta + 1, depth - PARAM_PROBCUT_DEPTH,
					                       !cutNode);
...
moves_loop:
	...
	while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
		...
		if (rootNode &&
		    !std::count(thisThread->rootMoves.begin() + thisThread->pvIdx, thisThread->rootMoves.end(), move))
			continue;
		...
		if (!rootNode
			...
			if (!captureOrPawnPromotion && !givesCheck) {
				...
				if (lmrDepth <
				        PARAM_PRUNING_BY_HISTORY_DEPTH /*4*/ + ((ss - 1)->statScore > 0 || (ss - 1)->moveCount == 1) &&
				...
				if (lmrDepth < PARAM_FUTILITY_AT_PARENT_NODE_DEPTH /*7*/
				    && !ss->inCheck &&
				    ss->staticEval + PARAM_FUTILITY_AT_PARENT_NODE_MARGIN1 /*283*/ +
				            PARAM_FUTILITY_MARGIN_BETA /*170*/ * lmrDepth <=
				        alpha &&
				    (*contHist[0])[to_sq(move)][movedPiece] + (*contHist[1])[to_sq(move)][movedPiece] +
				            (*contHist[3])[to_sq(move)][movedPiece] + (*contHist[5])[to_sq(move)][movedPiece] / 2 <
				        27376)
					continue;
				...
				if (!pos.see_ge(move, Value(-(PARAM_FUTILITY_AT_PARENT_NODE_GAMMA1 /*29*/
				                              - std::min(lmrDepth, PARAM_FUTILITY_AT_PARENT_NODE_GAMMA2 /*18*/)) *
				                            lmrDepth * lmrDepth)))
					continue;
			} else {
		...
		else if (givesCheck && (pos.is_discovery_check_on_king(~us, move) || pos.see_ge(move)))
			extension = 1;
		...
		ss->continuationHistory =
		    &thisThread->continuationHistory[ss->inCheck][captureOrPawnPromotion][to_sq(move)][movedPiece];
		...
		if (depth >= 3 && moveCount > 1 + 3 * rootNode &&
		    (!captureOrPawnPromotion || moveCountPruning ||
		     ss->staticEval + CapturePieceValue[pos.captured_piece()] <= alpha || cutNode ||
		     thisThread->ttHitAverage < 427 * TtHitAverageResolution * TtHitAverageWindow / 1024)) {
			// Reduction量
			Depth r = reduction(improving, depth, moveCount);

			// Decrease reduction if the ttHit running average is large
			if (thisThread->ttHitAverage > 509 * TtHitAverageResolution * TtHitAverageWindow / 1024) r--;
			...
			// best
			// moveが頻繁に変更されていないならば局面が安定しているのだろうから、rootとnon-PVではreductionを増やす。
			if ((rootNode || !PvNode) && depth > 10 && thisThread->bestMoveChanges <= 2) r++;

			if (moveCountPruning && !formerPv) r++;
			...
			if (!captureOrPawnPromotion) {
				...
				ss->statScore = thisThread->mainHistory[from_to(move)][us] + (*contHist[0])[to_sq(move)][movedPiece] +
				                (*contHist[1])[to_sq(move)][movedPiece] + (*contHist[3])[to_sq(move)][movedPiece] -
				...
				if (depth < 8 && moveCount > 2) r++;

				// Unless giving check, this capture is likely bad
				if (!givesCheck && ss->staticEval + CapturePieceValue[pos.captured_piece()] + 213 * depth <= alpha) r++;
			}
		...
		if (move != bestMove) {
			...
			if (captureOrPawnPromotion && captureCount < 32) capturesSearched[captureCount++] = move;

			// 探索した駒を捕獲しない指し手を64手目までquietsSearchedに登録しておく。
			// あとでhistoryなどのテーブルに加点/減点するときに使う。

			if (!captureOrPawnPromotion && quietCount < 64) quietsSearched[quietCount++] = move;
		}
	}
	// end of while
	...
	if (!moveCount) bestValue = excludedMove ? alpha : mated_in(ss->ply);
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	if (PvNode) {
		// PV
		// nodeではalpha値を上回る指し手が存在した場合は(そこそこ指し手を調べたので)置換表にはBOUND_EXACTで保存したいから、
		// そのことを示すフラグとして元の値が必要(non PVではこの変数は参照しない)
		// PV
		// nodeでalpha値を上回る指し手が存在しなかった場合は、調べ足りないのかも知れないからBOUND_UPPERとしてbestValueを保存しておく。
		oldAlpha = alpha;
	...
	if (ss->inCheck) {
		...
		if (PARAM_QSEARCH_MATE1 && !ss->ttHit) {
			...
			if (PARAM_WEAK_MATE_PLY == 1) {
				if (pos.mate1ply() != MOVE_NONE) return mate_in(ss->ply + 1);
			} else {
		...
		if (ss->ttHit) {
			...
			if (!PARAM_QSEARCH_FORCE_EVAL) {
				// Stockfish相当のコード
				ss->staticEval = bestValue =
				    (ss - 1)->currentMove != MOVE_NULL ? evaluate(pos) : -(ss - 1)->staticEval + 2 * PARAM_EVAL_TEMPO;
	...
	MovePicker mp(pos, ttMove, depth, &thisThread->mainHistory, &thisThread->captureHistory, contHist,
	...
	while ((move = mp.next_move()) != MOVE_NONE) {
		...
		if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY
		    ...
		    // !ss->inCheck
		    && !givesCheck && futilityBase > -VALUE_KNOWN_WIN
		...
		if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY
		    // !ss->inCheck
		    // cf. https://github.com/official-stockfish/Stockfish/commit/392b529c3f52103ad47ad096b86103c17758cb4f
		    && !(givesCheck && pos.is_discovery_check_on_king(~pos.side_to_move(), move)) && !pos.see_ge(move))
			continue;
		...
		ss->continuationHistory =
		    &thisThread
		         ->continuationHistory[ss->inCheck][captureOrPawnPromotion][to_sq(move)][pos.moved_piece_after(move)];
	...
	if (ss->inCheck && bestValue == -VALUE_INFINITE) return mated_in(ss->ply);  // rootからの手数による詰みである。
...
void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
	...
	if (!pos.capture_or_promotion(bestMove)) {
		...
		for (int i = 0; i < quietCount; ++i) {
			...
			update_continuation_histories(ss, pos.moved_piece_after(quietsSearched[i]), to_sq(quietsSearched[i]),
			                              -bonus2);
		}
	} else
	...
	// Extra penalty for a quiet early move that was not a TT move or main killer move in previous ply when it gets
	// refuted (ss-1)->ttHit : 一つ前のnodeで置換表にhitしたか
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
	for (int i : {1, 2, 4, 6}) {
		...
		if (is_ok((ss - i)->currentMove)) (*(ss - i)->continuationHistory)[to][pc] << bonus;
	}
}
...
void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth) {
	...
	if (depth > 11 && ss->ply < MAX_LPH) thisThread->lowPlyHistory[ss->ply][from_to(move)] << stat_bonus(depth - 7);
}
...
Move Skill::pick_best(size_t multiPV) {
	...
	for (size_t i = 0; i < multiPV; ++i) {
		// これが魔法の公式
		int push = (weakness * int(topScore - rootMoves[i].score) + delta * (rng.rand<unsigned>() % weakness)) / 128;
...
#if defined(ENABLE_OUTPUT_GAME_RESULT)
		{
			if (!result_log.is_open()) result_log.open(Options["PARAMETERS_LOG_FILE_PATH"], std::ios::app);
			// 今回のパラメーターをログファイルに書き出す。
			for (size_t i = 0; i < param_names.size(); ++i) {
				if (param_names[i] == "FIXED") continue;
...
void init_for_search(Position& pos, Stack* ss) {
	...
	// this_threadに関して。
	{
		...
		for (int i = 7; i > 0; i--) (ss - i)->continuationHistory = &th->continuationHistory[0][0][SQ_ZERO][NO_PIECE];

		// rootMovesの設定
		auto& rootMoves = th->rootMoves;

		rootMoves.clear();
		for (auto m : MoveList<LEGAL>(pos)) rootMoves.push_back(Search::RootMove(m));
...
ValueAndPV qsearch(Position& pos) {
	...
	// 得られたPVを返す。
	for (Move* p = &ss->pv[0]; is_ok(*p); ++p) pvs.push_back(*p);
...
ValueAndPV search(Position& pos, int depth_, size_t multiPV /* = 1 */, u64 nodesLimit /* = 0 */) {
	...
	Depth depth = depth_;
	if (depth < 0) return std::pair<Value, std::vector<Move>>(Eval::evaluate(pos), std::vector<Move>());

	if (depth == 0) return qsearch(pos);
