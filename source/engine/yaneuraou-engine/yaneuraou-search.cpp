...
moves_loop:
	...
	while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
		...
		else if (givesCheck && (pos.blockers_for_king(~us) & from_sq(move) || pos.see_ge(move)))
			extension = 1;
		...
		if (doFullDepthSearch) value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha, newDepth, !cutNode);
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	while ((move = mp.next_move()) != MOVE_NONE) {
		...
		if (!ss->inCheck && !givesCheck && futilityBase > -VALUE_KNOWN_WIN
		    //	&& !pos.advanced_pawn_push(move))
		) {
		...
		if (!ss->inCheck && !(givesCheck && pos.is_discovery_check_on_king(~pos.side_to_move(), move)) &&
		    !pos.see_ge(move))
			continue;
