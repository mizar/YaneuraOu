...
std::string pretty(File f) {
	return pretty_jp ? std::string("１２３４５６７８９").substr((size_t)f * 2, 2) : std::to_string((size_t)f + 1);
}
std::string pretty(Rank r) {
	return pretty_jp ? std::string("一二三四五六七八九").substr((size_t)r * 2, 2) : std::to_string((size_t)r + 1);
}
...
std::string pretty(Move m, Piece movedPieceType) {
	if (is_drop(m))
		...
		return pretty(move_to(m)) + pretty2(movedPieceType) + (is_promote(m) ? (pretty_jp ? "成" : "+") : "") + "[" +
		       pretty(move_from(m)) + "]";
}
...
std::ostream& operator<<(std::ostream& os, Color c) {
	os << ((c == BLACK) ? (pretty_jp ? "先手" : "BLACK") : (pretty_jp ? "後手" : "WHITE"));
	return os;
}
...
bool RootMove::extract_ponder_from_tt(Position& pos, Move ponder_candidate) {
	...
	if (ttHit) {
		...
		if (MoveList<LEGAL_ALL>(pos).contains(m)) goto FOUND;
	}
