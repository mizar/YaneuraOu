...
struct Weight {
	...
	static void calc_eta(u64 epoch) {
		if (Weight::eta1_epoch == 0)  // eta2適用除外
			...
			Weight::eta = Weight::eta2 + (Weight::eta3 - Weight::eta2) * (epoch - Weight::eta1_epoch) /
			                                 (Weight::eta2_epoch - Weight::eta1_epoch);
		else
	...
	// gradの設定
	template <typename T>
	void set_grad(const T& g_) {
		g = g_;
	}

	// gradの加算
	template <typename T>
	void add_grad(const T& g_) {
		g += g_;
	}
...
struct Weight2 {
	...
	void updateFV(std::array<T, 2>& v) {
		w[0].updateFV(v[0], 1.0);
		w[1].updateFV(v[1], 1.0 / 8.0);
	}

	template <typename T>
	void set_grad(const std::array<T, 2>& g) {
		for (int i = 0; i < 2; ++i) w[i].set_grad(g[i]);
	}
	template <typename T>
	void add_grad(const std::array<T, 2>& g) {
		for (int i = 0; i < 2; ++i) w[i].add_grad(g[i]);
	}

	std::array<LearnFloatType, 2> get_grad() const {
		return std::array<LearnFloatType, 2>{w[0].get_grad(), w[1].get_grad()};
	}
};
...
struct SerializerBase {
	...
	/*final*/ u64 toIndex() const { return min_index() + toRawIndex(); }
...
#define KK_LOWER_COUNT 1
	...
	virtual u64 toRawIndex() const { return (u64)king0_ * (u64)max_king_sq_ + (u64)king1_; }

	// toLowerDimensionsで次元下げしたものがinverseしたものであるかを返す。
	bool is_inverse() const { return inverse_sign; }
...
struct KKP : public SerializerBase {
   protected:
	...
	KKP(Square king0, Square king1, Eval::BonaPiece p, bool inverse)
	    : king0_(king0), king1_(king1), piece_(p), inverse_sign(inverse) {}
...
#define KKP_LOWER_COUNT 1
	...
	// 比較演算子
	bool operator==(const KKP& rhs) {
		return king0() == rhs.king0() && king1() == rhs.king1() && piece() == rhs.piece();
	}
...
#define KPP_LOWER_COUNT 1
	...
	bool is_inverse() const { return false; }
...
#define KPPP_LOWER_COUNT 1
	...
	virtual u64 toRawIndex() const {
		...
		auto PcPcPcOnSq = [this](int king, Eval::BonaPiece i, Eval::BonaPiece j, Eval::BonaPiece k) {
			...
			// BonaPiece型は、32bitを想定しているので掛け算には気をつけないとオーバーフローする。
			return (u64)king * triangle_fe_end +
			       (u64)(u64(i) * (u64(i) - 1) * (u64(i) - 2) / 6 + u64(j) * (u64(j) - 1) / 2 + u64(k));
		};
...
struct KKPP : SerializerBase {
   protected:
	KKPP(int king, Eval::BonaPiece p0, Eval::BonaPiece p1) : king_(king), piece0_(p0), piece1_(p1) {
