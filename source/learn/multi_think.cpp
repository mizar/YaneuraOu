...
// 通常探索をして、その結果を返す。
pair<Value, vector<Move> >  MultiThink::search(Position& pos, int depth) { return Learner::search(pos, depth); }

// 静止探索をして、その結果を返す。
pair<Value, vector<Move> > MultiThink::qsearch(Position& pos) { return Learner::qsearch(pos); }
