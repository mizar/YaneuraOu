...
void MultiThinkGenSfen2019::parse_book_file() {
	...
	struct PackedSfenValueHash {
		size_t operator()(const PackedSfenValue& s) const {
			...
			size_t tmp = 0;
			for (int i = 0; i < (int)(sizeof(PackedSfen) / sizeof(size_t)); ++i) tmp ^= ((size_t*)&s.sfen.data)[i];
			return tmp;
		}
	};
	...
	auto my_do_move = [&move, &pos, &states, &count, &line_number, &book_sfens]() {
		...
		// すでに挿入済であればこの局面は無視する。
		if (book_sfens.find(ps) != book_sfens.end()) return;

		book_sfens.insert(ps);
		++count;
	};

	auto out_status = [&count, &line_number] { cout << count << " positions , line_number = " << line_number << endl; };

	ASSERT_LV3(Search::Limits.enteringKingRule = EKR_27_POINT);

	for (auto book_line : my_book) {
		if ((++line_number % 1000) == 0) out_status();
		...
		for (int book_move_index = 2;
		     book_move_index < (int)book_moves.size() &&
		     pos.game_ply() <= MAX_PLY2 - 32 /* あまり直前の局面だと即シミュレーションが終了してしまうので… */
		     ;
		     ++book_move_index) {
			// /* 詰みの局面もゴミでしかない。1手詰め、宣言勝ちの局面も除外。*/
			if (pos.is_mated() || (!pos.checkers() && pos.mate1ply() != MOVE_NONE) || pos.DeclarationWin() != MOVE_NONE)
				break;
			...
			if (!is_ok(move) || !pos.pseudo_legal(move) || !pos.legal(move)) break;
	...
	// vectorに局面をcopy
	my_book_sfens.clear();
	for (auto& it : book_sfens) my_book_sfens.push_back(it);
...
Retry:;
	...
	for (int i = 0; i < random_move_ply; ++i) {
		...
		if (legal_moves.size() == 0) goto Retry;
			// なぜか合法手がないので局面の選択に戻る。
		...
		if (move == MOVE_NONE) move = legal_moves.at(prng.rand(legal_moves.size())).move;
...
void MultiThinkGenSfen2019::thread_worker(size_t thread_id) {
	...
	while (!quit) {
		...
		*/
		...
		// 対局シミュレーションのループ
		while (pos.game_ply() < MAX_PLY2 && !pos.is_mated() && pos.DeclarationWin() == MOVE_NONE &&
			...
			if (abs(lastValue) > eval_limit) break;
