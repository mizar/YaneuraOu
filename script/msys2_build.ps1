...
ForEach-Object{@(
  @{
    ...
    TGTAIL = @{
      ...
      'tournament-sse42' = '-tournament-sse42';
      'evallearn' = '-evallearn-avx2';
      'evallearn-sse42' = '-evallearn-sse42';
      'sse41' = '-sse41';
      'sse2' = '-sse2';
      'nosse' = '-nosse';
    };
...
ForEach-Object{
  ...
  ForEach-Object{
    ...
    $log = $null;
    msys2_shell.cmd -here -defterm -no-start -l -c "nice -n 15 $MAKE -f $MAKEFILE -j$Jobs $($_.Name) YANEURAOU_EDITION=$Edition COMPILER=$Compiler OS=$OS 2>&1"|Tee-Object -Variable log
